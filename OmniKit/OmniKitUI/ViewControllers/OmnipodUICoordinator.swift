...
class OmnipodUICoordinator: UINavigationController, PumpManagerOnboarding, CompletionNotifying, UINavigationControllerDelegate {
    ...
    private func viewControllerForScreen(_ screen: OmnipodUIScreen) -> UIViewController {
        ...
        case .setupComplete:
            ...
            return hostedView
        case .pendingCommandRecovery:
            if let pendingCommand = pumpManager.state.podState?.unacknowledgedCommand,
               pumpManager.state.podState?.needsCommsRecovery == true
            {
                ...
                let dataSource = RileyLinkListDataSource(rileyLinkPumpManager: pumpManager)
                let view = DeliveryUncertaintyRecoveryView(
                    model: model,
                    rileyLinkListDataSource: dataSource,
                    handleRileyLinkSelection: handleRileyLinkSelection
                )
    ...
    init(
        pumpManager: OmnipodPumpManager? = nil,
