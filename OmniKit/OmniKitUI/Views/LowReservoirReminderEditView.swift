...
struct LowReservoirReminderEditView: View {
    ...
    )
    {
        ...
        _selectedValue = State(initialValue: lowReservoirReminderValue)
    }
    ...
    var content: some View {
        VStack {
            RoundedCardScrollView(title: LocalizedString(
                ...
                comment: "Title for low reservoir reminder edit page"
            )) {
    ...
    private func alert(error: Error?) -> SwiftUI.Alert {
        SwiftUI.Alert(
            title: Text(LocalizedString(
                "Failed to Update Low Reservoir Reminder",
                comment: "Alert title for error when updating low reservoir reminder"
            )),
