...
struct LowReservoirReminderSetupView: View {
    ...
    func formatValue(_ value: Int) -> String {
        insulinQuantityFormatter.string(from: HKQuantity(unit: .internationalUnit(), doubleValue: Double(value))) ?? ""
    }
    ...
    var body: some View {
        GuidePage(content: {
            VStack(alignment: .leading, spacing: 15) {
                Text(LocalizedString(
                    "You will be notified when the amount of insulin in the Pod reaches a selected level.\n\nScroll to set the number of units (1 to 50) at which you would like to be reminded.",
                    comment: "Description text on LowReservoirReminderSetupView"
                ))
                Divider()
                HStack {
        ...
        .navigationBarTitle(
            LocalizedString("Low Reservoir", comment: "Navigation bar title for LowReservoirReminderSetupView"),
