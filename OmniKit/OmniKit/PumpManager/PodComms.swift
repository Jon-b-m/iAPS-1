...
class PodComms: CustomDebugStringConvertible {
    ...
    func mockPodStateChanges(_ changes: (_ podState: inout PodState) -> Void) {
        podStateLock.lock()
        changes(&podState!)
        podStateLock.unlock()
    }
    ...
    private func sendPairMessage(
        address: UInt32,
        transport: PodMessageTransport,
        message: Message,
        ...
        while true {
            ...
            guard config.address == address else {
                log.error(
                    "sendPairMessage unexpected address return of %{public}@ instead of expected %{public}@",
    ...
    private func setupPod(
        ...
        let transport = PodMessageTransport(
            ...
            state: podState.messageTransportState
        )
        transport.messageLogger = messageLogger
        ...
        do {
            versionResponse = try sendPairMessage(
                address: podState.address,
                transport: transport,
                message: message,
                insulinType: insulinType
