    ...
    public var errorDescription: String? {
        ...
        case .podAlreadyPaired:
            return LocalizedString(
                "Pod already paired",
                comment: "Error message shown when user cannot pair because pod is already paired"
            )
        case .insulinTypeNotConfigured:
...
public class OmnipodPumpManager: RileyLinkPumpManager {
    ...
    private func setState(_ changes: (_ state: inout OmnipodPumpManagerState) -> Void) {
        setStateWithResult(changes)
    }
    ...
    private func setStateWithResult<ReturnType>(_ changes: (_ state: inout OmnipodPumpManagerState) -> ReturnType) -> ReturnType {
        ...
        if oldValue.podState != newValue.podState {
            ...
            if oldValue.podState?.lastInsulinMeasurements?.reservoirLevel != newValue.podState?.lastInsulinMeasurements?
                .reservoirLevel
            {
                   ...
                   reservoirLevel != Pod.reservoirLevelAboveThresholdMagicNumber
                {
                    pumpDelegate.notify({ delegate in
                        ...
                        delegate?
                            .pumpManager(
                                self,
                                didReadReservoirValue: reservoirLevel,
                                at: lastInsulinMeasurements.validTime
                            ) { _ in
                            }
                    })
                }
            ...
            if oldValue.podState?.setupProgress != newValue.podState?.setupProgress,
               newValue.podState?.setupProgress == .completed
            {
                pumpDelegate.notify { delegate in
                    let date = Date()
                    let event = NewPumpEvent(
                        date: date,
                        dose: nil,
                        raw: "Pod Change \(date)".data(using: .utf8)!,
                        title: "Pod Change",
                    ...
                    delegate?.pumpManager(
                        ...
                        lastReconciliation: self.lastSync,
    ...
    private func logDeviceCommunication(_ message: String, type: DeviceLogEntryType = .send) {
        ...
        pumpDelegate.delegate?.deviceManager(
            self,
            logEventForDeviceIdentifier: podAddress,
            type: type,
            message: message,
            completion: nil
        )
    ...
    override public var rileyLinkConnectionManagerState: RileyLinkConnectionState? {
        get {
            state.rileyLinkConnectionManagerState
        }
        set {
            setState { state in
    ...
    override public func deviceTimerDidTick(_: RileyLinkDevice) {
        pumpDelegate.notify { delegate in
    ...
    public var rileyLinkBatteryAlertLevel: Int? {
        get {
            state.rileyLinkBatteryAlertLevel
        }
        set {
            setState { state in
           ...
           state.lastRileyLinkBatteryAlertDate.addingTimeInterval(repeatInterval) < Date()
        {
            ...
            pumpDelegate.notify { delegate in
                ...
                let content = Alert.Content(
                    title: LocalizedString("Low RileyLink Battery", comment: "Title for RileyLink low battery alert"),
                    body: alertBody,
                    acknowledgeActionButtonLabel: LocalizedString(
                        "OK",
                        comment: "Acknowledge button label for RileyLink low battery alert"
                    )
                )
                delegate?
                    .issueAlert(Alert(
...
public extension OmnipodPumpManager {
    ...
    private func updateBLEHeartbeatPreference() {
        ...
        rileyLinkDeviceProvider.timerTickEnabled = state.isPumpDataStale || pumpDelegate.call({ (delegate) -> Bool in
            delegate?.pumpManagerMustProvideBLEHeartbeat(self) == true
        })
    }

    var expiresAt: Date? {
        state.podState?.expiresAt
    }

    func buildPumpStatusHighlight(for state: OmnipodPumpManagerState, andDate date: Date = Date()) -> PumpStatusHighlight? {
        ...
        case .deactivating:
            return PumpStatusHighlight(
                localizedMessage: LocalizedString(
                    "Finish Deactivation",
                    comment: "Status highlight that when pod is deactivating."
                ),
                imageName: "exclamationmark.circle.fill",
                state: .warning
            )
        ...
        case .active:
            if let reservoirPercent = state.reservoirLevel?.percentage, reservoirPercent == 0 {
                ...
                return PumpStatusHighlight(
                    localizedMessage: LocalizedString(
                        "Manual Basal",
                        comment: "Status highlight when manual temp basal is running."
                    ),
                    imageName: "exclamationmark.circle.fill",
                    state: .warning
                )
    ...
    func isRunningManualTempBasal(for state: OmnipodPumpManagerState) -> Bool {
    ...
    func buildPumpLifecycleProgress(for _: OmnipodPumpManagerState) -> PumpLifecycleProgress? {
        switch podCommState {
        case .active:
            if shouldWarnPodEOL,
    ...
    private func basalDeliveryState(for state: OmnipodPumpManagerState, at date: Date = Date()) -> PumpManagerStatus
        .BasalDeliveryState
    {
    ...
    private func beepMessageBlock(beepType: BeepType) -> MessageBlock? {
        guard beepPreference.shouldBeepForManualCommand, !silencePod else {
            return nil
        }
    ...
    var podCommState: PodCommState {
        podCommState(for: state)
    }

    var podActivatedAt: Date? {
        state.podState?.activatedAt
    }
    ...
    private var shouldWarnPodEOL: Bool {
        ...
        guard let podTimeRemaining = podTimeRemaining,
              podTimeRemaining > 0, podTimeRemaining <= eolDisplayActiveTime
        else {
            return false
        }
    ...
    var durationBetweenLastPodCommAndActivation: TimeInterval? {
        guard let lastPodCommDate = state.podState?.lastInsulinMeasurements?.validTime,
              let activationTime = podActivatedAt
        else {
            return nil
        }
    ...
    // MARK: Testing
        ...
        private func jumpStartPod(
            address: UInt32,
            lot: UInt32,
            tid: UInt32,
            fault: DetailedStatus? = nil,
            startDate: Date? = nil,
            mockFault: Bool
        ) {
            ...
            var podState = PodState(
                address: address,
                pmVersion: "jumpstarted",
                piVersion: "jumpstarted",
                lot: lot,
                tid: tid,
                insulinType: .novolog
            )
            ...
            let fault = mockFault ?
                try? DetailedStatus(encodedData: Data(hexadecimalString: "020f0000000900345c000103ff0001000005ae056029")!) : nil
    ...
    func pairAndPrime(completion: @escaping (PumpManagerResult<TimeInterval>) -> Void) {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + .seconds(2)) {
                self.jumpStartPod(address: 0x1F0B_3557, lot: 40505, tid: 6439, mockFault: mockFaultDuringPairing)
        ...
        #else
            let deviceSelector = rileyLinkDeviceProvider.firstConnectedDevice
            ...
            if needsPairing {
                ...
                log.default("Pairing pod before priming")
                ...
                if state.pairingAttemptAddress == nil {
                    lockedState.mutate { state in
                        state.pairingAttemptAddress = 0x1F00_0000 | (arc4random() & 0x000F_FFFF)
                    }
                }

                podComms.assignAddressAndSetupPod(
                    address: state.pairingAttemptAddress!,
                ...
                log.default("Pod already paired. Continuing.")

                // Resuming the pod setup, try to ensure pod comms will work right away
                resumingPodSetup()

                podComms.runSession(withName: "Prime pod", using: deviceSelector) { result in
                    // Calls completion
                    primeSession(result)
                }
            }
    ...
    func insertCannula(completion: @escaping (Result<TimeInterval, OmnipodPumpManagerError>) -> Void) {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + mockDelay) {
                let result = self.setStateWithResult({ (state) -> Result<TimeInterval, OmnipodPumpManagerError> in
                    if mockFaultDuringInsertCannula {
                        let fault =
                            try! DetailedStatus(
                                encodedData: Data(hexadecimalString: "020d0000000e00c36a020703ff020900002899080082")!
                            )
            ...
            let preError = setStateWithResult({ (state) -> OmnipodPumpManagerError? in
                guard let podState = state.podState, podState.readyForCannulaInsertion
                else {
            ...
            podComms.runSession(withName: "Insert cannula", using: rileyLinkSelector) { result in
                switch result {
                case let .success(session):
                    ...
                    do {
                        ...
                        let alerts: [PodAlert] = [
                            .expirationReminder(
                                offset: self.podTime,
                                absAlertTime: self.state.defaultExpirationReminderOffset > 0 ? expirationReminderTime : 0,
                                silent: self.state.silencePod
                            ),
    ...
    func checkCannulaInsertionFinished(completion: @escaping (OmnipodPumpManagerError?) -> Void) {
        ...
        #else
            let deviceSelector = rileyLinkDeviceProvider.firstConnectedDevice
            podComms.runSession(withName: "Check cannula insertion finished", using: deviceSelector) { result in
                switch result {
                case let .success(session):
                    do {
    ...
    func resumingPodSetup() {
        ...
        podComms.runSession(withName: "Resuming pod setup", using: rileyLinkSelector) { result in
            ...
            case let .failure(error):
                self.log.debug(
                    "### Pod setup resume session failure, sleeping %d seconds: %@",
                    sleepTime,
    ...
    private func tryToValidateComms(session: PodCommsSession) -> LocalizedError? {
        ...
        if state.podState?.lastDeliveryStatusReceived != nil, state.podState?.unacknowledgedCommand == nil {
            return nil
        }
        ...
        do {
            ...
            log.debug("### tryToValidateComms getStatus resolved all pending comms issues")
            return nil
        } catch {
            log.debug("### tryToValidateComms getStatus failed, returning: %@", error.localizedDescription)
    ...
    func acknowledgePodAlerts(_ alertsToAcknowledge: AlertSet, completion: @escaping (_ alerts: AlertSet?) -> Void) {
        ...
        let rileyLinkSelector = rileyLinkDeviceProvider.firstConnectedDevice
        podComms.runSession(withName: "Acknowledge Alerts", using: rileyLinkSelector) { result in
    ...
    func setTime(completion: @escaping (OmnipodPumpManagerError?) -> Void) {
        ...
        podComms.runSession(withName: "Set time zone", using: rileyLinkSelector) { result in
            ...
            case let .success(session):
                do {
                    ...
                    _ = try session.setTime(
                        timeZone: timeZone,
                        basalSchedule: self.state.basalSchedule,
                        date: Date(),
                        acknowledgementBeep: beep
                    )
                    self.setState { state in
                        state.timeZone = timeZone
                    }
        ...
        podComms.runSession(withName: "Save Basal Profile", using: rileyLinkDeviceProvider.firstConnectedDevice) { result in
            do {
                ...
                case let .success(session):
                    ...
                    switch result {
                    case let .certainFailure(error):
                        throw error
                    case let .unacknowledged(error):
                        throw error
                    case .success:
                        break
                    }
                    let beep = self.silencePod ? false : self.beepPreference.shouldBeepForManualCommand
                    _ = try session.setBasalSchedule(
                        schedule: schedule,
    ...
    func playTestBeeps() async throws {
        ...
        guard state.podState?.unfinalizedBolus?.scheduledCertainty == .uncertain || state.podState?.unfinalizedBolus?
            .isFinished() != false
        else {
            log.info("Skipping Play Test Beeps due to bolus still in progress.")
    ...
    func readPulseLog() async throws -> String {
        ...
        guard state.podState?.isFaulted == true || state.podState?.unfinalizedBolus?.scheduledCertainty == .uncertain || state
            .podState?.unfinalizedBolus?.isFinished() != false
        else {
            log.info("Skipping Read Pulse Log due to bolus still in progress.")
            throw PodCommsError.unfinalizedBolus
        }
            ...
            self.podComms.runSession(withName: "Read Pulse Log", using: rileyLinkSelector) { result in
                ...
                case let .success(session):
                    do {
                        ...
                        let podInfoResponse = try session.readPodInfo(
                            podInfoResponseSubType: .pulseLogRecent,
                            beepBlock: beepBlock
                        )
                        ...
                        let str = pulseLogString(
                            pulseLogEntries: podInfoPulseLogRecent.pulseLog,
                            lastPulseNumber: lastPulseNumber
                        )
    ...
    func readPulseLogPlus() async throws -> String {
        ...
        guard state.podState?.isFaulted == true || state.podState?.unfinalizedBolus?.scheduledCertainty == .uncertain || state
            ...
            log.info("Skipping Read Pulse Log Plus due to bolus still in progress.")
            throw PodCommsError.unfinalizedBolus
        }
            ...
            podComms.runSession(withName: "Read Activation Time", using: rileyLinkSelector) { result in
                do {
                    ...
                    case let .success(session):
                        ...
                        let podInfoResponse = try session.readPodInfo(
                            podInfoResponseSubType: .activationTime,
                            beepBlock: beepBlock
                        )
            ...
            podComms.runSession(withName: "Read Triggered Alerts", using: rileyLinkSelector) { result in
                do {
                    ...
                    case let .success(session):
                        let beepBlock = self.beepMessageBlock(beepType: .beepBeep)
                        let podInfoResponse = try session.readPodInfo(
                            podInfoResponseSubType: .triggeredAlerts,
                            beepBlock: beepBlock
                        )
    ...
    func setConfirmationBeeps(newPreference: BeepPreference, completion: @escaping (OmnipodPumpManagerError?) -> Void) {
        ...
        if !hasActivePod || silencePod {
            log.default("%{public}@ for internal state only", name)
            setState { state in
        ...
        let rileyLinkSelector = rileyLinkDeviceProvider.firstConnectedDevice
        podComms.runSession(withName: name, using: rileyLinkSelector) { result in
            switch result {
            case let .success(session):
                if let error = self.tryToValidateComms(session: session) {
                    completion(.communication(error))
    ...
    func setSilencePod(silencePod: Bool, completion: @escaping (OmnipodPumpManagerError?) -> Void) {
        let name = String(format: "%@ Pod", silencePod ? "Silence" : "Unsilence")
        // allow Silence Pod changes without an active Pod
        guard hasActivePod else {
            log.default("%{public}@", name)
            setState { state in
        ...
        podComms.runSession(withName: name, using: rileyLinkSelector) { result in
            ...
            guard let configuredAlerts = self.state.podState?.configuredAlerts,
                  ...
                  let reservoirLevel = self.state.podState?.lastInsulinMeasurements?.reservoirLevel?.rawValue
            else {
            ...
            let podAlerts = regeneratePodAlerts(
                silent: silencePod,
                configuredAlerts: configuredAlerts,
                activeAlertSlots: activeAlertSlots,
                currentPodTime: self.podTime,
                currentReservoirLevel: reservoirLevel
            )
            do {
                ...
                self.log.error(
                    "Configure alerts %{public}@ failed: %{public}@",
                    String(describing: podAlerts),
...
extension OmnipodPumpManager: PumpManager {
    public static var onboardingMaximumBasalScheduleEntryCount: Int {
        Pod.maximumBasalScheduleEntryCount
    }

    public static var onboardingSupportedBasalRates: [Double] {
        // 0.05 units for rates between 0.05-30U/hr
        // 0 is not a supported scheduled basal rate
        (1 ... 600).map { Double($0) / Double(Pod.pulsesPerUnit) }
    }
    ...
    public var minimumBasalScheduleEntryDuration: TimeInterval {
        Pod.minimumBasalScheduleEntryDuration
    }
    ...
    public var pumpReservoirCapacity: Double {
        Pod.reservoirCapacity
    }

    public var isOnboarded: Bool { state.isOnboarded }

    public var lastSync: Date? {
        state.podState?.lastInsulinMeasurements?.validTime
    }
    ...
    public func suspendDelivery(
        withSuspendReminders suspendReminder: TimeInterval? = nil,
    ...
    public func resumeDelivery(completion: @escaping (Error?) -> Void) {
        ...
        podComms.runSession(withName: "Resume", using: rileyLinkSelector) { result in
            ...
            do {
                ...
                let beep = self.silencePod ? false : self.beepPreference.shouldBeepForManualCommand
                _ = try session.resumeBasal(
                    schedule: self.state.basalSchedule,
                    scheduleOffset: scheduleOffset,
                    acknowledgementBeep: beep
                )
    ...
    fileprivate func clearSuspendReminder() {
        pumpDelegate.notify { delegate in
            delegate?.retractAlert(identifier: Alert.Identifier(
                managerIdentifier: self.pluginIdentifier,
                alertIdentifier: PumpManagerAlert.suspendEnded(triggeringSlot: nil).alertIdentifier
            ))
            delegate?.retractAlert(identifier: Alert.Identifier(
                managerIdentifier: self.pluginIdentifier,
                alertIdentifier: PumpManagerAlert.suspendEnded(triggeringSlot: nil).repeatingAlertIdentifier
            ))
        }
    }
    ...
    public func setMustProvideBLEHeartbeat(_ mustProvideBLEHeartbeat: Bool) {
        rileyLinkDeviceProvider.timerTickEnabled = state.isPumpDataStale || mustProvideBLEHeartbeat
    }
    ...
    public func enactBolus(
        ...
        if silencePod {
            ...
            acknowledgementBeep = beepPreference.shouldBeepForCommand(automatic: activationType.isAutomatic)
        ...
        podComms.runSession(withName: "Bolus", using: rileyLinkSelector) { result in
            ...
            guard let podState = self.state.podState, !podState.isSuspended,
                  podState.lastDeliveryStatusReceived?.suspended != true
            else {
    ...
    public func cancelBolus(completion: @escaping (PumpManagerResult<DoseEntry?>) -> Void) {
        ...
        let rileyLinkSelector = rileyLinkDeviceProvider.firstConnectedDevice
        podComms.runSession(withName: "Cancel Bolus", using: rileyLinkSelector) { result in
            ...
            do {
                ...
                if let bolus = self.state.podState?.unfinalizedBolus, !bolus.isFinished(),
                   bolus.scheduledCertainty == .uncertain
                {
                    let status = try session.getStatus()
                ...
                let beepType: BeepType = self.beepPreference.shouldBeepForManualCommand && !self
                    .silencePod ? .beeeeeep : .noBeepCancel
                ...
                case let .unacknowledged(error):
                    throw error
                case let .success(_, canceledBolus):
                    session.dosesForStorage { (doses) -> Bool in
                        self.store(doses: doses, in: session)
                    }
    ...
    public func runTemporaryBasalProgram(
        unitsPerHour: Double,
        for duration: TimeInterval,
        automatic: Bool,
        completion: @escaping (PumpManagerError?) -> Void
        ...
        podComms.runSession(withName: "Enact Temp Basal", using: rileyLinkSelector) { result in
            ...
            // Use a lastDeliveryStatusReceived?.suspended != true test here to not return a pod suspended failure if
            // there is not a valid last delivery status (which shouldn't even happen now with tryToValidateComms()).
            guard let podState = self.state.podState,
                  !podState.isSuspended && podState.lastDeliveryStatusReceived?.suspended != true
            else {
                ...
                podState.lastDeliveryStatusReceived == nil || podState.lastDeliveryStatusReceived!.tempBasalRunning
            {
                ...
                case let .unacknowledged(error):
                    completion(.communication(error))
                    return
                case let .success(cancelTempStatus, _):
            ...
            if resumingScheduledBasal {
                ...
                let result = session.setTempBasal(
                    rate: rate,
                    duration: duration,
    ...
    public func createBolusProgressReporter(reportingOn dispatchQueue: DispatchQueue) -> DoseProgressReporter? {
        if case let .inProgress(dose) = bolusState(for: state) {
            return PodDoseProgressEstimator(dose: dose, pumpManager: self, reportingQueue: dispatchQueue)
        }
        return nil
    }

    public func setMaximumTempBasalRate(_: Double) {}
    ...
    public func updateExpirationReminder(
        _ intervalBeforeExpiration: TimeInterval?,
        completion: @escaping (OmnipodPumpManagerError?) -> Void
    ) {
        guard hasActivePod, let podState = state.podState, let expiresAt = podState.expiresAt else {
            completion(OmnipodPumpManagerError.noPodPaired)
            return
        }

        let rileyLinkSelector = rileyLinkDeviceProvider.firstConnectedDevice
        podComms.runSession(withName: "Update Expiration Reminder", using: rileyLinkSelector) { result in
            ...
            if let intervalBeforeExpiration = intervalBeforeExpiration, intervalBeforeExpiration > 0 {
                let timeUntilReminder = expiresAt.addingTimeInterval(-intervalBeforeExpiration)
                    .timeIntervalSince(self.dateGenerator())
                ...
                if timeUntilReminder > .minutes(2) {
                    ...
                    self.log.debug(
                        "Update Expiration timeUntilReminder=%@, podTime=%@, expirationReminderPodTime=%@",
                        timeUntilReminder.timeIntervalStr,
                        podTime.timeIntervalStr,
                        expirationReminderPodTime.timeIntervalStr
                    )
                }
            }
            ...
            let expirationReminder = PodAlert.expirationReminder(
                offset: podTime,
                absAlertTime: expirationReminderPodTime,
    ...
    public func updateLowReservoirReminder(_ value: Int, completion: @escaping (OmnipodPumpManagerError?) -> Void) {
        ...
        guard let currentReservoirLevel = reservoirLevel?.rawValue, currentReservoirLevel > supportedValue else {
        ...
        let rileyLinkSelector = rileyLinkDeviceProvider.firstConnectedDevice
        podComms.runSession(withName: "Program Low Reservoir Reminder", using: rileyLinkSelector) { result in
    ...
    func issueAlert(alert: PumpManagerAlert) {
        let identifier = Alert.Identifier(managerIdentifier: pluginIdentifier, alertIdentifier: alert.alertIdentifier)
        let loopAlert = Alert(
            identifier: identifier,
        ...
        if let repeatInterval = alert.repeatInterval {
            ...
            let repeatingIdentifier = Alert.Identifier(
                managerIdentifier: pluginIdentifier,
                alertIdentifier: alert.repeatingAlertIdentifier
            )
            let loopAlert = Alert(
                identifier: repeatingIdentifier,
                foregroundContent: alert.foregroundContent,
                backgroundContent: alert.backgroundContent,
                trigger: .repeating(repeatInterval: repeatInterval)
            )
            pumpDelegate.notify { delegate in
                delegate?.issueAlert(loopAlert)
            }
    ...
    private func silenceAcknowledgedAlerts() {
        ...
        if let alert = state.alertsWithPendingAcknowledgment.first {
            if let slot = alert.triggeringSlot {
                let rileyLinkSelector = rileyLinkDeviceProvider.firstConnectedDevice
                podComms.runSession(withName: "Silence already acknowledged alert", using: rileyLinkSelector) { result in
                    switch result {
                    case let .success(session):
                        do {
                            _ = try session.acknowledgeAlerts(alerts: AlertSet(slots: [slot]))
                        } catch {
                            return
                        }
                        self.setState { state in
    ...
    private func notifyPodFault(fault: DetailedStatus) {
        pumpDelegate.notify { delegate in
            let content = Alert.Content(
                title: fault.faultEventCode.notificationTitle,
                body: fault.faultEventCode.notificationBody,
                acknowledgeActionButtonLabel: LocalizedString(
                    "OK",
                    comment: "Alert acknowledgment OK button"
                )
            )
            delegate?
                .issueAlert(Alert(
                    identifier: Alert
                        .Identifier(
                            managerIdentifier: OmnipodPumpManager.podAlarmNotificationIdentifier,
    ...
    func store(doses: [UnfinalizedDose], completion: @escaping (_ error: Error?) -> Void) {
        ...
        pumpDelegate.notify { delegate in
            ...
            delegate.pumpManager(
                self,
                hasNewPumpEvents: doses.map { NewPumpEvent($0) },
...
extension OmnipodPumpManager: MessageLogger {
    func didSend(_ message: Data) {
        logDeviceCommunication(message.hexadecimalString, type: .send)
    }

    func didReceive(_ message: Data) {
        logDeviceCommunication(message.hexadecimalString, type: .receive)
    }
...
extension OmnipodPumpManager: PodCommsDelegate {
    func podComms(_: PodComms, didChange podState: PodState?) {
        if let podState = podState {
            ...
            if oldAlerts != newAlerts {
                alertsChanged(oldAlerts: oldAlerts, newAlerts: newAlerts)
            }
        } else {
            // Resetting podState
            setState { state in
    ...
    func acknowledgeAlert(alertIdentifier: Alert.AlertIdentifier, completion: @escaping (Error?) -> Void) {
        ...
        for alert in state.activeAlerts {
            if alert.alertIdentifier == alertIdentifier || alert.repeatingAlertIdentifier == alertIdentifier {
                ...
                if let slot = alert.triggeringSlot {
                    if state.podState?.isSuspended == true || state.podState?.lastDeliveryStatusReceived?.suspended == true,
                    ...
                    let rileyLinkSelector = rileyLinkDeviceProvider.firstConnectedDevice
                    podComms.runSession(withName: "Acknowledge Alert", using: rileyLinkSelector) { result in
                        switch result {
                        case let .success(session):
                            do {
                                let beepBlock = self.beepMessageBlock(beepType: .beep)
                                _ = try session.acknowledgeAlerts(alerts: AlertSet(slots: [slot]), beepBlock: beepBlock)
                            } catch {
                                self.setState { state in
...
public extension FaultEventCode {
