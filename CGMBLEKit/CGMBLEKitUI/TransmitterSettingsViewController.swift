...
class TransmitterSettingsViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .remoteDataSync:
            ...
            switchCell.textLabel?.text = LocalizedString(
                "Upload Readings",
        ...
        case .latestReading:
            ...
            case .glucose:
                cell.setGlucose(
                    glucose?.glucose,
                    formatter: displayGlucosePreference.formatter,
                    isDisplayOnly: glucose?.isDisplayOnly ?? false
                )
            case .date:
        ...
        case .ages:
            ...
            case .sensorAge:
                ...
                if let stateDescription = glucose?.stateDescription, !stateDescription.isEmpty,
                   !stateDescription.contains("stopped")
                {
                    ...
                    if let sessionExp = cgmManager.latestReading?.sessionExpDate {
                        ...
                        if sessionCountDown < 0 {
                            ...
                            cell.detailTextLabel?
                                .text = (sessionLengthFormatter.string(from: sessionCountDown * -1) ?? "") + " ago"
                        } else {
            ...
            case .sensorExpirationDate:
                ...
                {
                    if let sessionExp = cgmManager.latestReading?.sessionExpDate {
                        if sensorExpirationRelativeFormatter.string(from: sessionExp) == sensorExpAbsFormatter
                            .string(from: sessionExp)
                        {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .latestConnection:
            let vc = CommandResponseViewController(command: { (_) -> String in
                String(reflecting: self.cgmManager)
            })
            vc.title = title
            show(vc, sender: nil)
        case .ages:
        ...
        case .share:
            switch ShareRow(rawValue: indexPath.row)! {
            case .settings:
                let vc = ShareClientSettingsViewController(
                    cgmManager: cgmManager.shareManager,
...
extension TransmitterSettingsViewController: TransmitterManagerObserver {
    func transmitterManagerDidUpdateLatestReading(_: TransmitterManager) {
    ...
    convenience init(cgmDeletionHandler handler: @escaping () -> Void) {
        self.init(
            ...
            message: LocalizedString(
                "Are you sure you want to delete this CGM?",
                comment: "Confirmation message for deleting a CGM"
            ),
