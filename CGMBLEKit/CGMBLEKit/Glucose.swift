...
public struct Glucose {
    ...
    init(
        ...
        lastCalibration = calibrationMessage != nil ? Calibration(
            calibrationMessage: calibrationMessage!,
            activationDate: activationDate
        ) : nil
    }
    ...
    public var isDisplayOnly: Bool {
        glucoseMessage.glucoseIsDisplayOnly
    }
    ...
    public var glucose: HKQuantity? {
        ...
        return HKQuantity(
            unit: unit,
            doubleValue: Double(min(max(glucoseMessage.glucose, GlucoseLimits.minimum), GlucoseLimits.maximum))
        )
    }

    public var state: CalibrationState {
        CalibrationState(rawValue: glucoseMessage.state)
    }

    public var trend: Int {
        Int(glucoseMessage.trend)
    }
    ...
    public var syncIdentifier: String {
        "\(transmitterID) \(glucoseMessage.timestamp)"
    }
}

extension Glucose: Equatable {
    public static func == (lhs: Glucose, rhs: Glucose) -> Bool {
        lhs.glucoseMessage == rhs.glucoseMessage && lhs.syncIdentifier == rhs.syncIdentifier
    }
}
