...
extension TransmitterError: CustomStringConvertible {
    public var description: String {
        switch self {
        case let .authenticationError(description):
            return description
        case let .controlError(description):
            return description
        case let .observationError(description):
            return description
        }
    }
}
...
public final class Transmitter: BluetoothManagerDelegate {
    ...
    public var peripheralIdentifier: UUID? {
        get {
            bluetoothManager.peripheralIdentifier
        }
        set {
    ...
    public var stayConnected: Bool {
        get {
            bluetoothManager.stayConnected
        }
        set {
    ...
    func bluetoothManager(_: BluetoothManager, peripheralManager: PeripheralManager, isReadyWithError error: Error?) {
        if let error = error {
            delegateQueue.async {
    ...
    func bluetoothManager(_: BluetoothManager, shouldConnectPeripheral peripheral: CBPeripheral) -> Bool {
        ...
        if let name = peripheral.name, name.suffix(2) == id.id.suffix(2) {
            ...
            log.info("Not connecting to peripheral: %{public}@", peripheral.name ?? String(describing: peripheral))
            return false
        }
    }

    func bluetoothManager(_: BluetoothManager, peripheralManager: PeripheralManager, didReceiveControlResponse response: Data) {
        ...
        case .glucoseG6Rx?,
            ...
            {
                delegateQueue.async {
                    self.delegate?.transmitter(
                        self,
                        didRead: Glucose(
                            transmitterID: self.id.id,
                            glucoseMessage: glucoseMessage,
                            timeMessage: timeMessage,
                            calibrationMessage: self.lastCalibrationMessage,
                            activationDate: activationDate
                        )
                    )
                }
                ...
                delegateQueue.async {
                    self.delegate?.transmitter(
                        self,
                        didError: TransmitterError.observationError("Unable to handle glucose control response")
                    )
                }
            }
            ...
            peripheralManager.perform { peripheral in
                ...
                do {
                    ...
                    self.log.error(
                        "Error trying to enable notifications on backfill characteristic: %{public}@",
        ...
        case .glucoseBackfillRx?:
            ...
            guard let backfillBuffer = backfillBuffer else {
                log.error(
                    "Received GlucoseBackfillRxMessage %{public}@ but backfillBuffer is nil",
                    String(describing: backfillMessage)
                )
                delegateQueue.async {
                    self.delegate?.transmitter(
                        self,
                        didError: TransmitterError.observationError("Received GlucoseBackfillRxMessage but backfillBuffer is nil")
                    )
                }
                break
            }
            ...
            guard let timeMessage = lastTimeMessage, let activationDate = activationDate else {
                log.error(
                    "Received GlucoseBackfillRxMessage %{public}@ but activationDate is unknown",
                    String(describing: backfillMessage)
                )
                delegateQueue.async {
                    self.delegate?.transmitter(
                        self,
                        didError: TransmitterError
                            .observationError("Received GlucoseBackfillRxMessage but activationDate is unknown")
                    )
                }
                break
            }
            ...
            guard backfillMessage.bufferLength == backfillBuffer.count else {
                log.error(
                    "GlucoseBackfillRxMessage expected buffer length %d, but was %d",
                    backfillMessage.bufferLength,
                    backfillBuffer.count
                )
                delegateQueue.async {
                    self.delegate?.transmitter(
                        self,
                        didError: TransmitterError
                            .observationError(
                                "GlucoseBackfillRxMessage expected buffer length \(backfillMessage.bufferLength), but was \(backfillBuffer.count): \(response.hexadecimalString) "
                            )
                    )
                }
                break
            ...
            guard backfillMessage.bufferCRC == backfillBuffer.crc16 else {
                log.error(
                    "GlucoseBackfillRxMessage expected CRC %04x, but was %04x",
                    backfillMessage.bufferCRC,
                    backfillBuffer.crc16
                )
                delegateQueue.async {
                    self.delegate?.transmitter(
                        self,
                        didError: TransmitterError
                            .observationError(
                                "GlucoseBackfillRxMessage expected CRC \(backfillMessage.bufferCRC), but was \(backfillBuffer.crc16)"
                            )
                    )
                }
                break
            ...
            guard glucose.first!.glucoseMessage.timestamp == backfillMessage.startTime,
                ...
                log.error(
                    "GlucoseBackfillRxMessage time interval not reflected in glucose: %{public}@, buffer: %{public}@",
                    response.hexadecimalString,
                    String(reflecting: backfillBuffer)
                )
                ...
                delegateQueue.async {
                    self.delegate?.transmitter(
                        ...
                        didError: TransmitterError
                            .observationError(
                                "GlucoseBackfillRxMessage time interval not reflected in glucose: \(backfillMessage.startTime) - \(backfillMessage.endTime), buffer: \(glucose.first!.glucoseMessage.timestamp) - \(glucose.last!.glucoseMessage.timestamp)"
                            )
                    )
                }
                break
    ...
    func bluetoothManager(_: BluetoothManager, didReceiveBackfillResponse response: Data) {
        ...
        if response[0] == 1 {
            ...
            backfillBuffer = GlucoseBackfillFrameBuffer(identifier: response[1])
        }

        log.info("appending to backfillBuffer: %@", response.hexadecimalString)

        backfillBuffer?.append(response)
    }

    func bluetoothManager(
        _: BluetoothManager,
        peripheralManager: PeripheralManager,
        didReceiveAuthenticationResponse response: Data
    ) {
        if let message = AuthChallengeRxMessage(data: response), message.isBonded, message.isAuthenticated {
            ...
            peripheralManager.perform { peripheral in
                ...
                do {
                    ...
                    self.log.error(
                        "Error trying to enable notifications on control characteristic: %{public}@",
                        String(describing: error)
                    )
                    self.delegateQueue.async {
                        self.delegate?.transmitter(self, didError: error)
        ...
        do {
            ...
            case let .startSensor(date):
                ...
                return try writeMessage(
                    SessionStartTxMessage(startTime: startTime, secondsSince1970: secondsSince1970),
                    for: .control
                )
