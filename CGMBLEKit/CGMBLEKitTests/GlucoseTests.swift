...
class GlucoseTests: XCTestCase {
    ...
    func testMessageData() {
        ...
        XCTAssertEqual(
            calendar.date(from: DateComponents(year: 2016, month: 12, day: 6, hour: 7, minute: 51, second: 38))!,
            glucose.readDate
        )
        XCTAssertEqual(
            calendar.date(from: DateComponents(year: 2016, month: 12, day: 26, hour: 11, minute: 16, second: 12))!,
            glucose.sessionStartDate
        )
    ...
    func testNegativeTrend() {
        ...
        XCTAssertEqual(TransmitterStatus.ok, glucose.status)
        XCTAssertEqual(
            calendar.date(from: DateComponents(year: 2016, month: 12, day: 6, hour: 8, minute: 26, second: 38))!,
            glucose.readDate
        )
        XCTAssertFalse(glucose.isDisplayOnly)
    ...
    func testDisplayOnly() {
        ...
        XCTAssertEqual(TransmitterStatus.ok, glucose.status)
        XCTAssertEqual(
            calendar.date(from: DateComponents(year: 2016, month: 12, day: 6, hour: 8, minute: 31, second: 45))!,
            glucose.readDate
        )
    ...
    func testOldTransmitter() {
        ...
        XCTAssertEqual(TransmitterStatus.ok, glucose.status)
        XCTAssertEqual(
            calendar.date(from: DateComponents(year: 2016, month: 12, day: 31, hour: 11, minute: 57, second: 09))!,
            glucose.readDate
        ) // 90 days, status is still OK
        XCTAssertFalse(glucose.isDisplayOnly)
