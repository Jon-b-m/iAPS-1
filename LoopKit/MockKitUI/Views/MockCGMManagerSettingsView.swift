...
struct MockCGMManagerSettingsView: View {
    ...
    private var statusCardSubSection: some View {
        Section {
            VStack(spacing: 8) {
                sensorProgressView
                    .openMockCGMSettingsOnLongPress(
                        enabled: true,
                        cgmManager: viewModel.cgmManager,
                        displayGlucosePreference: displayGlucosePreference
                    )
                Divider()
                lastReadingInfo
    ...
    @ViewBuilder private var lastGlucoseReading: some View {
    ...
    @ViewBuilder private var lastReadingTime: some View {
        HStack(alignment: .center, spacing: 16) {
    ...
    private var settingsSubSection: some View {
        Section {
            NavigationLink(destination: MockCGMManagerControlsView(
                cgmManager: viewModel.cgmManager,
                displayGlucosePreference: displayGlucosePreference
            )) {
    ...
    private func alert(for presentedAlert: PresentedAlert) -> SwiftUI.Alert {
        ...
        case let .suspendInsulinDeliveryError(error):
            return Alert(
                title: Text("Failed to Suspend Insulin Delivery"),
                message: Text(error.localizedDescription)
            )
        case let .resumeInsulinDeliveryError(error):
            return Alert(
