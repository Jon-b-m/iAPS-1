...
struct MockPumpManagerSettingsView: View {
    ...
    @ViewBuilder private var statusSection: some View {
        Section {
            VStack(spacing: 8) {
                pumpProgressView
                    .openMockPumpSettingsOnLongPress(
                        enabled: true,
    ...
    private var suspendResumeInsulinSubSection: some View {
        Section(header: SectionHeader(label: LocalizedString("Activity", comment: "Section header for the activity section"))) {
            ...
            if viewModel.isDeliverySuspended {
                LabeledValueView(
                    label: LocalizedString("Suspended At", comment: "Label for suspended at field"),
    ...
    private var settingsSubSection: some View {
        Section {
            NavigationLink(destination: MockPumpManagerControlsView(
                pumpManager: viewModel.pumpManager,
                supportedInsulinTypes: supportedInsulinTypes
            )) {
