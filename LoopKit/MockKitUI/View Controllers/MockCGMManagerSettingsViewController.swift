...
final class MockCGMManagerSettingsViewController: UITableViewController {
    ...
    init(cgmManager: MockCGMManager, displayGlucosePreference: DisplayGlucosePreference) {
        ...
        quantityFormatter = QuantityFormatter(for: displayGlucosePreference.unit)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .model:
            ...
            case .constant:
                ...
                if case let .constant(glucose) = cgmManager.dataSource.model {
            ...
            case .sineCurve:
                ...
                if case .sineCurve(parameters: (
                    baseGlucose: let baseGlucose,
                    amplitude: let amplitude,
                    period: _,
                    referenceDate: _
                )) = cgmManager
                    .dataSource.model
                {
                    if let baseGlucoseText = quantityFormatter.numberFormatter
                        .string(from: baseGlucose.doubleValue(for: glucoseUnit)),
        ...
        case .glucoseThresholds:
            ...
            case .enableAlerting:
                let cell = tableView.dequeueReusableCell(
                    withIdentifier: BoundSwitchTableViewCell.className,
                    for: indexPath
                ) as! BoundSwitchTableViewCell
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .model:
            ...
            case .sineCurve:
                ...
                if case let .sineCurve(parameters: parameters) = cgmManager.dataSource.model {
        ...
        case .glucoseThresholds:
            ...
            case .cgmLowerLimit:
                ...
                vc
                    .contextHelp =
                    "The glucose value that marks the lower limit of the CGM. Any value at or below this value is presented at `LOW`. This value must be lower than the urgent low threshold. If not, it will be set to 1 below the urgent low glucose threshold."
            case .urgentLowGlucoseThreshold:
                vc.title = "Urgent Low Glucose Threshold"
                vc
                    .contextHelp =
                    "The glucose value that marks the urgent low glucose threshold. Any value at or below this value is considered urgent low. This value must be above the cgm lower limit and lower than the low threshold. If not, it will be set to a value above the lower limit and below the low glucose threshold."
            case .lowGlucoseThreshold:
                vc.title = "Low Glucose Threshold"
                vc
                    .contextHelp =
                    "The glucose value that marks the low glucose threshold. Any value at or below this value is considered low. This value must be above the urgent low threshold and lower than the high threshold. If not, it will be set to a value above the urgent lower limit and below the high glucose threshold."
            case .highGlucoseThreshold:
                vc.title = "High Glucose Threshold"
                vc
                    .contextHelp =
                    "The glucose value that marks the high glucose threshold. Any value at or above this value is considered high. This value must be above the low threshold and lower than the cgm upper limit. If not, it will be set to a value above the low glucose threshold and below the upper limit."
            case .cgmUpperLimit:
                vc.title = "CGM Upper Limit"
                vc
                    .contextHelp =
                    "The glucose value that marks the upper limit of the CGM. Any value at or above this value is presented at `HIGH`. This value must be above the high threshold. If not, it will be set to 1 above the high glucose threshold."
            }
            show(vc, sender: sender)
        case .effects:
            switch EffectsRow(rawValue: indexPath.row)! {
            case .noise:
                ...
                vc
                    .contextHelp =
                    "The magnitude of glucose noise applied to CGM values determines the maximum random amount of variation applied to each glucose value."
                vc.indexPath = indexPath
                vc.glucoseEntryDelegate = self
                show(vc, sender: sender)
            case .lowOutlier:
            ...
            case .error:
                ...
                vc
                    .contextHelp =
                    "The percentage determines the chance with which the CGM will error when a glucose value is requested."
                vc.indexPath = indexPath
        ...
        case .history:
            ...
            case .backfill:
                ...
                vc.onSave { inputMode in
                    guard case let .duration(duration) = inputMode else {
                        assertionFailure()
                        return
                    }
    ...
    private func indexPaths<Row: CaseIterable & RawRepresentable>(
        ...
        return zip(rows, repeatElement(section, count: rows.count)).map { row, section in
            IndexPath(row: row.rawValue, section: section.rawValue)
        }
    }
}
...
extension MockCGMManagerSettingsViewController: PercentageTextFieldTableViewControllerDelegate {
    func percentageTextFieldTableViewControllerDidChangePercentage(_ controller: PercentageTextFieldTableViewController) {
        ...
        case .lifecycleProgress:
            ...
            case .warningThreshold:
                cgmManager.mockSensorState.progressWarningThresholdPercentValue = controller.percentage
                    .map { $0.clamped(to: 0 ... 1) }
            case .criticalThreshold:
                cgmManager.mockSensorState.progressCriticalThresholdPercentValue = controller.percentage
                    .map { $0.clamped(to: 0 ... 1) }
            }
        default:
            assertionFailure()
