...
final class SineCurveParametersTableViewController: UITableViewController {
    ...
    init(glucoseUnit: HKUnit) {
        ...
        glucoseFormatter = QuantityFormatter(for: glucoseUnit)
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        func presentDateAndDurationViewController(
            for inputMode: DateAndDurationTableViewController.InputMode,
            contextHelp: String?
        ) {
        ...
        switch Row(rawValue: indexPath.row)! {
        case .baseGlucose:
            presentGlucoseEntryViewController(
                for: baseGlucose,
                contextHelp: "The base glucose represents the zero about which the sine curve oscillates."
            )
        case .amplitude:
            presentGlucoseEntryViewController(
                for: amplitude,
                contextHelp: "The amplitude represents the magnitude of the oscillation of the glucose curve."
            )
        case .period:
            presentDateAndDurationViewController(
                for: .duration(period ?? defaultPeriod),
                contextHelp: "The period describes the duration of one complete glucose cycle."
            )
        case .referenceDate:
            presentDateAndDurationViewController(
                for: .date(referenceDate ?? defaultReferenceDate, mode: .dateAndTime),
                contextHelp: "The reference date describes the origin of the sine curve with respect to time. Changing the reference date applies a phase shift to the curve."
            )
        }
    }
...
extension SineCurveParametersTableViewController: DateAndDurationTableViewControllerDelegate {
    func dateAndDurationTableViewControllerDidChangeDate(_ controller: DateAndDurationTableViewController) {
        ...
        switch Row(rawValue: indexPath.row)! {
        case .period:
            guard case let .duration(duration) = controller.inputMode else {
                assertionFailure()
                return
            }
            period = duration
