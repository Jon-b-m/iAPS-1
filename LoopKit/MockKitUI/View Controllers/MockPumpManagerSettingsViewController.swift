...
final class MockPumpManagerSettingsViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .actions:
            ...
            case .suspendResume:
                let cell = tableView.dequeueReusableCell(
                    withIdentifier: SuspendResumeTableViewCell.className,
                    for: indexPath
                ) as! SuspendResumeTableViewCell
        ...
        case .settings:
            ...
            case .deliverableIncrements:
                let cell = tableView.dequeueReusableCell(
                    withIdentifier: SegmentedControlTableViewCell.className,
                    for: indexPath
                ...
                cell.segmentedControl.selectedSegmentIndex = possibleDeliverableIncrements
                    .firstIndex(of: pumpManager.state.deliverableIncrements)!
                cell.onSelection { [pumpManager] index in
                    pumpManager.state.deliverableIncrements = possibleDeliverableIncrements[index]
                    tableView.reloadRows(
                        at: [
                            IndexPath(row: SettingsRow.supportedBasalRates.rawValue, section: Section.settings.rawValue),
                            IndexPath(
                                row: SettingsRow.supportedBolusVolumes.rawValue,
            ...
            case .reservoirRemaining:
                ...
                cell.detailTextLabel?.text = reservoirFormatter
                    .string(from: HKQuantity(unit: .internationalUnit(), doubleValue: pumpManager.state.reservoirUnitsRemaining))
                cell.accessoryType = .disclosureIndicator
                return cell
            case .batteryRemaining:
                let cell = tableView.dequeueReusableCell(withIdentifier: SettingsTableViewCell.className, for: indexPath)
            ...
            case .tempBasalErrorToggle:
                return switchTableViewCell(
                    for: indexPath,
                    titled: "Error on Temp Basal",
                    boundTo: \.tempBasalEnactmentShouldError
                )
            case .bolusErrorToggle:
            ...
            case .uncertainDeliveryErrorToggle:
                return switchTableViewCell(
                    for: indexPath,
                    titled: "Next Delivery Command Uncertain",
                    boundTo: \.deliveryCommandsShouldTriggerUncertainDelivery
                )
            case .lastReconciliationDate:
                let cell = tableView.dequeueReusableCell(
    ...
    private func switchTableViewCell(
        for indexPath: IndexPath,
        titled title: String,
        boundTo keyPath: WritableKeyPath<MockPumpManagerState, Bool>
    ) -> SwitchTableViewCell {
        let cell = tableView.dequeueReusableCell(
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .actions:
            ...
            case .occlusion:
                pumpManager
                    .injectPumpEvents(
                        pumpManager.state
                            .occlusionDetected ? [NewPumpEvent(alarmClearAt: Date())] :
                            [NewPumpEvent(alarmAt: Date(), alarmType: .occlusion)]
                    )
            ...
            case .pumpError:
                pumpManager
                    .injectPumpEvents(
                        pumpManager.state
                            .pumpErrorDetected ? [NewPumpEvent(alarmClearAt: Date())] :
                            [NewPumpEvent(alarmAt: Date(), alarmType: .other("Mock Pump Error"))]
                    )
            ...
            case .pumpComponentReplacement:
                pumpManager
                    .injectPumpEvents(
                        pumpManager.state
                            .replacePumpComponent ? [NewPumpEvent(type: .replaceComponent(componentType: .pump))] :
        ...
        case .settings:
            ...
            case .supportedBasalRates:
                if pumpManager.state.deliverableIncrements == .custom, pumpManager.state.supportedBasalRates.indices.contains(1) {
                    ...
                    let vc = SupportedRangeTableViewController(
                        minValue: basalRates.first!,
                        maxValue: basalRates.last!,
                        stepSize: basalRates[1] - basalRates.first!
                    )
            ...
            case .supportedBolusVolumes:
                if pumpManager.state.deliverableIncrements == .custom,
                   pumpManager.state.supportedBolusVolumes.indices.contains(1)
                {
                    let bolusVolumes = pumpManager.state.supportedBolusVolumes
                    let vc = SupportedRangeTableViewController(
                        minValue: bolusVolumes.first!,
                        maxValue: bolusVolumes.last!,
                        stepSize: bolusVolumes[1] - bolusVolumes.first!
                    )
            ...
            case .insulinType:
                let view = InsulinTypeSetting(
                    initialValue: pumpManager.state.insulinType,
                    supportedInsulinTypes: InsulinType.allCases,
                    allowUnsetInsulinType: true
                ) { newType in
...
extension MockPumpManagerSettingsViewController: PumpManagerStatusObserver {
    public func pumpManager(_: PumpManager, didUpdate status: PumpManagerStatus, oldStatus _: PumpManagerStatus) {
        dispatchPrecondition(condition: .onQueue(.main))

        if let suspendResumeTableViewCell = tableView?
            .cellForRow(at: IndexPath(
                row: ActionRow.suspendResume.rawValue,
...
extension MockPumpManagerSettingsViewController: TextFieldTableViewControllerDelegate {
    ...
    private func update(from controller: TextFieldTableViewController) {
        guard let indexPath = controller.indexPath else { assertionFailure()
            return }
