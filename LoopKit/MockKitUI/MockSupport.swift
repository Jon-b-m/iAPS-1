...
public class MockSupport: SupportUI {
    ...
    public func checkVersion(bundleIdentifier _: String, currentVersion _: String) async -> VersionUpdate? {
    ...
    @ViewBuilder public func supportMenuItem(
        supportInfoProvider _: SupportInfoProvider,
        urlHandler _: @escaping (URL) -> Void
    ) -> some View {
        SupportMenuItem(mockSupport: self)
    }

    public func softwareUpdateView(
        bundleIdentifier: String,
        currentVersion: String,
        guidanceColors _: GuidanceColors,
        openAppStore: (() -> Void)?
    ) -> AnyView? {
        AnyView(
            Button(
                "versionUpdate: \(versionUpdate!.localizedDescription)\n\nbundleIdentifier: \(bundleIdentifier)\n\ncurrentVersion: \(currentVersion)"
            ) {
                openAppStore?()
            }
        )
...
extension MockSupport {
    ...
    private func maybeIssueAlert(_ versionUpdate: VersionUpdate) {
        ...
        let alertIdentifier = Alert.Identifier(
            managerIdentifier: MockSupport.pluginIdentifier,
            alertIdentifier: versionUpdate.rawValue
        )
        let alertContent: LoopKit.Alert.Content
        if firstAlert {
            alertContent = Alert.Content(
                title: versionUpdate.localizedDescription,
                body: String(format: LocalizedString(
                    ...
                    """,
                    comment: "Alert content body for first software update alert (1: app name)(2: app name)"
                ), appName, appName),
        ...
        {
            alertContent = Alert.Content(
                title: LocalizedString("Update Reminder", comment: "Recurring software update alert title"),
                body: String(format: LocalizedString(
                    """
...
struct SupportMenuItem: View {
    ...
    private var actionSheet: ActionSheet {
        ActionSheet(
            title: Text("Version Check Response"),
            message: Text("How should the simulator respond to a version check?"),
            buttons: buttons
        )
    }
    ...
    var currentVersionUpdate: String {
        mockSupport.versionUpdate.map { "(\($0.rawValue))" } ?? ""
    }
}
