...
public final class MockPumpManager: TestingPumpManager {
    ...
    public var localizedTitle: String {
        MockPumpManager.localizedTitle
    }
    ...
    public var pumpReservoirCapacity: Double {
        MockPumpManager.pumpReservoirCapacity
    }

    public var reservoirFillFraction: Double {
        get {
            state.reservoirUnitsRemaining / pumpReservoirCapacity
        }
        set {
    ...
    public var testLastReconciliation: Date?

    public var lastSync: Date? {
        testLastReconciliation ?? Date()
    }
    ...
    private func basalDeliveryState(for state: MockPumpManagerState) -> PumpManagerStatus.BasalDeliveryState? {
        if case let .suspended(date) = state.suspendState {
            return .suspended(date)
        }
        if state.occlusionDetected || state.pumpErrorDetected || state.pumpBatteryChargeRemaining == 0 || state
        ...
        if case let .resumed(date) = state.suspendState {
            return .active(date)
        } else {
            return .active(Date())
        }
    ...
    public func buildPumpStatusHighlight(for state: MockPumpManagerState) -> PumpStatusHighlight? {
        if state.deliveryIsUncertain {
            ...
            return PumpStatusHighlight(
                localizedMessage: LocalizedString("Pump Occlusion", comment: "Status highlight that an occlusion was detected."),
                imageName: "exclamationmark.circle.fill",
                state: .critical
            )
        } else if state.pumpErrorDetected {
            return PumpStatusHighlight(
                localizedMessage: LocalizedString("Pump Error", comment: "Status highlight that a pump error occurred."),
                imageName: "exclamationmark.circle.fill",
                state: .critical
            )
        } else if pumpBatteryChargeRemaining == 0 {
            return PumpStatusHighlight(
                localizedMessage: LocalizedString("Pump Battery Dead", comment: "Status highlight that pump has a dead battery."),
                imageName: "exclamationmark.circle.fill",
                state: .critical
            )
    ...
    public var state: MockPumpManagerState {
        didSet {
            ...
            if (newValue.occlusionDetected != oldValue.occlusionDetected && newValue.occlusionDetected) ||
                ...
                (
                    newValue.pumpBatteryChargeRemaining != oldValue.pumpBatteryChargeRemaining && newValue
            ...
            delegate.notify { delegate in
                if newValue.reservoirUnitsRemaining != oldValue.reservoirUnitsRemaining {
                    delegate?.pumpManager(self, didReadReservoirValue: self.state.reservoirUnitsRemaining, at: Date()) { _ in
                        // nothing to do here
                    }
                }
    ...
    private func logDeviceCommunication(_ message: String, type: DeviceLogEntryType = .send) {
        delegate.delegate?.deviceManager(
            self,
            logEventForDeviceIdentifier: "MockId",
            type: type,
            message: message,
            completion: nil
        )
    ...
    public func createBolusProgressReporter(reportingOn dispatchQueue: DispatchQueue) -> DoseProgressReporter? {
        if case let .inProgress(dose) = status.bolusState {
    ...
    private func storePumpEvents(completion: @escaping (_ error: Error?) -> Void) {
        ...
        delegate.notify { delegate in
            delegate?.pumpManager(
                self,
                hasNewPumpEvents: pendingPumpEvents,
    ...
    private func logDeviceComms(_ type: DeviceLogEntryType, message: String) {
        delegate.delegate?.deviceManager(
            self,
            logEventForDeviceIdentifier: "mockpump",
            type: type,
            message: message,
            completion: nil
        )
    ...
    public func enactBolus(
        ...
        if state.bolusEnactmentShouldError || state.pumpBatteryChargeRemaining == 0 {
            ...
            let bolus = UnfinalizedDose(
                bolusAmount: units,
                startTime: Date(),
                duration: .minutes(units / type(of: self).deliveryUnitsPerMinute),
                insulinType: state.insulinType,
    ...
    public func cancelBolus(completion: @escaping (PumpManagerResult<DoseEntry?>) -> Void) {
        ...
        if state.bolusCancelShouldError {
            let error = PumpManagerError.communication(MockPumpManagerError.communicationFailure)
    ...
    public func suspendDelivery(completion: @escaping (Error?) -> Void) {
        ...
        if state.deliverySuspensionShouldError {
            ...
            state.finalizedDoses.append(suspend)
            state.suspendState = .suspended(suspendDate)
    ...
    public func injectPumpEvents(_ pumpEvents: [NewPumpEvent]) {
        ...
        delegate.notify { delegate in
            delegate?
                .pumpManager(self, hasNewPumpEvents: pumpEvents, lastReconciliation: Date(), replacePendingEvents: true) { _ in }
        }
    }
    ...
    public func syncBasalRateSchedule(
        ...
        state.basalRateSchedule = BasalRateSchedule(dailyItems: scheduleItems, timeZone: status.timeZone)
