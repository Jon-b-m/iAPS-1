...
public final class MockService: Service {
    ...
    public init?(rawState: RawStateValue) {
        remoteData = rawState["remoteData"] as? Bool ?? false
        logging = rawState["logging"] as? Bool ?? false
        analytics = rawState["analytics"] as? Bool ?? false
    }
...
extension MockService: LoggingService {
    public func log(_ message: StaticString, subsystem _: String, category _: String, type _: OSLogType, _ args: [CVarArg]) {
        if logging {
            // Since this is only stored in memory, do not worry about public/private qualifiers
            let messageWithoutQualifiers = message.description.replacingOccurrences(of: "%{public}", with: "%")
...
extension MockService: RemoteDataService {
    public func uploadTemporaryOverrideData(
        updated: [TemporaryScheduleOverride],
        deleted: [TemporaryScheduleOverride],
    ...
    public func uploadCarbData(
        ...
        if remoteData {
            record(
                "[RemoteDataService] Upload carb data (created: \(created.count), updated: \(updated.count), deleted: \(deleted.count))"
            )
        }
        completion(.success(false))
    }
    ...
    public func uploadDosingDecisionData(_ stored: [StoredDosingDecision], completion: @escaping (Result<Bool, Error>) -> Void) {
        if remoteData {
            ...
            record(
                "[RemoteDataService] Upload dosing decision data (stored: \(stored.count), warned: \(warned.count), errored: \(errored.count))"
            )
        }
        completion(.success(false))
    }
