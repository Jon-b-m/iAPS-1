...
public struct MockCGMDataSource {
    public enum Model {
        public typealias SineCurveParameters = (
            baseGlucose: HKQuantity,
            amplitude: HKQuantity,
            period: TimeInterval,
            referenceDate: Date
        )
    ...
    func backfillData(from interval: DateInterval, completion: @escaping (CGMReadingResult) -> Void) {
        ...
        let request = MockGlucoseProvider.BackfillRequest(
            datingBack: interval.duration,
            dataPointFrequency: dataPointFrequency.frequency
        )
...
extension MockCGMDataSource: RawRepresentable {
    ...
    public init?(rawValue: RawValue) {
        guard
            ...
            let dataPointFrequency = (rawValue["dataPointFrequency"] as? MeasurementFrequency.RawValue)
            .flatMap(MeasurementFrequency.init(rawValue:))
        else {
            return nil
        }
...
extension MockCGMDataSource.Model: RawRepresentable {
    ...
    public init?(rawValue: RawValue) {
        ...
        case .scenario:
            let pastGlucoseValues = (rawValue["pastGlucoseValues"] as? [NewGlucoseSample.RawValue])?
                .compactMap({ NewGlucoseSample(rawValue: $0) }) ?? []
            let futureGlucoseValues = (rawValue["futureGlucoseValues"] as? [NewGlucoseSample.RawValue])?
                .compactMap({ NewGlucoseSample(rawValue: $0) }) ?? []
    ...
    public var rawValue: RawValue {
        ...
        case let .scenario(pastGlucoseValues, futureGlucoseValues):
...
extension MockCGMDataSource.Effects: RawRepresentable {
    ...
    public init?(rawValue: RawValue) {
        ...
        randomLowOutlier = randomOutlier(forKey: "randomLowOutlier")
        randomHighOutlier = randomOutlier(forKey: "randomHighOutlier")
        randomErrorChance = rawValue["randomErrorChance"] as? Double
    }
