...
public struct MockCGMState: GlucoseDisplayable {
    ...
    public var cgmLowerLimit: HKQuantity {
        get {
            HKQuantity.init(unit: unit, doubleValue: cgmLowerLimitValue)
        }
        set {
    ...
    public var urgentLowGlucoseThreshold: HKQuantity {
        get {
            HKQuantity.init(unit: unit, doubleValue: urgentLowGlucoseThresholdValue)
        }
        set {
            var newDoubleValue = newValue.doubleValue(for: unit)
    ...
    public var lowGlucoseThreshold: HKQuantity {
        get {
            HKQuantity.init(unit: unit, doubleValue: lowGlucoseThresholdValue)
        }
        set {
            var newDoubleValue = newValue.doubleValue(for: unit)
            if newDoubleValue <= urgentLowGlucoseThresholdValue {
    ...
    public var highGlucoseThreshold: HKQuantity {
        get {
            HKQuantity.init(unit: unit, doubleValue: highGlucoseThresholdValue)
        }
        set {
            var newDoubleValue = newValue.doubleValue(for: unit)
            if newDoubleValue <= lowGlucoseThresholdValue {
    ...
    public var cgmUpperLimit: HKQuantity {
        get {
            HKQuantity.init(unit: unit, doubleValue: cgmUpperLimitValue)
        }
        set {
            var newDoubleValue = newValue.doubleValue(for: unit)
            if newDoubleValue <= highGlucoseThresholdValue {
...
extension MockCGMLifecycleProgress: RawRepresentable {
    ...
    public init?(rawValue: RawValue) {
        guard let percentComplete = rawValue["percentComplete"] as? Double,
              ...
              let progressState = DeviceLifecycleProgressState(rawValue: progressStateRawValue)
        else {
            return nil
        }
...
public final class MockCGMManager: TestingCGMManager {
    ...
    public static let submarine = MockAlert(
        sound: .sound(name: "sub.caf"),
        identifier: "submarine",
        foregroundContent: Alert.Content(
            title: "Alert: FG Title",
            body: "Alert: Foreground Body",
            acknowledgeActionButtonLabel: "FG OK"
        ),
        backgroundContent: Alert.Content(
            title: "Alert: BG Title",
            body: "Alert: Background Body",
            acknowledgeActionButtonLabel: "BG OK"
        ),
        interruptionLevel: .timeSensitive
    )
    public static let critical = MockAlert(
        sound: .sound(name: "critical.caf"),
        identifier: "critical",
        foregroundContent: Alert.Content(
            title: "Critical Alert: FG Title",
            body: "Critical Alert: Foreground Body",
            acknowledgeActionButtonLabel: "Critical FG OK"
        ),
        backgroundContent: Alert.Content(
            title: "Critical Alert: BG Title",
            body: "Critical Alert: Background Body",
            acknowledgeActionButtonLabel: "Critical BG OK"
        ),
        interruptionLevel: .critical
    )
    public static let buzz = MockAlert(
        sound: .vibrate,
        identifier: "buzz",
        foregroundContent: Alert.Content(
            title: "Alert: FG Title",
            body: "FG bzzzt",
            acknowledgeActionButtonLabel: "Buzz"
        ),
        backgroundContent: Alert.Content(
            title: "Alert: BG Title",
            body: "BG bzzzt",
    ...
    public static let signalLoss = MockAlert(
        sound: .sound(name: "critical.caf"),
        identifier: "signalLoss",
        foregroundContent: Alert.Content(
            title: "Signal Loss",
            body: "CGM simulator signal loss",
    ...
    public var mockSensorState: MockCGMState {
        ...
        set {
            ...
            notifyStatusObservers(cgmManagerStatus: cgmManagerStatus)
        }
    }
    ...
    public init?(rawState: RawStateValue) {
        ...
        {
            lockedMockSensorState.value = mockSensorState
        } else {
            lockedMockSensorState.value = MockCGMState(isStateValid: true)
        }
        ...
        {
            lockedDataSource.value = dataSource
        } else {
            lockedDataSource.value = MockCGMDataSource(model: .sineCurve(parameters: (
                baseGlucose: HKQuantity(unit: .milligramsPerDeciliter, doubleValue: 110),
                amplitude: HKQuantity(unit: .milligramsPerDeciliter, doubleValue: 20),
                period: TimeInterval(hours: 6),
                referenceDate: Date()
            )))
        }
    ...
    public var shouldSyncToRemoteService: Bool {
        mockSensorState.samplesShouldBeUploaded
    }
    ...
    private func logDeviceComms(_ type: DeviceLogEntryType, message: String) {
        delegate.delegate?.deviceManager(
            self,
            logEventForDeviceIdentifier: "mockcgm",
            type: type,
            message: message,
            completion: nil
        )
    ...
    private func sendCGMReadingResult(_ result: CGMReadingResult) {
        if case let .newData(samples) = result,
    ...
    public func backfillData(datingBack duration: TimeInterval) {
        let now = Date()
        logDeviceComms(.send, message: "backfillData(\(duration))")
    ...
    private func setupGlucoseUpdateTimer() {
        glucoseUpdateTimer = Timer
            .scheduledTimer(withTimeInterval: dataSource.dataPointFrequency.frequency, repeats: true) { [weak self] _ in
    ...
    public func trigger(action _: DeviceAction) {}
...
private extension NewGlucoseSample {
...
public extension MockCGMManager {
    ...
    var hasRetractableAlert: Bool {
        // signal loss alerts can only be removed by switching the CGM data source
        currentAlertIdentifier != nil && currentAlertIdentifier != MockCGMManager.signalLoss.identifier
    }

    var currentAlertIdentifier: Alert.AlertIdentifier? {
        mockSensorState.cgmStatusHighlight?.alertIdentifier
    }
    ...
    func issueAlert(
        ...
        delegate.notifyDelayed(by: delay ?? 0) { delegate in
            ...
            delegate?
                .issueAlert(Alert(
                    identifier: Alert
                        .Identifier(managerIdentifier: self.pluginIdentifier, alertIdentifier: identifier),
        ...
        setStatusHighlight(MockCGMStatusHighlight(
            localizedMessage: alert.foregroundContent.title,
            alertIdentifier: alert.identifier
        ))
    }

    func issueSignalLossAlert() {
        issueAlert(identifier: MockCGMManager.signalLoss.identifier, trigger: .immediate, delay: nil)
    }

    func retractSignalLossAlert() {
    ...
    func acknowledgeAlert(alertIdentifier: Alert.AlertIdentifier, completion: @escaping (Error?) -> Void) {
        logDeviceComms(.delegateResponse, message: "\(#function): Alert \(alertIdentifier) acknowledged.")
        completion(nil)
    }

    func retractCurrentAlert() {
        guard hasRetractableAlert, let identifier = currentAlertIdentifier else { return }

        retractAlert(identifier: identifier)
    }

    func retractAlert(identifier: Alert.AlertIdentifier) {
        delegate
            .notify {
                $0?
                    .retractAlert(
    ...
    func setStatusHighlight(_ statusHighlight: MockCGMStatusHighlight?) {
    ...
    private func issueAlert(for glucose: NewGlucoseSample) {
        ...
        let alertIdentifier = Alert.Identifier(
...
public extension MockCGMManager {
    func requestCalibration(_ requestCalibration: Bool) {
    ...
    var cgmBatteryChargeRemaining: Double? {
        get {
            mockSensorState.cgmBatteryChargeRemaining
        }
        set {
    ...
    var isCalibrationRequested: Bool {
        mockSensorState.cgmStatusBadge?.badgeType == .calibrationRequested
    }
...
extension MockCGMState: RawRepresentable {
    ...
    public init?(rawValue: RawValue) {
        guard let isStateValid = rawValue["isStateValid"] as? Bool,
              ...
              let cgmUpperLimitValue = rawValue["cgmUpperLimitValue"] as? Double
        else {
            return nil
        }
        ...
        self.isStateValid = isStateValid
        self.glucoseAlertingEnabled = glucoseAlertingEnabled
        samplesShouldBeUploaded = rawValue["samplesShouldBeUploaded"] as? Bool ?? false
        self.urgentLowGlucoseThresholdValue = urgentLowGlucoseThresholdValue
        self.lowGlucoseThresholdValue = lowGlucoseThresholdValue
        self.highGlucoseThresholdValue = highGlucoseThresholdValue
        self.cgmLowerLimitValue = cgmLowerLimitValue
        ...
        {
            cgmStatusHighlight = MockCGMStatusHighlight(localizedMessage: localizedMessage, alertIdentifier: alertIdentifier)
        }
        ...
        {
            cgmStatusBadge = MockCGMStatusBadge(badgeType: statusBadgeType)
        }

        if let cgmLifecycleProgressRawValue = rawValue["cgmLifecycleProgress"] as? MockCGMLifecycleProgress.RawValue {
            cgmLifecycleProgress = MockCGMLifecycleProgress(rawValue: cgmLifecycleProgressRawValue)
        }
        ...
        cgmBatteryChargeRemaining = rawValue["cgmBatteryChargeRemaining"] as? Double
        ...
        if let trendTypeRaw = rawValue["trendType"] as? GlucoseTrend.RawValue {
            trendType = GlucoseTrend(rawValue: trendTypeRaw)
        }

        if let currentGlucoseValue = rawValue["currentGlucoseValue"] as? Double {
            currentGlucose = HKQuantity(unit: .milligramsPerDeciliter, doubleValue: currentGlucoseValue)
        }
