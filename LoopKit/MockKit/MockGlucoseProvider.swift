...
struct MockGlucoseProvider {
    struct BackfillRequest {
        ...
        var dataPointCount: Int {
            Int(duration / dataPointFrequency)
        }
    ...
    func backfill(_ backfill: BackfillRequest, endingAt date: Date, completion: @escaping (CGMReadingResult) -> Void) {
        let dataPointDates = (0 ... backfill.dataPointCount).map { offset in
            date.addingTimeInterval(-backfill.dataPointFrequency * Double(offset))
        }
...
private extension MockGlucoseProvider {
    static func constant(_ quantity: HKQuantity) -> MockGlucoseProvider {
        MockGlucoseProvider { date, completion in
            let sample = glucoseSample(
                at: date,
                quantity: quantity,
                condition: nil,
                trend: .flat,
                trendRate: HKQuantity(unit: .milligramsPerDeciliterPerMinute, doubleValue: 0)
            )
            completion(.newData([sample]))
        }
    }

    static func sineCurve(parameters: MockCGMDataSource.Model.SineCurveParameters) -> MockGlucoseProvider {
        ...
        return MockGlucoseProvider { date, completion in
            ...
            func sine(_ t: TimeInterval) -> Double {
                Double(baseGlucoseValue + amplitudeValue * sin(2 * .pi / period * t)).rounded()
            }
            let glucoseValue = sine(timeOffset)
            var trend: GlucoseTrend?
            var trendRate: HKQuantity?
            if let prevGlucoseValue = prevGlucoseValue,
               let trendRateValue = coinFlip(
                   withChanceOfHeads: chanceOfNilTrendRate,
                   ifHeads: nil,
                   ifTails: glucoseValue - prevGlucoseValue
               )
            {
                let smallDelta = 0.9
                let mediumDelta = 2.0
            ...
            let sample = glucoseSample(
                at: date,
                quantity: HKQuantity(unit: unit, doubleValue: glucoseValue),
                condition: nil,
                trend: trend,
                trendRate: trendRate
            )
    ...
    static func scenario(pastSamples: [NewGlucoseSample], futureSamples: [NewGlucoseSample]) -> MockGlucoseProvider {
    ...
    static var noData: MockGlucoseProvider {
        MockGlucoseProvider { _, completion in completion(.noData) }
    }

    static var signalLoss: MockGlucoseProvider {
        MockGlucoseProvider { _, _ in }
    }

    static var unreliableData: MockGlucoseProvider {
        MockGlucoseProvider { _, completion in completion(.unreliableData) }
    }

    static func error(_ error: Error) -> MockGlucoseProvider {
        MockGlucoseProvider { _, completion in completion(.error(error)) }
    }
}
...
private extension MockGlucoseProvider {
    func withRandomNoise(upTo magnitude: HKQuantity) -> MockGlucoseProvider {
        let unit = HKUnit.milligramsPerDeciliter
    ...
    func randomlyProducingLowOutlier(withChance chanceOfOutlier: Double, outlierDelta: HKQuantity) -> MockGlucoseProvider {
        randomlyProducingOutlier(withChance: chanceOfOutlier, outlierDeltaMagnitude: outlierDelta, outlierDeltaSign: -)
    }

    func randomlyProducingHighOutlier(withChance chanceOfOutlier: Double, outlierDelta: HKQuantity) -> MockGlucoseProvider {
        randomlyProducingOutlier(withChance: chanceOfOutlier, outlierDeltaMagnitude: outlierDelta, outlierDeltaSign: +)
    }
    ...
    func randomlyErroringOnNewData(withChance chance: Double) -> MockGlucoseProvider {
        mapResult { result in
            coinFlip(withChanceOfHeads: chance, ifHeads: .error(MockGlucoseProviderError()), ifTails: result)
        }
    }
    ...
    private func mapGlucoseQuantities(_ transform: @escaping (HKQuantity) -> HKQuantity) -> MockGlucoseProvider {
        mapResult { result in
            result.mapGlucoseQuantities(transform)
        }
    }
}
...
private extension CGMReadingResult {
    func mapGlucoseQuantities(_ transform: (HKQuantity) -> HKQuantity) -> CGMReadingResult {
        guard case let .newData(samples) = self else {
            return self
        }
        return .newData(
...
private extension MockCGMDataSource.Model {
    var glucoseProvider: MockGlucoseProvider {
        ...
        case let .sineCurve(parameters: parameters):
            return .sineCurve(parameters: parameters)
        case let .scenario(pastSamples, futureSamples):
...
private extension MockCGMDataSource.Effects {
    var transformations: [(MockGlucoseProvider) -> MockGlucoseProvider] {
        ...
        [
            ...
            randomHighOutlier
                .map { chance, delta in { $0.randomlyProducingHighOutlier(withChance: chance, outlierDelta: delta) } },
