...
class UnfinalizedDoseTests: XCTestCase {
    ...
    func testInitializationTBR() {
        ...
        let unfinalizedTBR = UnfinalizedDose(
    ...
    func testIsFinished() {
        ...
        let now = Date()
        let duration = TimeInterval.minutes(30)
        var unfinalizedTBR = UnfinalizedDose(
            tempBasalRate: amount,
            startTime: now,
            duration: duration
        )
        ...
        unfinalizedTBR = UnfinalizedDose(
            tempBasalRate: amount,
    ...
    func testRestoreFromRawValue() {
        ...
        let startTime = Date()
        let duration = TimeInterval.minutes(30)
        let expectedUnfinalizedTBR = UnfinalizedDose(
            tempBasalRate: rate,
            startTime: startTime,
            duration: duration
        )
