...
class InsulinDeliveryStoreTestsBase: PersistenceControllerTestCase {
    ...
    override func tearDown() {
        ...
        insulinDeliveryStore
            .purgeAllDoseEntries(healthKitPredicate: HKQuery.predicateForObjects(from: HKSource.default())) { error in
                XCTAssertNil(error)
                semaphore.signal()
            }
        semaphore.wait()
...
class InsulinDeliveryStoreTests: InsulinDeliveryStoreTestsBase {
    ...
    func testHealthKitQueryAnchorPersistence() {
        ...
        _ = InsulinDeliveryStore(
            healthKitSampleStore: newSampleStore,
            cacheStore: cacheStore,
            provenanceIdentifier: HKSource.default().bundleIdentifier
        )
    ...
    func testGetDoseEntries() {
        ...
        insulinDeliveryStore.getDoseEntries(
            start: Date(timeIntervalSinceNow: -.minutes(3.75)),
            end: Date(timeIntervalSinceNow: -.minutes(1.75))
        ) { result in
            switch result {
            case let .failure(error):
                XCTFail("Unexpected failure: \(error)")
    ...
    func testLastBasalEndDate() {
        ...
        insulinDeliveryStore.getLastImmutableBasalEndDate { result in
            ...
            case let .failure(error):
                XCTFail("Unexpected failure: \(error)")
            case let .success(lastBasalEndDate):
    ...
    func testAddDoseEntriesNotification() {
        ...
        let observer = NotificationCenter.default.addObserver(
            forName: InsulinDeliveryStore.doseEntriesDidChange,
            object: insulinDeliveryStore,
            queue: nil
        ) { _ in
...
class InsulinDeliveryStoreQueryTests: PersistenceControllerTestCase {
    ...
    func testDoseEmptyWithDefaultQueryAnchor() {
        insulinDeliveryStore.executeDoseQuery(fromQueryAnchor: queryAnchor, limit: limit) { result in
            ...
            case let .failure(error):
                XCTFail("Unexpected failure: \(error)")
            case let .success(anchor, created, deleted):
