...
class QuantityFormatterTests: XCTestCase {
    ...
    func testAvoidLineBreaking() {
        ...
        XCTAssertEqual("U", formatter.localizedUnitStringWithPlurality())
        XCTAssertEqual(
            "10\(String.nonBreakingSpace)U",
            formatter.string(from: HKQuantity(unit: HKUnit.internationalUnit(), doubleValue: 10))!
        )
        formatter.unitStyle = .short
        XCTAssertEqual(
            "10\(String.wordJoiner)U",
            formatter.string(from: HKQuantity(unit: HKUnit.internationalUnit(), doubleValue: 10))!
        )
        formatter.unitStyle = .long
        XCTAssertEqual("Units", formatter.localizedUnitStringWithPlurality())
        ...
        XCTAssertEqual(
            "60\(String.nonBreakingSpace)mg\(String.wordJoiner)/\(String.wordJoiner)dL",
            formatter.string(from: HKQuantity(unit: HKUnit.milligramsPerDeciliter, doubleValue: 60))!
        )
        XCTAssertEqual(
            "180\(String.nonBreakingSpace)mg\(String.wordJoiner)/\(String.wordJoiner)dL",
            formatter.string(from: HKQuantity(unit: HKUnit.milligramsPerDeciliter, doubleValue: 180))!
        )
        formatter.unitStyle = .short
        XCTAssertEqual("mg\(String.wordJoiner)/\(String.wordJoiner)dL", formatter.localizedUnitStringWithPlurality())
        XCTAssertEqual(
            "60\(String.wordJoiner)mg\(String.wordJoiner)/\(String.wordJoiner)dL",
            formatter.string(from: HKQuantity(unit: HKUnit.milligramsPerDeciliter, doubleValue: 60))!
        )
        XCTAssertEqual(
            "180\(String.wordJoiner)mg\(String.wordJoiner)/\(String.wordJoiner)dL",
            formatter.string(from: HKQuantity(unit: HKUnit.milligramsPerDeciliter, doubleValue: 180))!
        )
        formatter.unitStyle = .long
        XCTAssertEqual("milligrams per deciliter", formatter.localizedUnitStringWithPlurality())
        ...
        XCTAssertEqual(
            ...
            formatter.string(from: HKQuantity(unit: HKUnit.milligramsPerDeciliter, doubleValue: 0))!
        )
        XCTAssertEqual(
            "1 milligrams per deciliter",
            formatter.string(from: HKQuantity(unit: HKUnit.milligramsPerDeciliter, doubleValue: 1))!
        )
        ...
        XCTAssertEqual(
            "6.0\(String.nonBreakingSpace)mmol\(String.wordJoiner)/\(String.wordJoiner)L",
            formatter.string(from: HKQuantity(unit: HKUnit.millimolesPerLiter, doubleValue: 6))!
        )
        XCTAssertEqual(
            "7.8\(String.nonBreakingSpace)mmol\(String.wordJoiner)/\(String.wordJoiner)L",
            formatter.string(from: HKQuantity(unit: HKUnit.millimolesPerLiter, doubleValue: 7.84))!
        )
        XCTAssertEqual(
            "12.0\(String.nonBreakingSpace)mmol\(String.wordJoiner)/\(String.wordJoiner)L",
            formatter.string(from: HKQuantity(unit: HKUnit.millimolesPerLiter, doubleValue: 12))!
        )
        formatter.unitStyle = .short
        XCTAssertEqual("mmol\(String.wordJoiner)/\(String.wordJoiner)L", formatter.localizedUnitStringWithPlurality())
        XCTAssertEqual(
            "6.0\(String.wordJoiner)mmol\(String.wordJoiner)/\(String.wordJoiner)L",
            formatter.string(from: HKQuantity(unit: HKUnit.millimolesPerLiter, doubleValue: 6))!
        )
        XCTAssertEqual(
            "7.8\(String.wordJoiner)mmol\(String.wordJoiner)/\(String.wordJoiner)L",
            formatter.string(from: HKQuantity(unit: HKUnit.millimolesPerLiter, doubleValue: 7.8))!
        )
        formatter.unitStyle = .long
        XCTAssertEqual("millimoles per liter", formatter.localizedUnitStringWithPlurality())
        XCTAssertEqual(
            "5.5 millimoles per liter",
            formatter.string(from: HKQuantity(unit: HKUnit.millimolesPerLiter, doubleValue: 5.5))!
        )
        XCTAssertEqual(
            "0.0 millimoles per liter",
            formatter.string(from: HKQuantity(unit: HKUnit.millimolesPerLiter, doubleValue: 0))!
        )
        XCTAssertEqual(
            "1.0 millimoles per liter",
            formatter.string(from: HKQuantity(unit: HKUnit.millimolesPerLiter, doubleValue: 1))!
        )
    }
