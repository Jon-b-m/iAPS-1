...
class CarbStorePersistenceTests: PersistenceControllerTestCase, CarbStoreDelegate {
    ...
    func carbStore(_: CarbStore, didError _: CarbStore.CarbStoreError) {}

    // MARK: -

    func testGetCarbEntriesAfterAdd() {
        let firstCarbEntry = NewCarbEntry(
            quantity: HKQuantity(unit: .gram(), doubleValue: 10),
            startDate: Date(timeIntervalSinceNow: -1),
            foodType: "First",
            absorptionTime: .hours(5)
        )
        let secondCarbEntry = NewCarbEntry(
            quantity: HKQuantity(unit: .gram(), doubleValue: 20),
            startDate: Date(),
            foodType: "Second",
            absorptionTime: .hours(3)
        )
        let thirdCarbEntry = NewCarbEntry(
            quantity: HKQuantity(unit: .gram(), doubleValue: 30),
            startDate: Date(timeIntervalSinceNow: 1),
            foodType: "Third",
            absorptionTime: .minutes(30)
        )
    ...
    func testAddCarbEntry() {
        ...
        healthStore.setSaveHandler({ objects, _, _ in
        ...
        carbStore.addCarbEntry(addCarbEntry) { _ in
            addCarbEntryCompletion.fulfill()
        }

        wait(
            for: [addHealthStoreHandler, addCarbEntryCompletion, addCarbEntryHandler, getCarbEntriesCompletion],
            timeout: 10,
            enforceOrder: true
        )
    }
    ...
    func testAddAndReplaceCarbEntry() {
        ...
        let replaceCarbEntry = NewCarbEntry(
            quantity: HKQuantity(unit: .gram(), doubleValue: 15),
            startDate: Date(),
            foodType: "Replace",
            absorptionTime: .hours(4)
        )
        let addHealthStoreHandler = expectation(description: "Add health store handler")
        let addCarbEntryCompletion = expectation(description: "Add carb entry completion")
        let addCarbEntryHandler = expectation(description: "Add carb entry handler")
        ...
        carbStoreHasUpdatedCarbDataHandler = { carbStore in
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                ...
                case 1:
                    ...
                    self.healthStore.setSaveHandler({ objects, _, _ in
                        XCTAssertEqual(1, objects.count)
                    ...
                    self.carbStore
                        .replaceCarbEntry(StoredCarbEntry(managedObject: objects[0]), withEntry: replaceCarbEntry) { _ in
                            updateCarbEntryCompletion.fulfill()
                        }
                case 2:
                    updateCarbEntryHandler.fulfill()
                    let objects: [CachedCarbObject] = self.cacheStore.managedObjectContext.all()
                        .sorted { $0.syncVersion! < $1.syncVersion! }
        ...
        wait(
            for: [
                addHealthStoreHandler,
                addCarbEntryCompletion,
                addCarbEntryHandler,
                updateHealthStoreHandler,
                updateCarbEntryCompletion,
                updateCarbEntryHandler,
                getCarbEntriesCompletion
            ],
            timeout: 10,
            enforceOrder: true
        )
    ...
    func testAddAndDeleteCarbEntry() {
        ...
        carbStoreHasUpdatedCarbDataHandler = { carbStore in
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                ...
                case 1:
                    ...
                    self.healthStore.setDeletedObjectsHandler({ objectType, predicate, _, _, _ in
                    ...
                    self.carbStore.deleteCarbEntry(StoredCarbEntry(managedObject: objects[0])) { _ in
        ...
        wait(
            for: [
                ...
                addCarbEntryCompletion,
                addCarbEntryHandler,
                deleteHealthStoreHandler,
                deleteCarbEntryCompletion,
                deleteCarbEntryHandler,
                getCarbEntriesCompletion
            ],
            timeout: 10,
            enforceOrder: true
        ...
        carbStoreHasUpdatedCarbDataHandler = { carbStore in
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                ...
                case 2:
                    ...
                    self.carbStore.deleteCarbEntry(StoredCarbEntry(managedObject: objects[1])) { _ in
                        deleteCarbEntryCompletion.fulfill()
                    }
                case 3:
                    deleteCarbEntryHandler.fulfill()
                        ...
                        self.carbStore.getCarbEntries(start: Date().addingTimeInterval(-.minutes(1))) { result in
                            ...
                            case let .success(entries):
                                ...
                                for index in 0 ..< 3 {
                                    ...
                                    XCTAssertEqual(
                                        entries[index].provenanceIdentifier,
                                        syncCarbObjects[index].provenanceIdentifier
                                    )
...
class CarbStoreQueryTests: PersistenceControllerTestCase {
    ...
    func testEmptyWithDefaultQueryAnchor() {
        carbStore.executeCarbQuery(fromQueryAnchor: queryAnchor, limit: limit) { result in
            ...
            case let .failure(error):
                XCTFail("Unexpected failure: \(error)")
            case let .success(anchor, created, updated, deleted):
...
class CarbStoreCriticalEventLogTests: PersistenceControllerTestCase {
    ...
    override func setUp() {
        ...
        let objects = [
            SyncCarbObject(
                ...
                foodType: nil,
                grams: 11,
                startDate: dateFormatter.date(from: "2100-01-02T03:08:00Z")!,
                uuid: nil,
                provenanceIdentifier: Bundle.main.bundleIdentifier!,
                syncIdentifier: nil,
                syncVersion: nil,
                ...
                userDeletedDate: nil,
                operation: .create,
                addedDate: dateFormatter.date(from: "2100-01-02T03:08:00Z")!,
                supercededDate: nil
            ),
            SyncCarbObject(
                absorptionTime: nil,
                ...
                foodType: nil,
                grams: 12,
                startDate: dateFormatter.date(from: "2100-01-02T03:10:00Z")!,
                uuid: nil,
                provenanceIdentifier: Bundle.main.bundleIdentifier!,
                syncIdentifier: nil,
                syncVersion: nil,
                ...
                userDeletedDate: nil,
                operation: .create,
                addedDate: dateFormatter.date(from: "2100-01-02T03:10:00Z")!,
                supercededDate: nil
            ),
            SyncCarbObject(
                absorptionTime: nil,
                ...
                foodType: nil,
                grams: 13,
                startDate: dateFormatter.date(from: "2100-01-02T03:04:00Z")!,
                uuid: nil,
                provenanceIdentifier: Bundle.main.bundleIdentifier!,
                syncIdentifier: nil,
                syncVersion: nil,
                ...
                userDeletedDate: nil,
                operation: .create,
                addedDate: dateFormatter.date(from: "2100-01-02T02:04:00Z")!,
                supercededDate: dateFormatter.date(from: "2100-01-02T03:04:00Z")!
            ),
            SyncCarbObject(
                absorptionTime: nil,
                createdByCurrentApp: true,
                foodType: nil,
                grams: 14,
                startDate: dateFormatter.date(from: "2100-01-02T03:06:00Z")!,
                uuid: nil,
                provenanceIdentifier: Bundle.main.bundleIdentifier!,
                syncIdentifier: nil,
                syncVersion: nil,
                ...
                userDeletedDate: nil,
                operation: .create,
                addedDate: dateFormatter.date(from: "2100-01-02T03:06:00Z")!,
                supercededDate: nil
            ),
            SyncCarbObject(
                absorptionTime: nil,
                ...
                foodType: nil,
                grams: 15,
                startDate: dateFormatter.date(from: "2100-01-02T03:02:00Z")!,
                uuid: nil,
                provenanceIdentifier: Bundle.main.bundleIdentifier!,
                syncIdentifier: nil,
                syncVersion: nil,
                ...
                userDeletedDate: nil,
                operation: .create,
                addedDate: dateFormatter.date(from: "2100-01-02T03:02:00Z")!,
                supercededDate: nil
            )
        ]
    ...
    func testExportProgressTotalUnitCount() {
        switch carbStore.exportProgressTotalUnitCount(
    ...
    func testExportCancelled() {
        progress.cancel()
        XCTAssertEqual(carbStore.export(
            startDate: dateFormatter.date(from: "2100-01-02T03:03:00Z")!,
            endDate: dateFormatter.date(from: "2100-01-02T03:09:00Z")!,
            to: outputStream,
            progress: progress
