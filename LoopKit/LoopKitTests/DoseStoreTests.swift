...
class DoseStoreTests: PersistenceControllerTestCase {
    ...
    func defaultStore(testingDate: Date? = nil) -> DoseStore {
        ...
        let doseStore = DoseStore(
            ...
            basalProfile: BasalRateSchedule(rawValue: [
                "timeZone": -28800,
                "items": [
                    ["value": 0.75, "startTime": 0.0],
                    ["value": 0.8, "startTime": 10800.0],
                    ["value": 0.85, "startTime": 32400.0],
                    ["value": 1.0, "startTime": 68400.0]
                ]
            ]),
            insulinSensitivitySchedule: InsulinSensitivitySchedule(rawValue: [
                "unit": "mg/dL",
                "timeZone": -28800,
                "items": [
                    ["value": 40.0, "startTime": 0.0],
                    ["value": 35.0, "startTime": 21600.0],
                    ["value": 40.0, "startTime": 57600.0]
                ]
            ]),
            syncVersion: 1,
    ...
    func testingDate(_ input: String) -> Date {
        testingDateFormatter.date(from: input)!
    }
    ...
    func testGetNormalizedDoseEntriesUsingReservoir() {
        ...
        let pumpEvent = NewPumpEvent(
            date: bolus.startDate,
            dose: bolus,
            raw: Data(hexadecimalString: "0000")!,
            title: "Bolus 2.3U"
        )

        doseStore.addPumpEvents([pumpEvent], lastReconciliation: testingDate("2022-09-05 01:50:18 +0000")) { _ in
    ...
    func testMutableDosesIncludedInIOB() {
        ...
        let pumpEvents: [NewPumpEvent] = [
            ...
            NewPumpEvent(
                date: tempBasal.startDate,
                dose: tempBasal,
                raw: Data(hexadecimalString: "0001")!,
                title: "TempBasal 0.575 U/hr"
            )
        ]

        doseStore.addPumpEvents(pumpEvents, lastReconciliation: lastReconciliation) { _ in
            storageExpectations.fulfill()
        }
    ...
    func testDeduplication() {
        cacheStore.managedObjectContext.performAndWait {
            ...
            bolus1.dose = DoseEntry(
                type: .bolus,
                startDate: bolus1.date!,
                value: 4.15,
                unit: .units,
                syncIdentifier: bolus1.raw?.hexadecimalString
            )
            ...
            bolus2.dose = DoseEntry(
                type: .bolus,
                startDate: bolus2.date!,
                value: 0.15,
                unit: .units,
                syncIdentifier: bolus1.raw?.hexadecimalString
            )
    ...
    func testOutOfOrderDosesSyncedToHealth() {
        ...
        let pumpEvents1 = [
            NewPumpEvent(
                date: f("2018-12-12 17:35:58 +0000"),
                dose: nil,
                raw: UUID().data,
                title: "TempBasalPumpEvent(length: 8, rawData: 8 bytes, rateType: MinimedKit.TempBasalPumpEvent.RateType.Absolute, rate: 2.125, timestamp: calendar: gregorian (fixed) year: 2018 month: 12 day: 12 hour: 9 minute: 35 second: 58 isLeapMonth: false )",
                type: nil
            ),
            NewPumpEvent(
                date: f("2018-12-12 17:35:58 +0000"),
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: f("2018-12-12 17:35:58 +0000"),
                    endDate: f("2018-12-12 18:05:58 +0000"),
                    value: 2.125,
                    unit: .unitsPerHour
                ),
                raw: Data(hexadecimalString: "1601fa23094c12")!,
                title: "TempBasalDurationPumpEvent(length: 7, rawData: 7 bytes, duration: 30, timestamp: calendar: gregorian (fixed) year: 2018 month: 12 day: 12 hour: 9 minute: 35 second: 58 isLeapMonth: false )",
                type: .tempBasal
            )
        ]
        ...
        healthStore.setSaveHandler({ objects, _, error in
        ...
        let pumpEvents3 = [
            NewPumpEvent(
                date: f("2018-12-12 18:16:31 +0000"),
                dose: nil,
                raw: UUID().data,
                title: "TempBasalPumpEvent(length: 8, rawData: 8 bytes, rateType: MinimedKit.TempBasalPumpEvent.RateType.Absolute, rate: 0.0, timestamp: calendar: gregorian (fixed) year: 2018 month: 12 day: 12 hour: 10 minute: 16 second: 31 isLeapMonth: false )",
                type: nil
            ),
            NewPumpEvent(
                date: f("2018-12-12 18:16:31 +0000"),
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: f("2018-12-12 18:16:31 +0000"),
                    endDate: f("2018-12-12 18:46:31 +0000"),
                    value: 0.0,
                    unit: .unitsPerHour
                ),
                raw: Data(hexadecimalString: "1601df100a4c12")!,
                title: "TempBasalDurationPumpEvent(length: 7, rawData: 7 bytes, duration: 30, timestamp: calendar: gregorian (fixed) year: 2018 month: 12 day: 12 hour: 10 minute: 16 second: 31 isLeapMonth: false )",
                type: .tempBasal
            )
        ]
        ...
        healthStore.setSaveHandler({ _, _, _ in
            XCTFail()
        })
    ...
    func testSplitBasalsSyncedToHealth() {
        ...
        let pumpEvents1 = [
            NewPumpEvent(
                date: f("2018-11-29 10:59:28 +0000"),
                dose: nil,
                raw: UUID().data,
                title: "TempBasalPumpEvent(length: 8, rawData: 8 bytes, rateType: MinimedKit.TempBasalPumpEvent.RateType.Absolute, rate: 0.3, timestamp: calendar: gregorian (fixed) year: 2018 month: 11 day: 29 hour: 2 minute: 59 second: 28 isLeapMonth: false )",
                type: nil
            ),
            NewPumpEvent(
                date: f("2018-11-29 10:59:28 +0000"),
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: f("2018-11-29 10:59:28 +0000"),
                    endDate: f("2018-11-29 11:29:28 +0000"),
                    value: 0.3,
                    unit: .unitsPerHour
                ),
                raw: Data(hexadecimalString: "5bffc7cace53e48e87f7cfcb")!,
                title: "TempBasalDurationPumpEvent(length: 7, rawData: 7 bytes, duration: 30, timestamp: calendar: gregorian (fixed) year: 2018 month: 11 day: 29 hour: 2 minute: 59 second: 28 isLeapMonth: false )",
                type: .tempBasal
            )
        ]
        ...
        let pumpEvents3 = [
            NewPumpEvent(
                date: f("2018-11-29 11:09:27 +0000"),
                dose: nil,
                raw: UUID().data,
                title: "TempBasalPumpEvent(length: 8, rawData: 8 bytes, rateType: MinimedKit.TempBasalPumpEvent.RateType.Absolute, rate: 0.325, timestamp: calendar: gregorian (fixed) year: 2018 month: 11 day: 29 hour: 3 minute: 9 second: 27 isLeapMonth: false )",
                type: nil
            ),
            NewPumpEvent(
                date: f("2018-11-29 11:09:27 +0000"),
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: f("2018-11-29 11:09:27 +0000"),
                    endDate: f("2018-11-29 11:39:27 +0000"),
                    value: 0.325,
                    unit: .unitsPerHour
                ),
                raw: Data(hexadecimalString: "5bffca22ce53e48e87f7d624")!,
                title: "TempBasalDurationPumpEvent(length: 7, rawData: 7 bytes, duration: 30, timestamp: calendar: gregorian (fixed) year: 2018 month: 11 day: 29 hour: 3 minute: 9 second: 27 isLeapMonth: false )",
                type: .tempBasal
            )
        ]
        ...
        healthStore.setSaveHandler({ objects, _, error in
            ...
            XCTAssertEqual(
                "BasalRateSchedule 2018-11-29T10:54:28Z 2018-11-29T10:59:28Z",
                basal.metadata![HKMetadataKeySyncIdentifier] as! String
            )
        ...
        let pumpEvents4 = [
            NewPumpEvent(
                date: f("2018-11-29 11:14:28 +0000"),
                dose: nil,
                raw: UUID().data,
                title: "TempBasalPumpEvent(length: 8, rawData: 8 bytes, rateType: MinimedKit.TempBasalPumpEvent.RateType.Absolute, rate: 0, timestamp: calendar: gregorian (fixed) year: 2018 month: 11 day: 29 hour: 3 minute: 14 second: 28 isLeapMonth: false )",
                type: nil
            ),
            NewPumpEvent(
                date: f("2018-11-29 11:14:28 +0000"),
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: f("2018-11-29 11:14:28 +0000"),
                    endDate: f("2018-11-29 11:14:28 +0000"),
                    value: 0.0,
                    unit: .unitsPerHour
                ),
                raw: Data(hexadecimalString: "5bffced1ce53e48e87f7e33b")!,
                title: "TempBasalDurationPumpEvent(length: 7, rawData: 7 bytes, duration: 30, timestamp: calendar: gregorian (fixed) year: 2018 month: 11 day: 29 hour: 3 minute: 14 second: 28 isLeapMonth: false )",
                type: .tempBasal
            )
        ]
        ...
        let pumpEvents5 = [
            NewPumpEvent(
                ...
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: f("2018-11-29 11:14:28 +0000"),
                    endDate: f("2018-11-29 11:44:28 +0000"),
                    value: 1.0,
                    unit: .unitsPerHour,
                    isMutable: true
                ),
                raw: Data(hexadecimalString: "e48e87f7e33b5bffced1ce53")!,
                title: "TempBasalDurationPumpEvent(length: 7, rawData: 7 bytes, duration: 30, timestamp: calendar: gregorian (fixed) year: 2018 month: 11 day: 29 hour: 3 minute: 14 second: 28 isLeapMonth: false )",
                type: .tempBasal
            )
        ]
    ...
    func testAddPumpEventsProgrammedByPumpUI() {
        ...
        let pumpEvents1 = [
            NewPumpEvent(
                date: f("2018-12-12 17:35:00 +0000"),
                dose: nil,
                raw: UUID().data,
                title: "TempBasalPumpEvent(length: 8, rawData: 8 bytes, rateType: MinimedKit.TempBasalPumpEvent.RateType.Absolute, rate: 2.125, timestamp: calendar: gregorian (fixed) year: 2018 month: 12 day: 12 hour: 9 minute: 35 second: 0 isLeapMonth: false )",
                type: nil
            ),
            NewPumpEvent(
                date: f("2018-12-12 17:35:00 +0000"),
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: f("2018-12-12 17:35:00 +0000"),
                    endDate: f("2018-12-12 18:05:00 +0000"),
                    value: 2.125,
                    unit: .unitsPerHour,
                    wasProgrammedByPumpUI: true
                ),
                raw: Data(hexadecimalString: "1601fa23094c12")!,
                title: "TempBasalDurationPumpEvent(length: 7, rawData: 7 bytes, duration: 30, timestamp: calendar: gregorian (fixed) year: 2018 month: 12 day: 12 hour: 9 minute: 35 second: 0 isLeapMonth: false )",
                type: .tempBasal
            )
        ]
    ...
    func testUnfinalizedTempBasalCrossingScheduleChange() {
        ...
        let doseStore = DoseStore(
            ...
            basalProfile: BasalRateSchedule(rawValue: [
                "timeZone": 0,
                "items": [["value": 0.75, "startTime": 0.0], ["value": 0.8, "startTime": 61200.0]]
            ]),
            insulinSensitivitySchedule: InsulinSensitivitySchedule(rawValue: [
                "unit": "mg/dL",
                "timeZone": 0,
                "items": [["value": 40.0, "startTime": 0.0]]
            ]),
            syncVersion: 1,
            provenanceIdentifier: Bundle.main.bundleIdentifier!,
        ...
        let pumpEvent = [
            NewPumpEvent(
                ...
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: doseStart,
                    endDate: doseStart.addingTimeInterval(.minutes(30)),
                    value: 2.125,
                    unit: .unitsPerHour,
                    isMutable: true
                ),
                raw: Data(hexadecimalString: "1234567890")!,
                title: "TempBasal Test Data",
                type: .tempBasal
            )
        ]
    ...
    func testAddPumpEventsPurgesMutableDosesFromInsulinDeliveryStore() {
        ...
        let pumpEvents2 = [
            NewPumpEvent(
                date: f("2018-12-12 18:05:00 +0000"),
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: f("2018-12-12 18:05:00 +0000"),
                    endDate: f("2018-12-12 18:25:00 +0000"),
                    value: 1.375,
                    unit: .unitsPerHour,
                    isMutable: true,
                    wasProgrammedByPumpUI: true
                ),
                raw: Data(hexadecimalString: "3094c121601fa2")!,
                title: "TempBasalDurationPumpEvent(length: 7, rawData: 7 bytes, duration: 30, timestamp: calendar: gregorian (fixed) year: 2018 month: 12 day: 12 hour: 10 minute: 05 second: 0 isLeapMonth: false )",
                type: .tempBasal
            )
        ]
        ...
        let pumpEvents3 = [
            NewPumpEvent(
                ...
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: f("2018-12-12 18:05:00 +0000"),
                    endDate: f("2018-12-12 18:35:00 +0000"),
                    value: 0.875,
                    unit: .unitsPerHour,
                    isMutable: true
                ),
        ...
        let pumpEvents4 = [
            NewPumpEvent(
                date: f("2018-12-12 18:15:00 +0000"),
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: f("2018-12-12 18:15:00 +0000"),
                    endDate: f("2018-12-12 18:45:00 +0000"),
                    value: 0.5,
                    unit: .unitsPerHour,
                    isMutable: true
                ),
                raw: Data(hexadecimalString: "121601f3094ca2")!,
                title: "TempBasalDurationPumpEvent(length: 7, rawData: 7 bytes, duration: 30, timestamp: calendar: gregorian (fixed) year: 2018 month: 12 day: 12 hour: 10 minute: 15 second: 0 isLeapMonth: false )",
                type: .tempBasal
            )
        ]

        var basalDoseEntry: DoseEntry?
        ...
        doseStore.addPumpEvents(pumpEvents4, lastReconciliation: Date()) { error in
            ...
            doseStore.insulinDeliveryStore.getDoseEntries { result in
                ...
                case let .success(doseEntries):
                    ...
                    XCTAssertEqual(
                        doseEntries[1].scheduledBasalRate,
                        HKQuantity(unit: .internationalUnitsPerHour, doubleValue: 0.85)
                    )
        ...
        let pumpEvents6 = [
            NewPumpEvent(
                date: f("2018-12-12 18:25:00 +0000"),
                dose: DoseEntry(
                    type: .tempBasal,
                    startDate: f("2018-12-12 18:25:00 +0000"),
                    endDate: f("2018-12-12 18:40:00 +0000"),
                    value: 0.75,
                    unit: .unitsPerHour
                ),
                raw: Data(hexadecimalString: "1201f3094c16a2")!,
                title: "TempBasalDurationPumpEvent(length: 7, rawData: 7 bytes, duration: 30, timestamp: calendar: gregorian (fixed) year: 2018 month: 12 day: 12 hour: 10 minute: 25 second: 0 isLeapMonth: false )",
                type: .tempBasal
            )
        ]
...
class DoseStoreQueryTests: PersistenceControllerTestCase {
    ...
    let basalProfile = BasalRateSchedule(rawValue: [
        "timeZone": -28800,
        "items": [
            ["value": 0.75, "startTime": 0.0],
            ["value": 0.8, "startTime": 10800.0],
    ...
    let insulinSensitivitySchedule = InsulinSensitivitySchedule(rawValue: [
        "unit": "mg/dL",
        "timeZone": -28800,
        "items": [["value": 40.0, "startTime": 0.0], ["value": 35.0, "startTime": 21600.0], ["value": 40.0, "startTime": 57600.0]]
    ])
...
class DoseStoreCriticalEventLogTests: PersistenceControllerTestCase {
    ...
    override func setUp() {
        ...
        let events = [
            PersistedPumpEvent(
                date: dateFormatter.date(from: "2100-01-02T03:08:00Z")!,
            ...
            PersistedPumpEvent(
                date: dateFormatter.date(from: "2100-01-02T03:10:00Z")!,
                persistedDate: persistedDate,
                dose: nil,
                isUploaded: false,
                objectIDURL: url,
            ...
            PersistedPumpEvent(
                date: dateFormatter.date(from: "2100-01-02T03:04:00Z")!,
                persistedDate: persistedDate,
                dose: nil,
                isUploaded: false,
                objectIDURL: url,
            ...
            PersistedPumpEvent(
                date: dateFormatter.date(from: "2100-01-02T03:06:00Z")!,
                persistedDate: persistedDate,
                dose: nil,
                isUploaded: false,
                objectIDURL: url,
            ...
            PersistedPumpEvent(
                date: dateFormatter.date(from: "2100-01-02T03:02:00Z")!,
                persistedDate: persistedDate,
                dose: nil,
                isUploaded: false,
                objectIDURL: url,
    ...
    func testExportProgressTotalUnitCount() {
        switch doseStore.exportProgressTotalUnitCount(
            startDate: dateFormatter.date(from: "2100-01-02T03:03:00Z")!,
            endDate: dateFormatter.date(from: "2100-01-02T03:09:00Z")!
        ) {
        case let .failure(error):
            XCTFail("Unexpected failure: \(error)")
        case let .success(progressTotalUnitCount):
    ...
    func testExportProgressTotalUnitCountEmpty() {
        switch doseStore.exportProgressTotalUnitCount(
            startDate: dateFormatter.date(from: "2100-01-02T03:00:00Z")!,
    ...
    func testExportCancelled() {
        progress.cancel()
        XCTAssertEqual(doseStore.export(
            startDate: dateFormatter.date(from: "2100-01-02T03:03:00Z")!,
            endDate: dateFormatter.date(from: "2100-01-02T03:09:00Z")!,
            to: outputStream,
            progress: progress
        ) as? CriticalEventLogError, CriticalEventLogError.cancelled)
    }
