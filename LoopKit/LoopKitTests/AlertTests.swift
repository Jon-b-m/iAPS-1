...
class AlertTests: XCTestCase {
    ...
    let foregroundContent = Alert.Content(
        title: "title1",
        body: "body1",
        acknowledgeActionButtonLabel: "acknowledgeActionButtonLabel1"
    )
    let backgroundContent = Alert.Content(
        title: "title2",
        body: "body2",
        acknowledgeActionButtonLabel: "acknowledgeActionButtonLabel2"
    )
    ...
    func testAlertRepeatingEncodable() {
        let alert = Alert(
            ...
            foregroundContent: foregroundContent,
            backgroundContent: backgroundContent,
            trigger: .repeating(repeatInterval: 2.0)
        )
        let str = try! alert.encodeToString()
        let decoded = try! Alert.decode(from: str)
        XCTAssertEqual(alert, decoded)
