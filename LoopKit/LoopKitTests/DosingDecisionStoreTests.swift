...
class DosingDecisionStorePersistenceTests: PersistenceControllerTestCase, DosingDecisionStoreDelegate {
    ...
    func testStoreDosingDecision() {
        ...
        dosingDecisionStoreHasUpdatedDosingDecisionDataHandler = { _ in
            handlerInvocation += 1
    ...
    func testStoreDosingDecisionMultiple() {
        ...
        wait(
            for: [
                storeDosingDecisionHandler1,
                storeDosingDecisionCompletion1,
                storeDosingDecisionHandler2,
                storeDosingDecisionCompletion2
            ],
            timeout: 2,
            enforceOrder: true
        )
...
class DosingDecisionStoreQueryTests: PersistenceControllerTestCase {
    ...
    private func addData(withSyncIdentifiers syncIdentifiers: [UUID]) {
        ...
        for syncIdentifier in syncIdentifiers {
            dosingDecisionStore
                .storeDosingDecision(StoredDosingDecision(reason: "test", syncIdentifier: syncIdentifier)) { semaphore.signal() }
        }
...
class DosingDecisionStoreCriticalEventLogTests: PersistenceControllerTestCase {
    ...
    override func setUp() {
        ...
        let dosingDecisions = [
            StoredDosingDecision(
                ...
                reason: "test",
                syncIdentifier: UUID(uuidString: "18CF3948-0B3D-4B12-8BFE-14986B0E6784")!
            ),
            StoredDosingDecision(
                date: dateFormatter.date(from: "2100-01-02T03:10:00Z")!,
                controllerTimeZone: TimeZone(identifier: "America/Los_Angeles")!,
                reason: "test",
                syncIdentifier: UUID(uuidString: "C86DEB61-68E9-464E-9DD5-96A9CB445FD3")!
            ),
            StoredDosingDecision(
                date: dateFormatter.date(from: "2100-01-02T03:04:00Z")!,
                controllerTimeZone: TimeZone(identifier: "America/Los_Angeles")!,
                reason: "test",
                syncIdentifier: UUID(uuidString: "2B03D96C-6F5D-4140-99CD-80C3E64D6010")!
            ),
            StoredDosingDecision(
                date: dateFormatter.date(from: "2100-01-02T03:06:00Z")!,
                controllerTimeZone: TimeZone(identifier: "America/Los_Angeles")!,
                reason: "test",
                syncIdentifier: UUID(uuidString: "FF1C4F01-3558-4FB2-957E-FA1522C4735E")!
            ),
            StoredDosingDecision(
                date: dateFormatter.date(from: "2100-01-02T03:02:00Z")!,
                controllerTimeZone: TimeZone(identifier: "America/Los_Angeles")!,
                reason: "test",
                syncIdentifier: UUID(uuidString: "71B699D7-0E8F-4B13-B7A1-E7751EB78E74")!
            )
        ]
    ...
    func testExportProgressTotalUnitCount() {
        switch dosingDecisionStore.exportProgressTotalUnitCount(
    ...
    func testExport() {
        XCTAssertNil(dosingDecisionStore.export(
            startDate: dateFormatter.date(from: "2100-01-02T03:03:00Z")!,
            endDate: dateFormatter.date(from: "2100-01-02T03:09:00Z")!,
            to: outputStream,
            progress: progress
    ...
    func testExportCancelled() {
        progress.cancel()
        XCTAssertEqual(dosingDecisionStore.export(
            startDate: dateFormatter.date(from: "2100-01-02T03:03:00Z")!,
            endDate: dateFormatter.date(from: "2100-01-02T03:09:00Z")!,
            to: outputStream,
            progress: progress
...
extension StoredDosingDecision.LastReservoirValue: Equatable {
    public static func == (lhs: StoredDosingDecision.LastReservoirValue, rhs: StoredDosingDecision.LastReservoirValue) -> Bool {
        lhs.startDate == rhs.startDate && lhs.unitVolume == rhs.unitVolume
    }
}
...
extension ManualBolusRecommendationWithDate: Equatable {
    public static func == (lhs: ManualBolusRecommendationWithDate, rhs: ManualBolusRecommendationWithDate) -> Bool {
        lhs.recommendation == rhs.recommendation && lhs.date == rhs.date
    }
}
...
extension ManualBolusRecommendation: Equatable {
    public static func == (lhs: ManualBolusRecommendation, rhs: ManualBolusRecommendation) -> Bool {
        lhs.amount == rhs.amount && lhs.pendingInsulin == rhs.pendingInsulin && lhs.notice == rhs.notice
    }
}

private extension StoredDosingDecision {
    static var test: StoredDosingDecision {
        ...
        let controllerStatus = StoredDosingDecision.ControllerStatus(
            batteryState: .charging,
            batteryLevel: 0.5
        )
        ...
        let cgmManagerStatus = CGMManagerStatus(
            hasValidSensorSession: true,
        ...
        let lastReservoirValue = StoredDosingDecision.LastReservoirValue(
            startDate: dateFormatter.date(from: "2020-05-14T22:07:19Z")!,
            unitVolume: 113.3
        )
        let historicalGlucose = [
            HistoricalGlucoseValue(
                startDate: dateFormatter.date(from: "2020-05-14T22:29:15Z")!,
                quantity: HKQuantity(unit: .milligramsPerDeciliter, doubleValue: 117.3)
            ),
            HistoricalGlucoseValue(
                startDate: dateFormatter.date(from: "2020-05-14T22:33:15Z")!,
                quantity: HKQuantity(unit: .milligramsPerDeciliter, doubleValue: 119.5)
            ),
            HistoricalGlucoseValue(
                startDate: dateFormatter.date(from: "2020-05-14T22:38:15Z")!,
        ...
        let manualGlucoseSample = StoredGlucoseSample(
            uuid: UUID(uuidString: "da0ced44-e4f1-49c4-baf8-6efa6d75525f")!,
            provenanceIdentifier: "com.loopkit.loop",
            syncIdentifier: "d3876f59-adb3-4a4f-8b29-315cda22062e",
            syncVersion: 1,
        ...
        let carbsOnBoard = CarbValue(
            startDate: dateFormatter.date(from: "2020-05-14T22:48:41Z")!,
        ...
        let manualBolusRecommendation = ManualBolusRecommendationWithDate(
            recommendation: ManualBolusRecommendation(
                amount: 1.2,
                pendingInsulin: 0.75,
                notice: .predictedGlucoseBelowTarget(minGlucose: PredictedGlucoseValue(
                    startDate: dateFormatter
                        .date(
                            from: "2020-05-14T23:03:15Z"
                        )!,
                    quantity: HKQuantity(
                        unit: .milligramsPerDeciliter,
                        doubleValue: 75.5
