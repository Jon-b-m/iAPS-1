...
class InsulinSensitivityScheduleTests: XCTestCase {
    func testScheduleFor() {
        ...
        let insulinSensitivityScheduleMMOLL = InsulinSensitivitySchedule(
            ...
            dailyItems: [
                RepeatingScheduleValue(
                    startTime: 0,
                    value: HKQuantity(unit: .milligramsPerDeciliter, doubleValue: value1)
                        .doubleValue(for: .millimolesPerLiter, withRounding: true)
                ),
                RepeatingScheduleValue(
                    startTime: 1000,
                    value: HKQuantity(unit: .milligramsPerDeciliter, doubleValue: value2)
                        .doubleValue(for: .millimolesPerLiter, withRounding: true)
                )
            ]
        )
        ...
        XCTAssertEqual(
            insulinSensitivityScheduleMGDL!.value(at: date),
            insulinSensitivityScheduleMMOLL!.value(for: .milligramsPerDeciliter, at: date)
        )
        XCTAssertEqual(
            insulinSensitivityScheduleMGDL!.value(at: date),
            insulinSensitivityScheduleMGDL!.value(for: .milligramsPerDeciliter, at: date)
        )
    }
}
