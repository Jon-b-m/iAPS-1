...
class SettingsStorePersistenceTests: PersistenceControllerTestCase, SettingsStoreDelegate {
    ...
    func testStoreSettings() {
        ...
        settingsStoreHasUpdatedSettingsDataHandler = { _ in
            handlerInvocation += 1
    ...
    func testStoreSettingsMultiple() {
        ...
        wait(
            for: [storeSettingsHandler1, storeSettingsCompletion1, storeSettingsHandler2, storeSettingsCompletion2],
            timeout: 2,
            enforceOrder: true
        )
    }
    ...
    func testSettingsObjectEncodable() throws {
        cacheStore.managedObjectContext.performAndWait {
            do {
                ...
                try assertSettingsObjectEncodable(
                    object,
                    encodesJSON: """
                    {
                      "data" : {
...
class SettingsStoreQueryTests: PersistenceControllerTestCase {
    ...
    private func addData(withSyncIdentifiers syncIdentifiers: [UUID]) {
        ...
        for syncIdentifier in syncIdentifiers {
            settingsStore.storeSettings(StoredSettings(syncIdentifier: syncIdentifier)) { _ in semaphore.signal() }
        }
...
class SettingsStoreCriticalEventLogTests: PersistenceControllerTestCase {
    ...
    func testExportProgressTotalUnitCount() {
        switch settingsStore.exportProgressTotalUnitCount(
    ...
    func testExport() {
        XCTAssertNil(settingsStore.export(
            startDate: dateFormatter.date(from: "2100-01-02T03:03:00Z")!,
            endDate: dateFormatter.date(from: "2100-01-02T03:09:00Z")!,
            to: outputStream,
            progress: progress
    ...
    func testExportCancelled() {
        progress.cancel()
        XCTAssertEqual(settingsStore.export(
            startDate: dateFormatter.date(from: "2100-01-02T03:03:00Z")!,
            endDate: dateFormatter.date(from: "2100-01-02T03:09:00Z")!,
            to: outputStream,
            progress: progress
...
private extension StoredSettings {
    static var test: StoredSettings {
        ...
        let overridePresets = [TemporaryScheduleOverridePreset(
            id: UUID(uuidString: "2A67A303-5203-4CB8-8263-79498265368E")!,
            symbol: "🍎",
            name: "Apple",
        ...
        let defaultRapidActingModel = StoredInsulinModel(
            modelType: .rapidAdult,
            delay: .minutes(10),
            actionDuration: .hours(6),
            peakActivity: .hours(3)
        )
        ...
        let insulinSensitivitySchedule = InsulinSensitivitySchedule(
            unit: .milligramsPerDeciliter,
            dailyItems: [
                RepeatingScheduleValue(startTime: .hours(0), value: 45.0),
        ...
        let carbRatioSchedule = CarbRatioSchedule(
            unit: .gram(),
            dailyItems: [
                RepeatingScheduleValue(startTime: .hours(0), value: 15.0),
        ...
        let controllerDevice = StoredSettings.ControllerDevice(
            name: "Controller Name",
            systemName: "Controller System Name",
            systemVersion: "Controller System Version",
            model: "Controller Model",
        ...
        return StoredSettings(
            date: dateFormatter.date(from: "2020-05-14T22:48:15Z")!,
