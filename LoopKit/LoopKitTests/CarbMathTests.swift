...
class CarbMathTests: XCTestCase {
    ...
    private func loadSchedules() -> (CarbRatioSchedule, InsulinSensitivitySchedule) {
        ...
        let items = schedule.map {
            RepeatingScheduleValue(startTime: TimeInterval(minutes: $0["offset"] as! Double), value: $0["ratio"] as! Double)
        }
    ...
    func testCarbEffectFromHistory() {
        ...
        let effects = input.glucoseEffects(
            ...
            defaultAbsorptionTime: TimeInterval(minutes: 180),
            absorptionModel: ParabolicAbsorption()
        )
    ...
    func testCarbsOnBoardFromHistory() {
        ...
        for (expected, calculated) in zip(output, cob) {
            XCTAssertEqual(expected.startDate, calculated.startDate)
            XCTAssertEqual(
                expected.quantity.doubleValue(for: HKUnit.gram()),
                calculated.quantity.doubleValue(for: HKUnit.gram()),
                accuracy: Double(Float.ulpOfOne)
            )
        }
    }
    ...
    func testDynamicGlucoseEffectAbsorptionNoneObserved() {
        ...
        let statuses = [futureCarbEntry].map(
            ...
            insulinSensitivity: insulinSensitivities.quantitiesBetween(
                start: futureCarbEntry.startDate,
                end: futureCarbEntry.startDate
            ),
            absorptionTimeOverrun: defaultAbsorptionTimes.slow / defaultAbsorptionTimes.medium,
            defaultAbsorptionTime: defaultAbsorptionTimes.medium,
            delay: TimeInterval(minutes: 0),
        ...
        let effects = statuses.dynamicGlucoseEffects(
            ...
            insulinSensitivities: insulinSensitivities.quantitiesBetween(
                start: futureCarbEntry.startDate,
                end: futureCarbEntry.startDate
            ),
            defaultAbsorptionTime: defaultAbsorptionTimes.medium,
    ...
    func testDynamicAbsorptionPartiallyObserved() {
        ...
        let statuses = [carbEntries[0]].map(
            ...
            insulinSensitivity: insulinSensitivities.quantitiesBetween(
                start: carbEntries[0].startDate,
                end: carbEntries[0].startDate
            ),
            absorptionTimeOverrun: defaultAbsorptionTimes.slow / defaultAbsorptionTimes.medium,
            defaultAbsorptionTime: defaultAbsorptionTimes.medium,
            delay: TimeInterval(minutes: 0),
    ...
    func testDynamicAbsorptionNeverFullyObserved() {
        ...
        let statuses = [carbEntries[1]].map(
            ...
            insulinSensitivity: insulinSensitivities.quantitiesBetween(
                start: carbEntries[1].startDate,
                end: carbEntries[1].startDate
            ),
            absorptionTimeOverrun: defaultAbsorptionTimes.slow / defaultAbsorptionTimes.medium,
            defaultAbsorptionTime: defaultAbsorptionTimes.medium,
            delay: TimeInterval(minutes: 0),
    ...
    func testDynamicAbsorptionMultipleEntries() {
        ...
        let carbTimes = carbEntries.map(\.startDate)
