...
class SimpleGlucoseValueTests: XCTestCase {
    ...
    func testInitializerGlucoseValue() {
        ...
        let endDate = Date().addingTimeInterval(.hours(1))
        let quantity = HKQuantity(unit: .milligramsPerDeciliter, doubleValue: 234.5)
        let simpleGlucoseValue =
            SimpleGlucoseValue(SimpleGlucoseValue(startDate: startDate, endDate: endDate, quantity: quantity))
        XCTAssertEqual(simpleGlucoseValue.startDate, startDate)
        XCTAssertEqual(simpleGlucoseValue.endDate, endDate)
        XCTAssertEqual(simpleGlucoseValue.quantity, quantity)
    }
...
class SimpleGlucoseValueCodableTests: XCTestCase {
    func testCodableMilligramsPerDeciliter() throws {
        try assertSimpleGlucoseValueCodable(
            SimpleGlucoseValue(
                startDate: dateFormatter.date(from: "2020-05-14T22:00:03Z")!,
    ...
    func testCodableMillimolesPerLiter() throws {
        try assertSimpleGlucoseValueCodable(
            SimpleGlucoseValue(
                startDate: dateFormatter.date(from: "2020-05-14T14:05:03Z")!,
...
class PredictedGlucoseValueCodableTests: XCTestCase {
    func testCodableMilligramsPerDeciliter() throws {
        try assertPredictedGlucoseValueCodable(
            PredictedGlucoseValue(
                startDate: dateFormatter.date(from: "2020-05-14T22:38:26Z")!,
                quantity: HKQuantity(
                    unit: .milligramsPerDeciliter,
                    doubleValue: 234.5
                )
    ...
    func testCodableMillimolesPerLiter() throws {
        try assertPredictedGlucoseValueCodable(
            PredictedGlucoseValue(
                startDate: dateFormatter.date(from: "2020-05-14T21:23:33Z")!,
                quantity: HKQuantity(
                    unit: .millimolesPerLiter,
                    doubleValue: 12.3
                )
