...
extension TimeZone {
    static var fixtureTimeZone: TimeZone {
        TimeZone(secondsFromGMT: 25200)! // -0700
    }
...
class TemporaryScheduleOverrideTests: XCTestCase {
    ...
    private func date(at time: String) -> Date {
        dateFormatter.date(from: "2019-01-01T\(time):00")!
    }
    ...
    private func applyingActiveBasalOverride(
        from start: String,
        to end: String,
        on schedule: BasalRateSchedule,
    ...
    func testOutdatedOverride() {
        let overridden = applyingActiveBasalOverride(
            from: "02:00",
            to: "04:00",
            on: basalRateSchedule,
    ...
    func testDurationIsInfinite() {
        let tempOverride = TemporaryScheduleOverride(
            context: .legacyWorkout,
            settings: .init(
                unit: .milligramsPerDeciliter,
                targetRange: DoubleRange(minValue: 120, maxValue: 150)
            ),
            startDate: Date(),
            duration: .indefinite,
            enactTrigger: .local,
    ...
    func testOverrideScheduleAnnotatingReservoirSplitsDose() {
        ...
        XCTAssertEqual(dose.programmedUnits, annotated.map(\.unitsInDeliverableIncrements).reduce(0, +))
    }
    ...
    func testActiveTargetRangeOverride() {
        ...
        let rangeSchedule = GlucoseRangeSchedule(
            unit: .milligramsPerDeciliter,
            dailyItems: [RepeatingScheduleValue(startTime: 0, value: normalRange)]
        )!.applyingOverride(override)
    ...
    func testFutureTargetRangeOverride() {
        ...
        let overrideDuration = TimeInterval(hours: 4)
        let settings = TemporaryScheduleOverrideSettings(unit: .milligramsPerDeciliter, targetRange: overrideRange)
        let futureOverride = TemporaryScheduleOverride(
            context: .custom,
            settings: settings,
            startDate: overrideStart,
            duration: .finite(overrideDuration),
        ...
        let rangeSchedule = GlucoseRangeSchedule(
            unit: .milligramsPerDeciliter,
            dailyItems: [RepeatingScheduleValue(startTime: 0, value: normalRange)]
        )!.applyingOverride(futureOverride)
...
class TemporaryScheduleOverrideContextCodableTests: XCTestCase {
    ...
    func testCodablePreset() throws {
        let preset = TemporaryScheduleOverridePreset(
            id: UUID(uuidString: "238E41EA-9576-4981-A1A4-51E10228584F")!,
            symbol: "🚀",
            name: "Rocket",
            settings: TemporaryScheduleOverrideSettings(
                unit: .milligramsPerDeciliter,
    ...
    private func assertTemporaryScheduleOverrideContextCodable(
        _ original: TemporaryScheduleOverride.Context,
...
class TemporaryScheduleOverrideEnactTriggerCodableTests: XCTestCase {
    func testCodableLocal() throws {
        try assertTemporaryScheduleOverrideEnactTriggerCodable(
            .local,
            encodesJSON: """
            {
              "enactTrigger" : "local"
    ...
    private func assertTemporaryScheduleOverrideEnactTriggerCodable(
        _ original: TemporaryScheduleOverride.EnactTrigger,
        encodesJSON string: String
    ) throws {
...
class TemporaryScheduleOverrideDurationCodableTests: XCTestCase {
    func testCodableFinite() throws {
        try assertTemporaryScheduleOverrideDurationCodable(
            .finite(.hours(2.5)),
            encodesJSON: """
            {
              "duration" : {
    ...
    private func assertTemporaryScheduleOverrideDurationCodable(
        _ original: TemporaryScheduleOverride.Duration,
        encodesJSON string: String
    ) throws {
