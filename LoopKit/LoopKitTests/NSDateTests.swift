...
class NSDateTests: XCTestCase {
    func testDateCeiledToInterval() {
        ...
        let five01 = calendar.nextDate(
            after: Date(),
            matching: DateComponents(hour: 5, minute: 0, second: 1),
            matchingPolicy: .nextTime
        )!

        let five05 = calendar.nextDate(
            after: five01,
            matching: DateComponents(hour: 5, minute: 5, second: 0),
            matchingPolicy: .nextTime
        )!

        XCTAssertEqual(five05, five01.dateCeiledToTimeInterval(TimeInterval(minutes: 5)))

        let six = calendar.nextDate(
            after: five01,
        ...
        let five47 = calendar.nextDate(
            after: five01,
            matching: DateComponents(hour: 5, minute: 47, second: 58),
            matchingPolicy: .nextTime
        )!

        let five50 = calendar.nextDate(
            after: five01,
            matching: DateComponents(hour: 5, minute: 50, second: 0),
            matchingPolicy: .nextTime
        )!

        XCTAssertEqual(five50, five47.dateCeiledToTimeInterval(TimeInterval(minutes: 5)))

        let twentyThree59 = calendar.nextDate(
            after: five01,
            matching: DateComponents(hour: 23, minute: 59, second: 0),
            matchingPolicy: .nextTime
        )!

        let tomorrowMidnight = calendar.nextDate(
            after: five01,
    ...
    func testDateFlooredToInterval() {
        ...
        let five = calendar.nextDate(
            after: five01,
            matching: DateComponents(hour: 5, minute: 0, second: 0),
            matchingPolicy: .nextTime,
            direction: .backward
        )!
        ...
        let five59 = calendar.nextDate(
            after: five01,
            matching: DateComponents(hour: 5, minute: 59, second: 0),
            matchingPolicy: .nextTime
        )!

        XCTAssertEqual(five, five59.dateFlooredToTimeInterval(TimeInterval(minutes: 60)))

        let five55 = calendar.nextDate(
            after: five01,
            matching: DateComponents(hour: 5, minute: 55, second: 0),
            matchingPolicy: .nextTime
        )!
