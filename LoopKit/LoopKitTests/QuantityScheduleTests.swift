...
class QuantityScheduleTests: XCTestCase {
    ...
    func testCarbRatioScheduleLocalTimeZone() {
        ...
        let midMorning = calendar.nextDate(
            after: Date(),
            matching: DateComponents(hour: 10, minute: 29, second: 4),
            matchingPolicy: .nextTime
        )!

        XCTAssertEqual(10, schedule.quantity(at: midMorning).doubleValue(for: schedule.unit))

        let lunch = calendar.nextDate(
            after: midMorning,
            matching: DateComponents(hour: 12, minute: 01, second: 01),
            matchingPolicy: .nextTime
        )!

        XCTAssertEqual(9, schedule.quantity(at: lunch).doubleValue(for: schedule.unit))

        let dinner = calendar.nextDate(
            after: midMorning,
