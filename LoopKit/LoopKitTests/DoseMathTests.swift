...
class RecommendTempBasalTests: XCTestCase {
    ...
    var basalRateSchedule: BasalRateSchedule {
        loadBasalRateScheduleFixture("read_selected_basal_profile")
    }

    var glucoseTargetRange: GlucoseRangeSchedule {
        GlucoseRangeSchedule(
            unit: HKUnit.milligramsPerDeciliter,
            dailyItems: [RepeatingScheduleValue(startTime: TimeInterval(0), value: DoubleRange(minValue: 90, maxValue: 120))]
        )!
    }
    ...
    var insulinSensitivitySchedule: InsulinSensitivitySchedule {
        InsulinSensitivitySchedule(
            unit: HKUnit.milligramsPerDeciliter,
            dailyItems: [RepeatingScheduleValue(startTime: 0.0, value: 60.0)]
        )!
    }

    var suspendThreshold: GlucoseThreshold {
        GlucoseThreshold(unit: HKUnit.milligramsPerDeciliter, value: 55)
    }

    var exponentialInsulinModel: InsulinModel = ExponentialInsulinModel(
        actionDuration: 21600.0,
        peakActivityTime: 4500.0,
        delay: 0
    )

    var walshInsulinModel: InsulinModel {
        WalshInsulinModel(actionDuration: insulinActionDuration)
    }

    var insulinActionDuration: TimeInterval {
        TimeInterval(hours: 4)
    }
    ...
    func testHighAndRising() {
        ...
        let insulinSensitivitySchedule = InsulinSensitivitySchedule(
            ...
            dailyItems: [RepeatingScheduleValue(startTime: 0.0, value: 3.33)]
        )!
...
class RecommendBolusTests: XCTestCase {
    ...
    func testStartLowEndHigh() {
        ...
        if case let BolusRecommendationNotice.currentGlucoseBelowTarget(glucose) = dose.notice! {
    ...
    func testStartBelowSuspendThresholdEndHigh() {
        ...
        if case let BolusRecommendationNotice.glucoseBelowSuspendThreshold(glucose) = dose.notice! {
            XCTAssertEqual(glucose.quantity.doubleValue(for: .milligramsPerDeciliter), 60)
        } else {
            XCTFail("Expected currentGlucoseBelowTarget, but got \(dose.notice!)")
        }
    ...
    func testHighAndRising() {
        ...
        let insulinSensitivitySchedule = InsulinSensitivitySchedule(
            unit: HKUnit.millimolesPerLiter,
            dailyItems: [RepeatingScheduleValue(startTime: 0.0, value: 10.0 / 3)]
        )!
