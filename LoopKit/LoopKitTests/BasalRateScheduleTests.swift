...
func == <T: Equatable>(lhs: AbsoluteScheduleValue<T>, rhs: AbsoluteScheduleValue<T>) -> Bool {
    lhs.startDate == rhs.startDate && lhs.endDate == rhs.endDate && lhs.value == rhs.value
}
...
class BasalRateScheduleTests: XCTestCase {
    ...
    override func setUp() {
        ...
        let fixture = try! JSONSerialization
            .jsonObject(with: Data(contentsOf: URL(fileURLWithPath: path)), options: []) as! [JSONDictionary]
