...
func product<S: Sequence, C: Collection>(
    _ s: S,
    _ c: C
) -> LazySequence<FlattenSequence<LazyMapSequence<S, LazyMapSequence<C, (S.Element, C.Element)>>>> {
    s.lazy.flatMap { first in
...
extension RandomAccessCollection {
    ...
    func allPairs()
        -> LazyMapSequence<
            LazyFilterSequence<FlattenSequence<LazyMapSequence<Indices, LazyMapSequence<Indices, (Index, Index)>>>>,
            (Element, Element)
        >
    {
        product(indices, indices).filter(<).map {
            (self[$0], self[$1])
        }
    }
}
