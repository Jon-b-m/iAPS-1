...
extension ChartPoint {
    static func barsForGlucoseRangeSchedule(
        _ glucoseRangeSchedule: GlucoseRangeSchedule,
        unit: HKUnit,
        xAxisValues: [ChartAxisValue],
        considering potentialOverride: TemporaryScheduleOverride? = nil
    ) -> [TargetChartBar] {
        ...
        for (index, range) in targetRanges.enumerated() {
               ...
               (override.startDate ... override.scheduledEndDate).overlaps(startDate.date ... endDate.date)
            {
                result.append(createBar(
                    ...
                    unit: unit,
                    startDate: startDate,
                    endDate: ChartAxisValueDate(date: override.startDate, formatter: dateFormatter),
                    isOverride: false
                ))
                ...
                result.append(createBar(
                    value: range.value,
                    unit: unit,
                    startDate: ChartAxisValueDate(date: override.scheduledEndDate, formatter: dateFormatter),
                    endDate: endDate,
                    isOverride: false
                ))
            } else {
                result
                    .append(createBar(value: range.value, unit: unit, startDate: startDate, endDate: endDate, isOverride: false))
            }
        }
    ...
    fileprivate static func createBar(
        value: ClosedRange<HKQuantity>,
        unit: HKUnit,
        startDate: ChartAxisValueDate,
        endDate: ChartAxisValueDate,
        isOverride: Bool
    ) -> TargetChartBar? {
    ...
    static func pointsForGlucoseRangeScheduleOverride(
        _ override: TemporaryScheduleOverride,
        unit: HKUnit,
        xAxisValues: [ChartAxisValue],
        extendEndDateToChart: Bool = false
    ) -> [ChartPoint] {
    ...
    private static func pointsForGlucoseRangeScheduleOverride(
        range: DoubleRange,
        activeInterval: DateInterval,
        unit _: HKUnit,
        xAxisValues: [ChartAxisValue],
        extendEndDateToChart: Bool
    ) -> [ChartPoint] {
...
private extension ClosedRange where Bound == HKQuantity {
    func doubleRangeWithMinimumIncrement(in unit: HKUnit) -> DoubleRange {
        ...
        var minValue = lowerBound.doubleValue(for: unit)
        var maxValue = upperBound.doubleValue(for: unit)
