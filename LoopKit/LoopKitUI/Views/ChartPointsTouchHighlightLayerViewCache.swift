    ...
    init(
        ...
        highlightLayer = ChartPointsTouchHighlightLayer(
            ...
            modelFilter: { (screenLoc, chartPointModels) -> ChartPointLayerModel<ChartPoint>? in
                if let index = chartPointModels.map(\.screenLoc.x).findClosestElementIndex(matching: screenLoc.x) {
                    return chartPointModels[index]
                } else {
                    return nil
                }
            },
            viewGenerator: { [weak self] (chartPointModel, _, chart) -> UIView? in
                ...
                if let text = chartPointModel.chartPoint.y.labels.first?.text {
                    ...
                    label.frame.origin.x = min(
                        max(label.frame.origin.x, containerView.bounds.minX),
                        containerView.bounds.maxX - label.frame.size.width
                    )
