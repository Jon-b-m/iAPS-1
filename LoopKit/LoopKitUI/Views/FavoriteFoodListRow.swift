...
public struct FavoriteFoodListRow: View {
    ...
    let onTap: (StoredFavoriteFood) -> Void
    let onDelete: (StoredFavoriteFood) -> Void
    ...
    public init(
        food: StoredFavoriteFood,
        foodToConfirmDeleteId: Binding<String?>,
        onFoodTap: @escaping (StoredFavoriteFood) -> Void,
        onFoodDelete: @escaping (StoredFavoriteFood) -> Void,
        carbFormatter: QuantityFormatter,
        absorptionTimeFormatter: DateComponentsFormatter,
        preferredCarbUnit: HKUnit = .gram()
    ) {
        self.food = food
        _foodToConfirmDeleteId = foodToConfirmDeleteId
        onTap = onFoodTap
...
extension FavoriteFoodListRow {
    private var foodCardContent: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Text(
                "\(food.carbsString(formatter: carbFormatter)) carbs, \(food.absorptionTimeString(formatter: absorptionTimeFormatter)) absorption"
            )
            .font(.footnote)
        }
