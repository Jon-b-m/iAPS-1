...
class SetConstrainedScheduleEntryTableViewCell: UITableViewCell {
    ...
    @IBOutlet private var pickerHeightConstraint: NSLayoutConstraint!
    ...
    private func startTimeForTimeComponent(row: Int) -> TimeInterval {
        minimumStartTime + minimumTimeInterval * TimeInterval(row)
    }
    ...
    func updateValuePicker() {
        ...
        if let value = value {
            if let row = allowedValues.firstIndex(of: value) {
                ...
                selectedIndex = (
                    allowedValues.enumerated().filter({ $0.element >= value }).min(by: { $0.1 < $1.1 })?
                        .offset ?? 0
                ) + emptySelectionType.rowOffset
            }
        } else {
...
extension SetConstrainedScheduleEntryTableViewCellDelegate where Self: UITableViewController {
    func hideSetConstrainedScheduleEntryCells(excluding indexPath: IndexPath? = nil) {
        for case let cell as SetConstrainedScheduleEntryTableViewCell in tableView.visibleCells
            where tableView.indexPath(for: cell) != indexPath && cell.isPickerHidden == false
        {
