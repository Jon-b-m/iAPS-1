...
class GlucoseRangeTableViewCell: UITableViewCell {
    ...
    @IBOutlet open var picker: UIPickerView!
    @IBOutlet open var pickerHeightConstraint: NSLayoutConstraint!
    ...
    private func startTimeForTimeComponent(row: Int) -> TimeInterval {
        allowedTimeRange.lowerBound + minimumTimeInterval * TimeInterval(row)
    }
...
extension GlucoseRangeTableViewCell: UIPickerViewDelegate {
    ...
    func pickerView(_: UIPickerView, widthForComponent component: Int) -> CGFloat {
        [33, 16, 4, 16, 24][component] / 100.0 * picker.frame.width
    }
...
extension GlucoseRangeTableViewCellDelegate where Self: UITableViewController {
    func hideGlucoseRangeCells(excluding indexPath: IndexPath? = nil) {
        for case let cell as GlucoseRangeTableViewCell in tableView.visibleCells
