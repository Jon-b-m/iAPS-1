...
public struct TimePicker: View {
    ...
    public init(
        ...
        _offsetFromMidnight = offsetFromMidnight
        allValues = Swift.stride(from: bounds.lowerBound, through: bounds.upperBound, by: stride)
            .filter { !isTimeExcluded($0) }
    }

    public var body: some View {
        Picker(
            selection: $offsetFromMidnight,
            label: Text(LocalizedString("Time", comment: "Label for offset from midnight picker"))
        ) {
