...
public struct CorrectionRangeScheduleEditor: View {
    ...
    public init(
        ...
        if mode == .acceptanceFlow {
            _scheduleItems = State(
                initialValue: therapySettingsViewModel.glucoseTargetRangeSchedule?
                    .safeSchedule(with: therapySettingsViewModel.suspendThreshold?.quantity)?.quantityRanges ?? []
            )
        } else {
            _scheduleItems = State(initialValue: therapySettingsViewModel.glucoseTargetRangeSchedule?.quantityRanges ?? [])
        }
        viewModel = CorrectionRangeScheduleEditorViewModel(
            mode: mode,
            therapySettingsViewModel: therapySettingsViewModel,
            didSave: didSave
        )
