...
public struct TherapySettingsView: View {
    ...
    private var therapySettingsTitle: String {
        LocalizedString("Therapy Settings", comment: "Therapy Settings screen title")
    }
    ...
    @ViewBuilder private var cardListTrailer: some View {
        if mode == .acceptanceFlow {
    ...
    private var maxBasalRateItem: some View {
        HStack {
            ...
            if let basalRates = self.viewModel.pumpSupportedIncrements()?.basalRates {
                GuardrailConstrainedQuantityView(
                    value: self.viewModel.therapySettings.maximumBasalRatePerHour
    ...
    private var maxBolusItem: some View {
        HStack {
            ...
            if let maximumBolusVolumes = self.viewModel.pumpSupportedIncrements()?.maximumBolusVolumes {
                GuardrailConstrainedQuantityView(
                    value: self.viewModel.therapySettings.maximumBolus
                        .map { HKQuantity(unit: .internationalUnit(), doubleValue: $0) },
...
extension TherapySettingsView {
    @ViewBuilder func screen(for setting: TherapySetting, dismiss: @escaping () -> Void) -> some View {
        switch setting {
        ...
        case .insulinModel:
            InsulinModelSelection(
                mode: mode,
                therapySettingsViewModel: viewModel,
...
extension TherapySettingsView {
    ...
    private func card<Content>(for therapySetting: TherapySetting, @ViewBuilder content: @escaping () -> Content) -> Card
        where Content: View
    {
        Card {
        ...
        ScheduleItemView(
            ...
            valueContent: {
                GuardrailConstrainedQuantityRangeView(
                    range: range.quantityRange(for: unit),
                    unit: unit,
                    guardrail: guardrail,
                    isEditing: false
                )
...
struct ScheduleValueItem: View {
    ...
    public var body: some View {
        ScheduleItemView(
            ...
            valueContent: {
                GuardrailConstrainedQuantityView(
                    value: HKQuantity(unit: unit, doubleValue: value),
                    unit: unit,
                    guardrail: guardrail,
                    isEditing: false
                )
...
public struct TherapySettingsView_Previews: PreviewProvider {
    ...
    static let preview_therapySettings = TherapySettings(
        ...
        insulinSensitivitySchedule: InsulinSensitivitySchedule(
            unit: HKUnit.milligramsPerDeciliter.unitDivided(by: HKUnit.internationalUnit()),
            dailyItems: []
        ),
        carbRatioSchedule: nil,
        basalRateSchedule: BasalRateSchedule(dailyItems: [
            RepeatingScheduleValue(startTime: 0, value: 0.2),
            RepeatingScheduleValue(startTime: 1800, value: 0.75)
        ])
    )
    ...
    static func preview_viewModel() -> TherapySettingsViewModel {
        TherapySettingsViewModel(
            ...
            pumpSupportedIncrements: {
                PumpSupportedIncrements(
                    basalRates: preview_supportedBasalRates,
