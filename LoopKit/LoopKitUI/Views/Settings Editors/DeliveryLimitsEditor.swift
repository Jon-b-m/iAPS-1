...
public typealias SyncDeliveryLimits = (
    _ deliveryLimits: DeliveryLimits,
    _ completion: @escaping (Swift.Result<DeliveryLimits, Error>) -> Void
) -> Void
...
public struct DeliveryLimitsEditor: View {
    ...
    public init(
        ...
        selectableMaximumBasalRates = Guardrail.selectableMaxBasalRates(
        ...
        selectableMaximumBolusVolumes = Guardrail.selectableBolusVolumes(supportedBolusVolumes: supportedMaximumBolusVolumes)
    ...
    @ViewBuilder private var leadingNavigationBarItem: some View {
    ...
    var maximumBasalRateCard: Card {
        Card {
            SettingDescription(
                text: Text(DeliveryLimits.Setting.maximumBasalRate.localizedDescriptiveText(appName: appName)),
            ...
            ExpandableSetting(
                ...
                expandedContent: {
                    FractionalQuantityPicker(
                        value: Binding(
                            get: {
                                self.value.maximumBasalRate ?? self.maximumBasalRateGuardrail.startingSuggestion ?? self
                                    .maximumBasalRateGuardrail.recommendedBounds.upperBound },
                            set: { newValue in
                                withAnimation {
    ...
    var maximumBolusGuardrail: Guardrail<HKQuantity> {
        Guardrail.maximumBolus(supportedBolusVolumes: supportedMaximumBolusVolumes)
    }

    var maximumBolusCard: Card {
        Card {
            SettingDescription(
                text: Text(DeliveryLimits.Setting.maximumBolus.localizedDescriptiveText(appName: appName)),
                informationalContent: { TherapySetting.deliveryLimits.helpScreen() }
            )
            ExpandableSetting(
                isEditing: Binding(
                ...
                expandedContent: {
                    FractionalQuantityPicker(
                        value: Binding(
                            get: {
                                self.value.maximumBolus ?? self.maximumBolusGuardrail.startingSuggestion ?? self
                                    .maximumBolusGuardrail.recommendedBounds.upperBound },
                            set: { newValue in
                                withAnimation {
    ...
    private func startSaving() {
        guard mode == .settings else {
            monitorSavingMechanism(savingMechanism: .synchronous { deliveryLimits in
                save(deliveryLimits)
            })
            return
        }
    ...
    private func synchronizeDeliveryLimits(_ deliveryLimits: DeliveryLimits, _ completion: @escaping (Error?) -> Void) {
        ...
        syncDeliveryLimits(deliveryLimits) { result in
            switch result {
            case let .success(deliveryLimits):
                actuallySave(deliveryLimits)
                completion(nil)
    ...
    private func monitorSavingMechanism(savingMechanism: SavingMechanism<DeliveryLimits>) {
        ...
        case let .synchronous(save):
            save(value)
        case let .asynchronous(save):
            withAnimation {
                self.isSyncing = true
            }
    ...
    private func alert(for presentedAlert: PresentedAlert) -> SwiftUI.Alert {
        ...
        case let .saveConfirmation(content):
    ...
    private var confirmationAlertContent: AlertContent {
        AlertContent(
            ...
            ok: Text(
                LocalizedString("Continue", comment: "Text for continue action on confirmation alert")
            )
        )
    }
}
        ...
        case 1:
            ...
            case .maximumBasalRate:
                ...
                case .belowRecommended,
                     .minimum:
                    title =
                        Text(LocalizedString("Low Maximum Basal Rate", comment: "Title text for low maximum basal rate warning"))
