...
public struct InsulinModelSelection: View {
    ...
    static let defaultInsulinSensitivitySchedule = InsulinSensitivitySchedule(
        unit: .milligramsPerDeciliter,
        dailyItems: [RepeatingScheduleValue<Double>(startTime: 0, value: 40)]
    )!
    ...
    public init(
        ...
        _chartManager = State(initialValue: chartManager)
    }
    ...
    private var content: some View {
        VStack(spacing: 0) {
            CardList(
                title: Text(LocalizedString("Insulin Model", comment: "Title text for insulin model")),
    ...
    private func oneUnitBolusEffectPrediction(using modelPreset: ExponentialInsulinModelPreset) -> [GlucoseValue] {
        ...
        let startingGlucoseSample = HKQuantitySample(
            type: HealthKitSampleStore.glucoseType,
            quantity: startingGlucoseQuantity,
            start: chartManager.startDate,
            end: chartManager.startDate
        )
        let effects = [bolus].glucoseEffects(
            insulinModelProvider: StaticInsulinModelProvider(modelPreset),
    ...
    private var startingGlucoseQuantity: HKQuantity {
        let startingGlucoseValue = insulinSensitivitySchedule.quantity(at: chartManager.startDate)
            .doubleValue(for: displayGlucoseUnit) + displayGlucoseUnit.glucoseExampleTargetValue
