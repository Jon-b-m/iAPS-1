...
public struct FractionalQuantityPicker: View {
    ...
    public init(
        ...
        let (selectableWholeValues, fractionalValuesByWhole): ([Double], [Double: [Double]]) = selectableValues
            .reduce(into: ([], [:])) { pair, selectableValue in
        ...
        _whole = Binding(
            ...
            set: { newWholeValue in
                let newFractionValue = Self.matchingFraction(
                    for: doubleValue.wrappedValue.fraction,
                    from: fractionalValuesByWhole[newWholeValue] ?? [0.0]
                )
        ...
        _fraction = Binding(
            get: {
                doubleValue.wrappedValue.fraction
                    .roundedToNearest(of: fractionalValuesByWhole[doubleValue.wrappedValue.whole] ?? [0.0]) },
    ...
    private static func matchingFraction(
        ...
        currentFraction.matchingOrTruncatedValue(
            from: supportedFractionValues,
            withinDecimalPlaces: Self.maximumSupportedPrecision
        )
    }

    public var body: some View {
        switch usageContext {
        case let .component(availableWidth: availableWidth):
...
private extension Collection where Element == Decimal {
