...
private struct DosePointsCache {
...
public extension DoseChart {
    ...
    ) -> Chart
    {
        ...
        let yAxisValues = ChartAxisValuesStaticGenerator.generateYAxisValuesUsingLinearSegmentStep(
            ...
            multiple: log(2) / 2,
            axisValueGenerator: {
                ChartAxisValueDoubleLog(screenLocDouble: $0, formatter: integerFormatter, labelSettings: axisLabelSettings) },
        ...
        if !points.bolus.isEmpty {
            bolusLayer = ChartPointsScatterDownTrianglesLayer(
                xAxis: xAxisLayer.axis,
                yAxis: yAxisLayer.axis,
                chartPoints: points.bolus,
                displayDelay: 0,
                itemSize: CGSize(width: bolusPointSize, height: bolusPointSize),
                itemFillColor: colors.insulinTint
            )
        } else {
            bolusLayer = nil
        }
        ...
        let zeroGuidelineLayer = ChartPointsViewsLayer(
            xAxis: xAxisLayer.axis,
            yAxis: yAxisLayer.axis,
            chartPoints: [dummyZeroChartPoint],
            viewGenerator: { (chartPointModel, _, chart) -> UIView? in
                let width: CGFloat = 1
                let viewFrame = CGRect(
                    x: chart.contentView.bounds.minX,
                    y: chartPointModel.screenLoc.y - width / 2,
                    width: chart.contentView.bounds.size.width,
                    height: width
                )
    ...
    private func generateDosePoints(startDate: Date) -> DosePointsCache {
        ...
        for entry in doseEntries {
            ...
            if entry.type == .bolus, entry.netBasalUnits > 0 {
                ...
                let y = ChartAxisValueDoubleLog(
                    actualDouble: entry.unitsInDeliverableIncrements,
                    unitString: "U",
        ...
        let pointsCache = DosePointsCache(
            basal: basalPoints,
            basalFill: basalFillPoints,
            bolus: bolusPoints,
