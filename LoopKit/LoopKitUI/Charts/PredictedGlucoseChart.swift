...
public class PredictedGlucoseChart: GlucoseChart, ChartProviding {
    ...
    private var maxYAxisSegmentCount: Double {
        ...
        glucoseValueBelowSoftBoundsMinimum() ? 5 : 4
    }
    ...
    public init(
        predictedGlucoseBounds: PredictedGlucoseBounds? = nil,
        yAxisStepSizeMGDLOverride: Double? = nil
    ) {
        predictedGlucoseSoftBounds = predictedGlucoseBounds
        self.yAxisStepSizeMGDLOverride = yAxisStepSizeMGDLOverride
        super.init()
    }
}

public extension PredictedGlucoseChart {
    ...
    ) -> Chart
    {
        if targetGlucosePoints.isEmpty, xAxisValues.count > 1, let schedule = targetGlucoseSchedule {
            ...
            let potentialOverride = (preMealOverride?.isActive() ?? false) ? preMealOverride :
                (scheduleOverride?.isActive() ?? false) ? scheduleOverride : nil
            targetGlucosePoints = ChartPoint.barsForGlucoseRangeSchedule(
                schedule,
                unit: glucoseUnit,
                xAxisValues: xAxisValues,
                considering: potentialOverride
            )

            var displayedScheduleOverride = scheduleOverride
            if let preMealOverride = preMealOverride, preMealOverride.isActive() {
                preMealOverrideDurationPoints = ChartPoint.pointsForGlucoseRangeScheduleOverride(
                    preMealOverride,
                    unit: glucoseUnit,
                    xAxisValues: xAxisValues
                )
                ...
                if displayedScheduleOverride != nil {
                    if displayedScheduleOverride!.scheduledEndDate > preMealOverride.scheduledEndDate {
                        ...
                        displayedScheduleOverride!.scheduledInterval = DateInterval(
                            start: start,
                            end: displayedScheduleOverride!.scheduledEndDate
                        )
                    } else {
            ...
            if let override = displayedScheduleOverride, override.isActive() || override.startDate > Date() {
                targetOverrideDurationPoints = ChartPoint.pointsForGlucoseRangeScheduleOverride(
                    override,
                    unit: glucoseUnit,
                    xAxisValues: xAxisValues
                )
        ...
        let circles = ChartPointsScatterCirclesLayer(
            xAxis: xAxisLayer.axis,
            yAxis: yAxisLayer.axis,
    ...
    private func determineYAxisValues(axisLabelSettings: ChartLabelSettings? = nil) -> [ChartAxisValue] {
        let points = [
            ...
            targetGlucosePoints.flatMap(\.points),
...
extension PredictedGlucoseChart {
    ...
    func clampPredictedGlucoseValues(_ glucoseValues: [GlucoseValue]) -> [GlucoseValue] {
        ...
        let predictedGlucoseValueMaximum = chartMaximumValue != nil ? max(predictedGlucoseBounds.maximum, chartMaximumValue!) :
            predictedGlucoseBounds.maximum

        let predictedGlucoseValueMinimum = chartMinimumValue != nil ? min(predictedGlucoseBounds.minimum, chartMinimumValue!) :
            predictedGlucoseBounds.minimum
    ...
    func glucoseValueBelowSoftBoundsMinimum() -> Bool {
        guard let predictedGlucoseSoftBounds = predictedGlucoseSoftBounds,
              let chartedGlucoseValueMinimum = chartedGlucoseValueMinimum
        else {
            return false
        }
    ...
    public struct PredictedGlucoseBounds {
        ...
        public static var `default`: PredictedGlucoseBounds {
            PredictedGlucoseBounds(
                minimum: HKQuantity(unit: .milligramsPerDeciliter, doubleValue: 40),
