            ...
            if let range = glucoseDisplayRange {
                glucoseDisplayRangePoints = [
                    ChartPoint(
                        ...
                        y: ChartAxisValueDouble(range.lowerBound.doubleValue(for: glucoseUnit))
                    ),
                    ChartPoint(
                        x: ChartAxisValue(scalar: 0),
                        y: ChartAxisValueDouble(range.upperBound.doubleValue(for: glucoseUnit))
                    )
                ]
            } else {
                glucoseDisplayRangePoints = []
        ...
        return glucoseValues.map {
            return ChartPoint(
                ...
                y: ChartAxisValueDoubleUnit(
                    $0.quantity.doubleValue(for: glucoseUnit),
