...
public extension CarbEffectChart {
    ...
    ) -> Chart
    {
        ...
        let yAxisValues = ChartAxisValuesStaticGenerator.generateYAxisValuesWithChartPoints(
            carbEffectPoints + allCarbEffectPoints + carbEffectDisplayRangePoints,
            minSegmentCount: 2,
            maxSegmentCount: 4,
    ...
    func setInsulinCounteractionEffects(_ effects: [GlucoseEffectVelocity]) {
        ...
        let unitString = String(
            format: NSLocalizedString(
                "%1$@/min",
                comment: "Format string describing glucose units per minute (1: glucose unit string)"
            ),
            glucoseUnit.shortLocalizedUnitString()
        )
        ...
        for effect in effects {
            ...
            let endX = ChartAxisValueDate(date: effect.endDate, formatter: dateFormatter)
            let value = ChartAxisValueDoubleUnit(
                effect.quantity.doubleValue(for: unit),
