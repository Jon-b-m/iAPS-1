...
open class SetupTableViewController: UITableViewController {
    open private(set) lazy var footerView = SetupTableFooterView(frame: .zero)
    ...
    override open func viewDidLayoutSubviews() {
        ...
        if tableView.contentSize.height != lastContentHeight {
            ...
            var footerSize = footerView
                .systemLayoutSizeFitting(CGSize(
                    width: tableView.frame.size.width,
                    height: UIView.layoutFittingCompressedSize.height
                ))
            let visibleHeight = tableView.bounds.size
                .height - (tableView.adjustedContentInset.top + tableView.adjustedContentInset.bottom)
            let footerHeight = padFooterToBottom ? max(footerSize.height, visibleHeight - tableView.contentSize.height) :
                footerSize.height
