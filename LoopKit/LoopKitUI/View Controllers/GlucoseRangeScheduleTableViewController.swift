...
public protocol GlucoseRangeScheduleStorageDelegate {
    func saveSchedule(
        for viewController: GlucoseRangeScheduleTableViewController,
        completion: @escaping (_ result: SaveGlucoseRangeScheduleResult) -> Void
    )
}
...
public class GlucoseRangeScheduleTableViewController: UITableViewController {
    ...
    override public func viewDidLoad() {
        ...
        tableView.register(
            GlucoseRangeOverrideTableViewCell.nib(),
            forCellReuseIdentifier: GlucoseRangeOverrideTableViewCell.className
        )
    ...
    private func updateSaveButton() {
        if let section = sections.firstIndex(of: .save),
           let cell = tableView.cellForRow(at: IndexPath(row: 0, section: section)) as? TextButtonTableViewCell
        {
    ...
    public func setSchedule(
        _ schedule: GlucoseRangeSchedule,
        withOverrideRanges overrides: [TemporaryScheduleOverride.Context: DoubleRange]
    ) {
        unit = schedule.unit
    ...
    @IBAction func addScheduleItem(_: Any?) {
    ...
    private func updateTimeLimits(for index: Int) {
        ...
        if let allowedTimeRange = allowedTimeRange(for: index),
    ...
    var lastValidStartTime: TimeInterval {
        TimeInterval.hours(24) - minimumTimeInterval
    }

    public var timeZone = TimeZone.currentFixed

    private var unit = HKUnit.milligramsPerDeciliter
    ...
    override public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .schedule:
            let cell = tableView.dequeueReusableCell(
                withIdentifier: GlucoseRangeTableViewCell.className,
                for: indexPath
            ) as! GlucoseRangeTableViewCell
        ...
        case .override:
            let cell = tableView.dequeueReusableCell(
                withIdentifier: GlucoseRangeOverrideTableViewCell.className,
                for: indexPath
            ) as! GlucoseRangeOverrideTableViewCell
        ...
        if sourceIndexPath != destinationIndexPath {
            ...
            case .schedule:
                ...
                editableItems[destinationIndexPath.row] = RepeatingScheduleValue(
                    startTime: startTime,
                    value: editableItems[destinationIndexPath.row].value
                )
    ...
    override public func tableView(_: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
        switch sections[indexPath.section] {
        case .schedule:
    ...
    override public func tableView(_ tableView: UITableView, didDeselectRowAt _: IndexPath) {
        tableView.beginUpdates()
        tableView.endUpdates()
    }
