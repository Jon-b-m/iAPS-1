...
public class DismissibleHostingController<Content: View>: UIHostingController<_DismissibleHostingView<Content>> {
    ...
    public convenience init(
        ...
        case .modalDismiss:
            rootView.dismiss = { [weak self] in self?.dismiss(animated: true) }
        case let .pop(to: PredecessorViewController):
            rootView.dismiss = { [weak self] in
                guard
                    let navigationController = self?.navigationController,
                    let predecessor = navigationController.viewControllers
                    .last(where: { $0.isKind(of: PredecessorViewController) })
                else {
                    return
                }
