...
public protocol OverrideSelectionViewControllerDelegate: AnyObject {
    func overrideSelectionViewController(
        _ vc: OverrideSelectionViewController,
        didUpdatePresets presets: [TemporaryScheduleOverridePreset]
    )
    func overrideSelectionViewController(
        _ vc: OverrideSelectionViewController,
        didConfirmPreset preset: TemporaryScheduleOverridePreset
    )
    func overrideSelectionViewController(
        _ vc: OverrideSelectionViewController,
        didConfirmOverride override: TemporaryScheduleOverride
    )
    func overrideSelectionViewController(
        _ vc: OverrideSelectionViewController,
...
public final class OverrideSelectionViewController: UICollectionViewController, IdentifiableClass {
    ...
    private func indexPathOfCustomOverride() -> IndexPath {
        ...
        let row = collectionView(collectionView, numberOfItemsInSection: section) - 2
        return IndexPath(row: row, section: section)
    }

    override public func numberOfSections(in _: UICollectionView) -> Int {
        sections.count
    }

    override public func collectionView(_: UICollectionView, numberOfItemsInSection section: Int) -> Int {
    ...
    override public func collectionView(
        ...
        case UICollectionView.elementKindSectionHeader:
            let header = collectionView.dequeueReusableSupplementaryView(
                ofKind: kind,
                withReuseIdentifier: OverrideSelectionHeaderView.className,
                for: indexPath
            ) as! OverrideSelectionHeaderView
            switch section(for: indexPath.section) {
            case .scheduledOverride:
                header.titleLabel.text = LocalizedString(
                    "SCHEDULED PRESET",
        ...
        case UICollectionView.elementKindSectionFooter:
            let footer = collectionView.dequeueReusableSupplementaryView(
                ofKind: kind,
                withReuseIdentifier: OverrideSelectionFooterView.className,
                for: indexPath
            ) as! OverrideSelectionFooterView
    ...
    override public func collectionView(
        ...
        case let .scheduledOverride(override):
            let cell = collectionView.dequeueReusableCell(
                withReuseIdentifier: OverridePresetCollectionViewCell.className,
                for: indexPath
            ) as! OverridePresetCollectionViewCell
            cell.delegate = self
            ...
            cell.startTimeLabel.text = DateFormatter.localizedString(
                from: override.startDate,
                dateStyle: .none,
                timeStyle: .short
            )
        ...
        case .customOverride:
            let cell = collectionView.dequeueReusableCell(
                withReuseIdentifier: CustomOverrideCollectionViewCell.className,
                for: indexPath
            ) as! CustomOverrideCollectionViewCell
        ...
        case .history:
            let cell = collectionView.dequeueReusableCell(
                withReuseIdentifier: OverrideHistoryCollectionViewCell.className,
                for: indexPath
            ) as! OverrideHistoryCollectionViewCell
    ...
    private func configure(
        _ cell: OverridePresetCollectionViewCell,
        with settings: TemporaryScheduleOverrideSettings,
    ...
    private func makeTargetRangeText(from targetRange: ClosedRange<HKQuantity>) -> String {
        ...
        return String(
            format: LocalizedString(
                "%1$@ – %2$@ %3$@",
                comment: "The format for a glucose target range. (1: min target)(2: max target)(3: glucose unit)"
            ),
            minTarget,
            maxTarget,
            quantityFormatter.localizedUnitStringWithPlurality()
        )
    }
    ...
    @objc private func beginEditing() {
        ...
        if let scheduledOverrideSection = sections.firstIndex(of: .scheduledOverride) {
            ...
            guard let scheduledOverrideCell = collectionView
                .cellForItem(at: scheduledOverrideIndexPath) as? OverridePresetCollectionViewCell
            else {
                return
            }
        ...
        if let customOverrideCell = collectionView
            .cellForItem(at: indexPathOfCustomOverride()) as? CustomOverrideCollectionViewCell
        {
    ...
    override public func collectionView(_: UICollectionView, shouldHighlightItemAt indexPath: IndexPath) -> Bool {
        if !isEditingPresets {
            return true
        }
    ...
    override public func collectionView(_: UICollectionView, canMoveItemAt indexPath: IndexPath) -> Bool {
        isEditingPresets
...
extension OverrideSelectionViewController: AddEditOverrideTableViewControllerDelegate {
    public func addEditOverrideTableViewController(
...
extension OverrideSelectionViewController: OverridePresetCollectionViewCellDelegate {
    func overridePresetCollectionViewCellDidScheduleOverride(_ cell: OverridePresetCollectionViewCell) {
        guard
            ...
            case let .preset(preset) = cellContent(for: indexPath)
        else {
            return
        }
