...
public extension DoseEntry {
    ...
    private var hours: Double {
        endDate.timeIntervalSince(startDate).hours
    }
    ...
    var unitsInDeliverableIncrements: Double {
        guard case .unitsPerHour = unit else {
            return deliveredUnits ?? programmedUnits
        }

        return deliveredUnits ?? round(programmedUnits * DoseEntry.minimumMinimedIncrementPerUnit) / DoseEntry
            .minimumMinimedIncrementPerUnit
    }
}
...
extension DoseEntry: Codable {
    public init(from decoder: Decoder) throws {
        ...
        type = try container.decode(DoseType.self, forKey: .type)
        startDate = try container.decode(Date.self, forKey: .startDate)
        endDate = try container.decode(Date.self, forKey: .endDate)
        value = try container.decode(Double.self, forKey: .value)
        unit = try container.decode(DoseUnit.self, forKey: .unit)
        deliveredUnits = try container.decodeIfPresent(Double.self, forKey: .deliveredUnits)
        ...
        isMutable = try container.decodeIfPresent(Bool.self, forKey: .isMutable) ?? false
        wasProgrammedByPumpUI = try container.decodeIfPresent(Bool.self, forKey: .wasProgrammedByPumpUI) ?? false
    }
...
extension DoseEntry: RawRepresentable {
    ...
    public init?(rawValue: [String: Any]) {
        ...
        insulinType = (rawValue["insulinType"] as? InsulinType.RawValue).flatMap { InsulinType(rawValue: $0) }
        automatic = rawValue["automatic"] as? Bool
        syncIdentifier = rawValue["syncIdentifier"] as? String
        scheduledBasalRate = (rawValue["scheduledBasalRate"] as? Double)
            .flatMap { HKQuantity(unit: .internationalUnitsPerHour, doubleValue: $0) }
        isMutable = rawValue["isMutable"] as? Bool ?? false
        wasProgrammedByPumpUI = rawValue["wasProgrammedByPumpUI"] as? Bool ?? false
    }
