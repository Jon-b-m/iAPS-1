...
public final class DoseStore {
    ...
    public var insulinModelProvider: InsulinModelProvider {
        get {
            lockedInsulinModelProvider.value
        }
        set {
    ...
    public var insulinSensitivitySchedule: InsulinSensitivitySchedule? {
        get {
            lockedInsulinSensitivitySchedule.value
        }
        set {
    ...
    public var hkSampleStore: HealthKitSampleStore? {
        insulinDeliveryStore.hkSampleStore
    }
    ...
    public init(
        ...
        lockedInsulinSensitivitySchedule = Locked(insulinSensitivitySchedule)
        lockedInsulinModelProvider = Locked(insulinModelProvider)
        self.longestEffectDuration = longestEffectDuration
        lockedBasalProfile = Locked(basalProfile)
        self.overrideHistory = overrideHistory
        persistenceController = cacheStore
        self.cacheLength = cacheLength
        self.syncVersion = syncVersion
        lockedLastPumpEventsReconciliation = Locked(lastPumpEventsReconciliation)

        pumpEventQueryAfterDate = cacheStartDate
    ...
    private var purgeableValuesPredicate: NSPredicate {
        NSPredicate(format: "date < %@", cacheStartDate as NSDate)
    }
    ...
    public var cacheStartDate: Date {
        currentDate(timeIntervalSinceNow: -cacheLength)
    }
    ...
    internal func currentDate(timeIntervalSinceNow: TimeInterval = 0) -> Date {
        insulinDeliveryStore.currentDate(timeIntervalSinceNow: timeIntervalSinceNow)
    }
    ...
    private var lastStoredReservoirValue: StoredReservoirValue? {
        get {
            lockedLastStoredReservoirValue.value
        }
        set {
    ...
    public private(set) var pumpEventQueryAfterDate: Date {
        get {
            lockedPumpEventQueryAfterDate.value
        }
        set {
    ...
    public private(set) var lastPumpEventsReconciliation: Date? {
        get {
            lockedLastPumpEventsReconciliation.value
        }
        set {
    ...
    public var lastAddedPumpData: Date {
        [lastReservoirValue?.startDate, lastPumpEventsReconciliation].compactMap { $0 }.max() ?? .distantPast
    }
    ...
    private var lastRecordedPrimeEventDate: Date? {
        get {
            if _lastRecordedPrimeEventDate == nil {
                if let pumpEvents = try? getPumpEventObjects(
...
extension DoseStore {
    ...
    @discardableResult private func validateReservoirContinuity(at date: Date? = nil) -> [Reservoir] {
        ...
        if let recentReservoirObjects = try? getReservoirObjects(since: continuityStartDate - maximumInterval),
           let oldestRelevantReservoirObject = recentReservoirObjects.last
        {
            ...
            let primeEventExistsWithinInsulinActionDuration = (lastRecordedPrimeEventDate ?? .distantPast) >=
                oldestRelevantReservoirObject.startDate

            areReservoirValuesValid = areReservoirValuesContinuous && !primeEventExistsWithinInsulinActionDuration
            lastStoredReservoirValue = recentReservoirObjects.first?.storedReservoirValue
     ...
     */
    public func addReservoirValue(
        _ unitVolume: Double,
        at date: Date,
        completion: @escaping (
            _ value: ReservoirValue?,
            _ previousValue: ReservoirValue?,
            _ areStoredValuesContinuous: Bool,
            _ error: DoseStoreError?
        ) -> Void
    ) {
        persistenceController.managedObjectContext.perform {
            ...
            if let previousValue = self.lastReservoirValue {
                ...
                if isOutOfOrder || isConflicting {
                    self.log.error(
                        "Added inconsistent reservoir value of %{public}.3fU at %{public}@ after %{public}.3fU at %{public}@. Resetting.",
                        unitVolume,
                        String(describing: date),
                        previousValue.unitVolume,
                        String(describing: previousValue.endDate)
                    )
                    ...
                    do {
                        ...
                        completion(
                            ...
                            nil,
                            false,
                            DoseStoreError(error: error as? PersistenceController.PersistenceControllerError)
                        )
                        return
                    }
            ...
            if let basalProfile = self.basalProfileApplyingOverrideHistory {
                ...
                if self.recentReservoirNormalizedDoseEntriesCache != nil {
                    self.recentReservoirNormalizedDoseEntriesCache = self.recentReservoirNormalizedDoseEntriesCache!
                        .filterDateRange(
                            self.recentStartDate,
                            nil
                        )
    ...
    public func getReservoirValues(
        since startDate: Date,
        limit: Int? = nil,
        completion: @escaping (_ result: DoseStoreResult<[ReservoirValue]>) -> Void
    ) {
        persistenceController.managedObjectContext.perform {
            do {
                let values = try self.getReservoirObjects(since: startDate, limit: limit).map(\.storedReservoirValue)
    ...
    private func getReservoirObjects(since startDate: Date, limit: Int? = nil) throws -> [Reservoir] {
        ...
        do {
            ...
            throw DoseStoreError.fetchError(
                description: fetchError.localizedDescription,
                recoverySuggestion: fetchError.localizedRecoverySuggestion
            )
        }
    }
    ...
    private func getNormalizedReservoirDoseEntries(start: Date, end: Date? = nil) throws -> [DoseEntry] {
        if let normalizedDoses = recentReservoirNormalizedDoseEntriesCache, let firstDoseDate = normalizedDoses.first?.startDate,
           firstDoseDate <= start
        {
            return normalizedDoses.filterDateRange(start, end)
        } else {
            guard let basalProfile = basalProfileApplyingOverrideHistory else {
            ...
            let doses = try getReservoirObjects(since: start.addingTimeInterval(-.minutes(10))).reversed().doseEntries

            let normalizedDoses = doses.annotated(with: basalProfile)
            recentReservoirNormalizedDoseEntriesCache = normalizedDoses
            return normalizedDoses.filterDateRange(start, end)
        }
    }
     ...
     */
    public func deleteReservoirValue(
        _ value: ReservoirValue,
        completion: @escaping (_ deletedValues: [ReservoirValue], _ error: DoseStoreError?) -> Void
    ) {
        persistenceController.managedObjectContext.perform {
            var deletedObjects = [ReservoirValue]()
            if let storedValue = value as? StoredReservoirValue,
               let objectID = self.persistenceController.managedObjectContext.persistentStoreCoordinator?
               .managedObjectID(forURIRepresentation: storedValue.objectIDURL),
        ...
        do {
               ...
               !objectIDs.isEmpty
            {
                ...
                NSManagedObjectContext.mergeChanges(
                    ...
                    into: [persistenceController.managedObjectContext]
                )
...
public extension DoseStore {
    ...
    func addPumpEvents(
        ...
        replacePendingEvents: Bool = true,
        completion: @escaping (_ error: DoseStoreError?) -> Void
    ) {
        ...
        for event in events {
            if let dose = event.dose {
                log.debug("Add %@, isMutable=%@", String(describing: dose), String(describing: event.dose?.isMutable))
            }
        }
    ...
    func deletePumpEvent(_ event: PersistedPumpEvent, completion: @escaping (_ error: DoseStoreError?) -> Void) {
        persistenceController.managedObjectContext.perform {
            if let objectID = self.persistenceController.managedObjectContext.persistentStoreCoordinator?
                .managedObjectID(forURIRepresentation: event.objectIDURL),
                let object = try? self.persistenceController.managedObjectContext.existingObject(with: objectID)
            {
                self.persistenceController.managedObjectContext.delete(object)
            }
    ...
    /// - Parameter error: An error explaining why the deletion failed
    func deleteAllPumpEvents(_ completion: @escaping (_ error: DoseStoreError?) -> Void) {
        syncPumpEventsToInsulinDeliveryStore { error in
            if let error = error {
                self.log.error(
                    "Error performing final sync to insulin delivery store before deleteAllPumpEvents: %{public}@",
    ...
    func syncDoseEntries(_ entries: [DoseEntry], updateExistingRecords: Bool = true) async throws {
        try await insulinDeliveryStore.syncDoseEntries(entries, updateExistingRecords: updateExistingRecords)
    }
    ...
    func deleteDose(_ dose: DoseEntry, completion: @escaping (_ error: DoseStoreError?) -> Void) {
        guard let syncIdentifier = dose.syncIdentifier else {
            log.error("Unable to delete PersistedManualEntryDose: no syncIdentifier")
            completion(DoseStoreError.fetchError(
                description: "Unable to delete dose: syncIdentifier is missing",
                recoverySuggestion: "File an issue report in Github"
            ))
            return
        }
    ...
    /// - Parameter completion: A closure called after all the events are deleted. This closure takes a single argument:
    /// - Parameter error: An error explaining why the deletion failed
    func deleteAllManuallyEnteredDoses(since startDate: Date, _ completion: @escaping (_ error: DoseStoreError?) -> Void) {
        log.info("Deleting all manually entered doses since %{public}@", String(describing: startDate))
    ...
    private func syncPumpEventsToInsulinDeliveryStore(
        after start: Date? = nil,
        resolveMutable: Bool = false,
        ...
        insulinDeliveryStore.getLastImmutableBasalEndDate { result in
            ...
            case let .success(date):
                ...
                self.savePumpEventsToInsulinDeliveryStore(
                    after: start ?? date,
                    resolveMutable: resolveMutable,
    ...
    private func savePumpEventsToInsulinDeliveryStore(
        ...
        getPumpEventDoseEntriesForSavingToInsulinDeliveryStore(startingAt: start) { result in
            ...
            case let .success(doses):
                ...
                self.insulinDeliveryStore.addDoseEntries(
                    doses,
                    from: self.device,
    ...
    private func getPumpEventDoseEntriesForSavingToInsulinDeliveryStore(
        startingAt: Date,
        completion: @escaping (_ result: DoseStoreResult<[DoseEntry]>) -> Void
    ) {
        ...
        persistenceController.managedObjectContext.perform {
            let doses: [DoseEntry]
            do {
                doses = try self.getNormalizedPumpEventDoseEntriesForSavingToInsulinDeliveryStore(
                    basalStart: startingAt,
                    end: self.currentDate()
                )
            } catch let error as DoseStoreError {
                self.log.error(
                    "Error while fetching doses to add to insulin delivery store: %{public}@",
                    String(describing: error)
                )
                completion(.failure(error))
                return
            ...
            guard let basalSchedule = self.basalProfileApplyingOverrideHistory else {
                self.log.error(
                    "Can't save %d doses to insulin delivery store because no basal profile is configured",
                    doses.count
                )
            ...
            let reconciledDoses = doses.overlayBasalSchedule(
                basalSchedule,
                startingAt: startingAt,
                insertingBasalEntries: !self.pumpRecordsBasalProfileStartEvents
            )
    ...
    func getManuallyEnteredDoses(since startDate: Date, completion: @escaping (_ result: DoseStoreResult<[DoseEntry]>) -> Void) {
    ...
    func getPumpEventValues(
        since startDate: Date,
        completion: @escaping (_ result: DoseStoreResult<[PersistedPumpEvent]>) -> Void
    ) {
        persistenceController.managedObjectContext.perform {
            do {
                let events = try self.getPumpEventObjects(since: startDate).map(\.persistedPumpEvent)
    ...
    /// - Throws: An error describing the failure to fetch objects
    private func getPumpEventObjects(
    ...
    /// - Returns: An array of doses from pump events
    /// - Throws: An error describing the failure to fetch objects
    private func getNormalizedPumpEventDoseEntriesForSavingToInsulinDeliveryStore(
        basalStart: Date,
        end: Date
    ) throws -> [DoseEntry] {
        log.info(
            "Fetching Pump events between %{public}@ and %{public}@ for saving to InsulinDeliveryStore",
            String(describing: basalStart),
        ...
        let normalizedDoses = doses.reconciled().filter({ $0.endDate <= end || $0.isMutable })
            .filter({ $0.startDate >= basalStart || $0.type == .bolus })

        return normalizedDoses
    }

    func purgePumpEventObjects(before date: Date, completion: (Error?) -> Void) {
        do {
            let count = try purgePumpEventObjects(matching: NSPredicate(format: "date < %@", date as NSDate))
            log.info("Purged %d PumpEvents", count)
            completion(nil)
        } catch {
            log.error("Unable to purge PumpEvents: %{public}@", String(describing: error))
            completion(error)
        }
    }
     ...
     */
    @discardableResult private func purgePumpEventObjects(matching predicate: NSPredicate? = nil) throws -> Int {
...
public extension DoseStore {
    ...
    func getNormalizedDoseEntries(
        ...
        insulinDeliveryStore.getDoseEntries(start: start, end: end, includeMutable: true) { result in
            ...
            case let .success(insulinDeliveryDoses):
                ...
                self.persistenceController.managedObjectContext.perform {
                    do {
                        ...
                        if self.areReservoirValuesValid, let reservoirEndDate = self.lastStoredReservoirValue?.startDate,
                           reservoirEndDate > self.lastPumpEventsReconciliation ?? .distantPast
                        {
                            ...
                            doses = insulinDeliveryDoses
                                .map({ $0.trimmed(to: startOfReservoirData) }) + reservoirDoses + mutableDoses
                                .map({ $0.trimmed(from: endOfReservoirData) })
                        } else {
                            // Deduplicates doses by syncIdentifier
                            doses = insulinDeliveryDoses
                                .appendedUnion(with: try self.getNormalizedPumpEventDoseEntries(start: filteredStart, end: end))
                        }
    ...
    func getLatestBolus() async throws -> DoseEntry? {
        try await insulinDeliveryStore.getBoluses().first
    }
    ...
    func getBoluses(start: Date? = nil, end: Date? = nil) async throws -> [DoseEntry] {
        try await insulinDeliveryStore.getBoluses(start: start, end: end)
    }
    ...
    func getDoses(start: Date? = nil, end: Date? = nil) async throws -> [DoseEntry] {
        try await insulinDeliveryStore.getDoses(start: start, end: end)
    }
    ...
    func insulinOnBoard(at date: Date, completion: @escaping (_ result: DoseStoreResult<InsulinValue>) -> Void) {
        getInsulinOnBoardValues(
            start: date.addingTimeInterval(.minutes(-5)),
            ...
            case let .success(values):
                ...
                guard let maxValue = closest.max(by: { $0.value < $1.value }) else {
    ...
    func getInsulinOnBoardValues(
        start: Date,
        end: Date? = nil,
        basalDosingEnd: Date? = nil,
        completion: @escaping (_ result: DoseStoreResult<[InsulinValue]>) -> Void
    ) {
        ...
        getNormalizedDoseEntries(start: doseStart, end: end) { result in
            ...
            case let .success(doses):
                ...
                let insulinOnBoard = trimmedDoses.insulinOnBoard(
                    insulinModelProvider: self.insulinModelProvider,
                    longestEffectDuration: self.longestEffectDuration
                )
    ...
    func getGlucoseEffects(
        start: Date,
        end: Date? = nil,
        basalDosingEnd: Date? = Date(),
        completion: @escaping (_ result: DoseStoreResult<[GlucoseEffect]>) -> Void
    ) {
        guard let insulinSensitivitySchedule = insulinSensitivityScheduleApplyingOverrideHistory else {
            completion(.failure(.configurationError))
            return
        }
        ...
        getNormalizedDoseEntries(start: doseStart, end: end) { result in
            ...
            case let .success(doses):
                ...
                let glucoseEffects = trimmedDoses.glucoseEffects(
                    insulinModelProvider: self.insulinModelProvider,
                    longestEffectDuration: self.longestEffectDuration,
            ...
            self.getReservoirValues(since: historyStart) { result in
                ...
                self.getPumpEventValues(since: historyStart) { result in
                    ...
                    self.getNormalizedDoseEntries(start: firstPumpEventDate) { result in
                        ...
                        self.getPumpEventDoseEntriesForSavingToInsulinDeliveryStore(
                            startingAt: firstPumpEventDate,
...
public extension DoseStore {
    struct QueryAnchor: Equatable, RawRepresentable {
    ...
    enum PumpEventQueryResult {
        case success(QueryAnchor, [PersistedPumpEvent])
        case failure(Error)
    }

    func executePumpEventQuery(
        fromQueryAnchor queryAnchor: QueryAnchor?,
        limit: Int,
        completion: @escaping (PumpEventQueryResult) -> Void
    ) {
        ...
        persistenceController.managedObjectContext.performAndWait {
            ...
            do {
                ...
                if let modificationCounter = stored.max(by: { $0.modificationCounter < $1.modificationCounter })?
                    .modificationCounter
                {
                    queryAnchor.modificationCounter = modificationCounter
                }
                queryResult.append(contentsOf: stored.compactMap(\.persistedPumpEvent))
            } catch {
                queryError = error
            }
        }
                ...
                do {
                    ...
                    request
                        ...
                        NSCompoundPredicate(andPredicateWithSubpredicates: [
                            NSPredicate(
                                format: "modificationCounter > %d",
                                modificationCounter
                            ),
                            self.exportDatePredicate(
                                startDate: startDate,
                                endDate: endDate
                            )
                        ])
...
public extension DoseStore {
    func addPumpEvents(events: [PersistedPumpEvent]) -> Error? {
        ...
        persistenceController.managedObjectContext.perform {
            ...
            self.persistenceController.save { saveError in
                ...
                self.syncPumpEventsToInsulinDeliveryStore(after: events.compactMap(\.date).min()) { syncError in
                    error = syncError
                    dispatchGroup.leave()
                }
            }
        ...
        log.info("Added %d PumpEvents", events.count)
        delegate?.doseStoreHasUpdatedPumpEventData(self)
        return nil
    }
}
