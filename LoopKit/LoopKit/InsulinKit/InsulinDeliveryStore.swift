...
public class InsulinDeliveryStore {
    ...
    public static let doseEntriesDidChange = NSNotification
        .Name(rawValue: "com.loopkit.InsulinDeliveryStore.doseEntriesDidChange")
    ...
    internal func currentDate(timeIntervalSinceNow: TimeInterval = 0) -> Date {
        currentDate.addingTimeInterval(timeIntervalSinceNow)
    }
    ...
    public init(
        ...
        observationEnabled _: Bool = true,
...
extension InsulinDeliveryStore: HealthKitSampleStoreDelegate {
    ...
    public func storeQueryAnchor(_ anchor: HKQueryAnchor) {
        ...
        log.default("stored query anchor %{public}@", String(describing: anchor))
    }

    public func processResults(
        from _: HKAnchoredObjectQuery,
        added: [HKSample],
        ...
        queue.async {
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                do {
                    ...
                    if let samples = added as? [HKQuantitySample] {
                        for sample in samples {
                            if try self.addDoseEntry(for: sample) {
                                ...
                                self.log.default(
                                    "Sample %@ from HKAnchoredObjectQuery already present in cache",
                    ...
                    let count = try self.deleteDoseEntries(withUUIDs: deleted.map(\.uuid))
                    if count > 0 {
...
public extension InsulinDeliveryStore {
    ...
    func getDoseEntries(
        ...
        includeMutable: Bool = false,
        completion: @escaping (_ result: Result<[DoseEntry], Error>) -> Void
    ) {
        queue.async {
    ...
    private func getDoseEntries(
        start: Date? = nil,
        end: Date? = nil,
        includeMutable: Bool = false
    ) -> Result<[DoseEntry], Error> {
        ...
        cacheStore.managedObjectContext.performAndWait {
            do {
                entries = try self.getCachedInsulinDeliveryObjects(start: start, end: end, includeMutable: includeMutable)
                    .map(\.dose)
            } catch let coreDataError {
                error = coreDataError
            }
        ...
        if let error = error {
            log.error("Error getting CachedInsulinDeliveryObjects: %{public}@", String(describing: error))
    ...
    private func getCachedInsulinDeliveryObjects(
        start: Date? = nil,
        end: Date? = nil,
        includeMutable: Bool = false
    ) throws -> [CachedInsulinDeliveryObject] {
        dispatchPrecondition(condition: .onQueue(queue))
        ...
        if let end = end {
            predicates
                ...
                )) // Note: Using <= rather than < to match previous behavior
        }
        if !includeMutable {
        ...
        request.predicate = (predicates.count > 1) ? NSCompoundPredicate(andPredicateWithSubpredicates: predicates) : predicates
            .first
    ...
    func getManuallyEnteredDoses(
        ...
        queue.async {
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                ...
                do {
                    doses = try self.cacheStore.managedObjectContext.fetch(request).compactMap(\.dose)
    ...
    func getBoluses(start: Date? = nil, end: Date? = nil, limit: Int? = nil) async throws -> [DoseEntry] {
        try await withCheckedThrowingContinuation({ continuation in
            queue.async {
                self.cacheStore.managedObjectContext.performAndWait {
                    ...
                    var predicates = [
                        NSPredicate(format: "deletedAt == NIL"),
                        NSPredicate(format: "reason == %d", HKInsulinDeliveryReason.bolus.rawValue)
                    ]
                    if let start {
                        predicates.append(NSPredicate(format: "endDate >= %@", start as NSDate))
                    }
                    ...
                    do {
                        let doses = try self.cacheStore.managedObjectContext.fetch(request).compactMap(\.dose)
    ...
    func getDoses(start: Date? = nil, end: Date? = nil, limit: Int? = nil) async throws -> [DoseEntry] {
        try await withCheckedThrowingContinuation({ continuation in
            queue.async {
                self.cacheStore.managedObjectContext.performAndWait {
                    let request: NSFetchRequest<CachedInsulinDeliveryObject> = CachedInsulinDeliveryObject.fetchRequest()
    ...
    internal func getLastImmutableBasalEndDate(_ completion: @escaping (_ result: Result<Date, Error>) -> Void) {
        queue.async {
            ...
            case .none:
                ...
                completion(.failure(DoseStore.DoseStoreError.initializationError(
                    description: "lastImmutableBasalEndDate has not been set",
                    recoverySuggestion: "Avoid accessing InsulinDeliveryStore until initialization is complete"
                )))
            }
        }
    }
    ...
    private func updateLastImmutableBasalEndDate() {
        ...
        lastImmutableBasalEndDate = endDate ?? .distantPast
    }
}
...
extension InsulinDeliveryStore {
        ...
        queue.async {
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                do {
                    ...
                    let resolvedSampleObjects: [(HKQuantitySample, CachedInsulinDeliveryObject)] = entries
                        .compactMap { (entry) -> (HKQuantitySample, CachedInsulinDeliveryObject)? in
                            guard entry.syncIdentifier != nil else {
                                self.log.error(
                                    "Ignored adding dose entry without sync identifier: %{public}@",
                                    String(reflecting: entry)
                                )
                                return nil
                            }

                            guard let quantitySample = HKQuantitySample(
                                ...
                                self.log.error(
                                    "Failure to create HKQuantitySample from DoseEntry: %{public}@",
                            ...
                            if let object = mutableObjects
                                .first(where: {
                                    $0.provenanceIdentifier == self.provenanceIdentifier && $0.syncIdentifier == entry
                                        .syncIdentifier })
                            {
                        ...
                        if updateExistingRecords {
                            ...
                            request
                                ...
                                NSCompoundPredicate(andPredicateWithSubpredicates: [NSPredicate(
                                    format: "syncIdentifier IN %@",
                                    entries.map(\.syncIdentifier)
                                )])
    ...
    private func saveEntriesToHealthKit(_ sampleObjects: [(HKQuantitySample, CachedInsulinDeliveryObject)]) {
        ...
        if let error = error {
            log.error("Error saving HealthKit objects: %@", String(describing: error))
            return
        }
        ...
        if let error = cacheStore.save() {
            log.error("Error updating CachedInsulinDeliveryObjects after saving HealthKit objects: %@", String(describing: error))
    ...
    private func addDoseEntry(for sample: HKQuantitySample) throws -> Bool {
        ...
        object.create(fromExisting: sample, on: currentDate())
        ...
        queue.async {
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                do {
                    ...
                    let healthKitPredicate = HKQuery.predicateForObjects(
                        withMetadataKey: HKMetadataKeySyncIdentifier,
                        allowedValues: [syncIdentifier]
                    )
                    if let hkSampleStore = self.hkSampleStore {
                        hkSampleStore.healthStore.deleteObjects(
                            of: HealthKitSampleStore.insulinQuantityType,
                            predicate: healthKitPredicate
                        )
                            { _, _, error in
                                if let error = error {
    ...
    public func deleteAllManuallyEnteredDoses(
        since startDate: Date,
        _ completion: @escaping (_ error: DoseStore.DoseStoreError?) -> Void
    ) {
        queue.async {
...
extension InsulinDeliveryStore {
    ...
    @discardableResult private func purgeCachedInsulinDeliveryObjects(before date: Date? = nil) -> Error? {
        purgeCachedInsulinDeliveryObjects(matching: date.map { NSPredicate(format: "endDate < %@", $0 as NSDate) })
    }
    ...
    private func purgeCachedInsulinDeliveryObjects(matching predicate: NSPredicate? = nil) -> Error? {
        ...
        cacheStore.managedObjectContext.performAndWait {
            do {
                let count = try cacheStore.managedObjectContext.purgeObjects(
                    of: CachedInsulinDeliveryObject.self,
                    matching: predicate
                )
                if count > 0 {
    ...
    private func handleUpdatedDoseData() {
        ...
        purgeExpiredCachedInsulinDeliveryObjects()
        updateLastImmutableBasalEndDate()
        ...
        queue.async {
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                ...
                do {
                    ...
                    queryCreatedResult.append(contentsOf: stored.filter({ $0.deletedAt == nil }).compactMap(\.dose))
                    queryDeletedResult.append(contentsOf: stored.filter({ $0.deletedAt != nil }).compactMap(\.dose))
                } catch {
                    queryError = error
                }
            }
...
public extension InsulinDeliveryStore {
    var test_lastImmutableBasalEndDate: Date? {
        get {
            var date: Date?
            queue.sync {
