...
extension CachedInsulinDeliveryObject {
    func create(fromExisting sample: HKQuantitySample, on date: Date = Date()) {
        ...
        syncIdentifier = sample.syncIdentifier ?? sample.uuid
            .uuidString // External doses might not have a syncIdentifier, so use the UUID
        ...
        insulinType = sample.insulinType
        automaticallyIssued = sample.automaticallyIssued
        manuallyEntered = sample.manuallyEntered
        isSuspend = sample.isSuspend
...
extension CachedInsulinDeliveryObject {
    func create(from entry: DoseEntry, by provenanceIdentifier: String, at date: Date) {
        ...
        value = entry.unitsInDeliverableIncrements
        scheduledBasalRate = entry.scheduledBasalRate
        programmedTempBasalRate = (entry.type == .tempBasal) ?
            HKQuantity(unit: .internationalUnitsPerHour, doubleValue: entry.unitsPerHour) : nil
        reason = (entry.type == .bolus) ? .bolus : .basal
        createdAt = date
        deletedAt = nil
        insulinType = entry.insulinType
        automaticallyIssued = entry.automatic
        manuallyEntered = entry.manuallyEntered
        isSuspend = (entry.type == .suspend)
