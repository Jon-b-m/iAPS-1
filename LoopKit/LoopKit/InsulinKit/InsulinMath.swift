...
public enum InsulinMath {
    public static var defaultInsulinActivityDuration = TimeInterval(hours: 6) + TimeInterval(minutes: 10)
}
...
extension DoseEntry {
    ...
    func glucoseEffect(
        ...
        if endDate.timeIntervalSince(startDate) <= 1.05 * delta {
            ...
            return netBasalUnits * -insulinSensitivity * continuousDeliveryGlucoseEffect(
                at: interval.end,
                model: model,
                delta: delta
            )
    ...
    var doseEntries: [DoseEntry] {
        ...
        for value in self {
            if let previousValue = previousValue {
                ...
                if duration > 0, volumeDrop >= 0, volumeDrop <= MaximumReservoirDropPerMinute * duration.minutes {
...
private extension DoseEntry {
    var resolvingDelivery: DoseEntry {
...
public extension Collection where Element: TimelineValue {
    var timespan: DateInterval {
        guard count > 0 else {
...
public extension Collection where Element == DoseEntry {
    /**
     ...
     */
    internal func reconciled() -> [DoseEntry] {
        ...
        return reconciled.map(\.resolvingDelivery)
    }
    ...
    /// - Returns: An array of annotated dose entries
    func annotated(with basalHistory: [AbsoluteScheduleValue<Double>]) -> [DoseEntry] {
        var annotatedDoses: [DoseEntry] = []
     ...
     */
    internal var totalDelivery: Double {
        reduce(0) { (total, dose) -> Double in
            total + dose.unitsInDeliverableIncrements
        }
    }
    ...
    func insulinOnBoard(
        ...
        guard let (start, end) = LoopMath
            .simulationDateRangeForSamples(self, from: start, to: end, duration: longestEffectDuration, delta: delta)
        else {
            return []
        }
        ...
        repeat {
            let value = reduce(0) { (value, dose) -> Double in
                value + dose.insulinOnBoard(at: date, model: insulinModelProvider.model(for: dose.insulinType), delta: delta)
            }
    ...
    func glucoseEffects(
        ...
        guard let (start, end) = LoopMath.simulationDateRangeForSamples(filter({ entry in
        ...
        repeat {
            let value = reduce(0) { (value, dose) -> Double in
                ...
                let doseEffect = dose.glucoseEffect(
                    at: date,
                    model: insulinModelProvider.model(for: dose.insulinType),
    ...
    func glucoseEffects(
        ...
        let activeEntries = filter({ entry in
            entry.netBasalUnits != 0
        })

        guard let (start, end) = LoopMath.simulationDateRangeForSamples(
            activeEntries,
            from: start,
            to: end,
            duration: longestEffectDuration,
            delta: delta
        ) else {
            return []
        }
        ...
        repeat {
            let value = reduce(0) { (value, dose) -> Double in
                ...
                guard let isfScheduleValue = insulinSensitivityHistory.closestPrior(to: dose.startDate),
                      isfScheduleValue.endDate >= dose.startDate
                else {
        ...
        repeat {
            ...
            let value = reduce(0) { (value, dose) -> Double in
                ...
                return value + isfSegments.reduce(0, { partialResult, segment in
                    ...
                    return partialResult + dose.glucoseEffect(
    ...
    func glucoseEffects(
        insulinModelProvider: InsulinModelProvider = PresetInsulinModelProvider(defaultRapidActingModel: nil),
        longestEffectDuration _: TimeInterval = InsulinMath.defaultInsulinActivityDuration,
        insulinSensitivityTimeline: [AbsoluteScheduleValue<HKQuantity>],
        effectDates: [Date],
        delta: TimeInterval = TimeInterval( /* minutes: */ 60 * 5)
    ) -> [GlucoseEffect] {
    ...
    func infill(
        with basalHistory: [AbsoluteScheduleValue<Double>],
        endDate: Date? = nil,
        gapPatchInterval: TimeInterval = 0
    ) -> [DoseEntry] {
        guard !basalHistory.isEmpty else {
            return Array(self)
        }
    ...
    /// - Returns: An array of doses,
    func overlayBasalSchedule(
        _ basalSchedule: BasalRateSchedule,
        startingAt start: Date,
        endingAt end: Date = .distantFuture,
        insertingBasalEntries: Bool
    ) -> [DoseEntry] {
                ...
                if let lastBasal = lastBasal {
                    if insertingBasalEntries {
                        ...
                        for scheduled in basalSchedule.between(start: lastBasal.endDate, end: dose.startDate) {
                            ...
                            let syncIdentifier =
                                "BasalRateSchedule \(dateFormatter.string(from: start)) \(dateFormatter.string(from: end))"
    ...
    internal func appendedUnion(with otherDoses: [DoseEntry]) -> [DoseEntry] {
        var union: [DoseEntry] = []
