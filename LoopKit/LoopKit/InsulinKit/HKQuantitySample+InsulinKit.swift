...
extension HKQuantitySample {
    convenience init?(
        ...
        case .basal,
            ...
            if dose.type == .tempBasal {
                metadata[MetadataKeyProgrammedTempBasalRate] = HKQuantity(
    ...
    var scheduledBasalRate: HKQuantity? {
        metadata?[MetadataKeyScheduledBasalRate] as? HKQuantity
    }

    var programmedTempBasalRate: HKQuantity? {
        metadata?[MetadataKeyProgrammedTempBasalRate] as? HKQuantity
    }

    var isSuspend: Bool {
        metadata?[MetadataKeyIsSuspend] as? Bool ?? false
    }

    var manuallyEntered: Bool {
        metadata?[MetadataKeyManuallyEntered] as? Bool ?? false
    }

    var automaticallyIssued: Bool? {
        metadata?[MetadataKeyAutomaticallyIssued] as? Bool
    }
