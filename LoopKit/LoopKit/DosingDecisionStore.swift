                ...
                self.store.managedObjectContext.performAndWait {
                    ...
                    do {
                        ...
                        let dosingDecisions = dosingDecisionObjects.compactMap { object in
                            self.decodeDosingDecision(fromData: object.data)
                        }
...
public extension DosingDecisionStore {
    ...
    enum DosingDecisionQueryResult {
        case success(QueryAnchor, [StoredDosingDecision])
        case failure(Error)
    }

    func executeDosingDecisionQuery(
        fromQueryAnchor queryAnchor: QueryAnchor?,
        limit: Int,
        completion: @escaping (DosingDecisionQueryResult) -> Void
    ) {
        dataAccessQueue.async {
            ...
            self.store.managedObjectContext.performAndWait {
                ...
                defer {
                    ...
                    self.log.debug(
                        "executeDosingDecisionQuery (anchor = %{public}@: queueWait(%.03f), fetch(%.03f)",
                        String(describing: queryAnchor),
                        queueWait,
                        fetchWait
                    )
                }
...
extension StoredDosingDecision: Codable {
    public init(from decoder: Decoder) throws {
        ...
        self.init(
            ...
            automaticDoseRecommendation: try container.decodeIfPresent(
                AutomaticDoseRecommendation.self,
                forKey: .automaticDoseRecommendation
            ),
            manualBolusRecommendation: try container.decodeIfPresent(
                ManualBolusRecommendationWithDate.self,
                forKey: .manualBolusRecommendation
            ),
...
public extension DosingDecisionStore {
    func addStoredDosingDecisions(dosingDecisions: [StoredDosingDecision], completion: @escaping (Error?) -> Void) {
