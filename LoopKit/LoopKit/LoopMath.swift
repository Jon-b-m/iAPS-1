...
public enum LoopMath {
    ...
    public static func predictGlucose(
        ...
        predictGlucose(startingAt: startingGlucose, momentum: momentum, effects: effects)
    }
    ...
    public static func predictGlucose(
        ...
        let prediction = effectValuesAtDate.sorted { $0.0 < $1.0 }.reduce([PredictedGlucoseValue(
            startDate: startingGlucose.startDate,
            quantity: startingGlucose.quantity
        )]) { (prediction, effect) -> [PredictedGlucoseValue] in
...
public extension GlucoseValue {
     ...
     */
    func decayEffect(
        atRate rate: HKQuantity,
        for duration: TimeInterval,
        withDelta delta: TimeInterval = 5 * 60
    ) -> [GlucoseEffect] {
...
public extension BidirectionalCollection where Element == GlucoseEffect {
    ...
    func combinedSums(of duration: TimeInterval) -> [GlucoseChange] {
    ...
    func netEffect() -> GlucoseChange? {
        guard let first = self.first, let last = self.last else {
            return nil
        }

        let net = last.quantity.doubleValue(for: .milligramsPerDeciliter) - first.quantity
            .doubleValue(for: .milligramsPerDeciliter)

        return GlucoseChange(
            startDate: first.startDate,
            endDate: last.endDate,
            quantity: HKQuantity(unit: .milligramsPerDeciliter, doubleValue: net)
        )
    }
}
...
public extension BidirectionalCollection where Element == GlucoseEffectVelocity {
    ...
    func subtracting(
        _ otherEffects: [GlucoseEffect],
        withUniformInterval effectInterval: TimeInterval = GlucoseMath.defaultDelta
    ) -> [GlucoseEffect] {
        // Trim both collections to match
        let otherEffects = otherEffects.filterDateRange(first?.endDate, nil)
        let effects = filterDateRange(otherEffects.first?.startDate, nil)
