...
public protocol PersistenceControllerDelegate: AnyObject {
    ...
    func persistenceControllerDidSave(
...
public final class PersistenceController {
    ...
    @discardableResult func save(_ completion: ((_ error: PersistenceControllerError?) -> Void)? = nil)
        -> PersistenceControllerError?
    {
    ...
    func updateMetadata(key: String, value: Any?) {
        if let coordinator = managedObjectContext.persistentStoreCoordinator, let store = coordinator.persistentStores.first {
        ...
        managedObjectContext.perform {
            ...
            do {
                try FileManager.default.ensureDirectoryExists(
                    at: directoryURL,
                    with: FileProtectionType.completeUntilFirstUserAuthentication
                )
            } catch {
...
private extension FileManager {
    func ensureDirectoryExists(at url: URL, with protectionType: FileProtectionType? = nil) throws {
        try createDirectory(
            ...
            attributes: protectionType.map { [FileAttributeKey.protectionKey: $0] }
        )
