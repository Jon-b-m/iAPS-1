...
open class QuantityFormatter {
    ...
    open func string(from quantity: HKQuantity, includeUnit: Bool = true) -> String? {
        ...
        if let foundationUnit = unit.foundationUnit, unit.usesMeasurementFormatterForMeasurement {
            return measurementFormatter.string(from: Measurement(value: value, unit: foundationUnit))
                .avoidLineBreaking(enabled: avoidLineBreaking)
        }

        // Pass 'false' for `avoidLineBreaking` because we don't want to do it twice.
        return numberFormatter.string(
            from: value,
            unit: localizedUnitStringWithPlurality(forValue: value, avoidLineBreaking: false),
            style: unitStyle,
            avoidLineBreaking: avoidLineBreaking
        )
    }
    ...
    open func localizedUnitStringWithPlurality(forValue value: Double = 10, avoidLineBreaking: Bool? = nil) -> String {
        ...
        if let string = unit.localizedUnitString(
            in: unitStyle,
            singular: abs(1.0 - value) < .ulpOfOne,
            avoidLineBreaking: avoidLineBreaking
        ) {
            return string
        }
...
public extension HKQuantity {
    ...
    func doubleValue(
        for unit: HKUnit,
        withRounding: Bool,
        usingFractionDigits fractionDigits: Int? = nil,
        rule: FloatingPointRoundingRule = .toNearestOrAwayFromZero
    ) -> Double {
        var value = doubleValue(for: unit)
        if withRounding {
...
public extension ClosedRange where Bound == HKQuantity {
    func roundedDisplayValues(for unit: HKUnit) -> [Double] {
        let stride = 1 / pow(10.0, Double(unit.preferredFractionDigits))
        return Array(
            Swift.stride(
                from: lowerBound.doubleValue(for: unit, withRounding: true, rule: .up),
                through: upperBound.doubleValue(for: unit, withRounding: true, rule: .down),
                by: stride
            )
        )
    }
...
public extension HKUnit {
    ...
    var preferredFractionDigits: Int {
        switch self {
        case HKUnit.millimolesPerLiter.unitDivided(by: .internationalUnit()),
    ...
    /// if fractionDigits is nil, defaults to the unit maxFractionDigits
    func allValues(
        from lowerBound: HKQuantity,
        through upperBound: HKQuantity,
        usingFractionDigits fractionDigits: Int? = nil
    ) -> [Double] {
        let usedFractionDigits: Int = fractionDigits ?? maxFractionDigits
        return Array(stride(
    ...
    func round(value: Double) -> Double {
        roundForPreferredDigits(value: value)
    }

    func roundForPreferredDigits(value: Double) -> Double {
        round(value: value, fractionDigits: preferredFractionDigits)
    }

    func roundForPicker(value: Double) -> Double {
        round(value: value, fractionDigits: maxFractionDigits)
    }

    // Short localized unit string with unlocalized fallback
    func shortLocalizedUnitString(avoidLineBreaking: Bool = true) -> String {
        localizedUnitString(in: .short, avoidLineBreaking: avoidLineBreaking) ??
        ...
        func localizedUnitStringInternal(in style: Formatter.UnitStyle, singular: Bool = false) -> String? {
            ...
            if self == .internationalUnitsPerHour {
                ...
                case .medium,
                    ...
                    return LocalizedString(
                        "U/hr",
                        comment: "The short unit display string for international units of insulin per hour"
                    )
                case .long:
                    fallthrough
                @unknown default:
...
private extension String {
    func avoidLineBreaking(around string: String = "/", enabled: Bool) -> String {
        ...
        return replacingOccurrences(of: string, with: "\(String.wordJoiner)\(string)\(String.wordJoiner)")
    }
}
