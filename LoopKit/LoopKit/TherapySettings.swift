...
extension TherapySettings: Codable {
    public init(from decoder: Decoder) throws {
        ...
        let glucoseTargetRangeSchedule = try container.decodeIfPresent(
            GlucoseRangeSchedule.self,
            forKey: .glucoseTargetRangeSchedule
        )
        let correctionRangeOverrides = try container.decodeIfPresent(
            CorrectionRangeOverrides.self,
            forKey: .correctionRangeOverrides
        )
        ...
        let insulinSensitivitySchedule = try container.decodeIfPresent(
        ...
        let defaultRapidActingModel = try container.decodeIfPresent(
...
public extension TherapySettings {
    // Mock therapy settings for QA and mock prescriptions
    static var mockTherapySettings: TherapySettings {
        let timeZone = TimeZone(identifier: "America/Los_Angeles")!
        let glucoseTargetRangeSchedule = GlucoseRangeSchedule(
            rangeSchedule: DailyQuantitySchedule(
                unit: .milligramsPerDeciliter,
                dailyItems: [
                    RepeatingScheduleValue(startTime: .hours(0), value: DoubleRange(minValue: 100.0, maxValue: 110.0)),
            ...
            override: GlucoseRangeSchedule.Override(
                value: DoubleRange(minValue: 80.0, maxValue: 90.0),
        ...
        let basalRateSchedule = BasalRateSchedule(
            dailyItems: [
                RepeatingScheduleValue(startTime: .hours(0), value: 1),
