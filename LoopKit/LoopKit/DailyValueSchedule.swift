...
public struct RepeatingScheduleValue<T> {
    ...
    public func map<U>(_ transform: (T) -> U) -> RepeatingScheduleValue<U> {
        RepeatingScheduleValue<U>(startTime: startTime, value: transform(value))
    }
}
...
extension RepeatingScheduleValue: Equatable where T: Equatable {
    public static func == (lhs: RepeatingScheduleValue, rhs: RepeatingScheduleValue) -> Bool {
        abs(lhs.startTime - rhs.startTime) < .ulpOfOne && lhs.value == rhs.value
    }
}
...
public extension DailySchedule {
    func value(at time: Date) -> T {
        between(start: time, end: time).first!.value
    }
}

public extension DailySchedule where T: Comparable {
    func valueRange() -> ClosedRange<T> {
...
public struct DailyValueSchedule<T>: DailySchedule {
    ...
    public init?(dailyItems: [RepeatingScheduleValue<T>], timeZone: TimeZone? = nil) {
        ...
        items = dailyItems.sorted { $0.startTime < $1.startTime }
    ...
    var maxTimeInterval: TimeInterval {
        referenceTimeInterval + repeatInterval
    }
        ...
        where T == (L, R)
    {
        ...
        repeat {
            ...
            case let (.some(leftStart), .some(rightStart)):
...
extension DailyValueSchedule: RawRepresentable, CustomDebugStringConvertible where T: RawRepresentable {
    ...
    public var rawValue: RawValue {
        let rawItems = items.map(\.rawValue)
...
public extension RepeatingScheduleValue {
    static func == <L: Equatable, R: Equatable>(lhs: RepeatingScheduleValue, rhs: RepeatingScheduleValue) -> Bool
        where T == (L, R)
    {
...
public extension DailyValueSchedule {
    static func == <L: Equatable, R: Equatable>(lhs: DailyValueSchedule, rhs: DailyValueSchedule) -> Bool where T == (L, R) {
