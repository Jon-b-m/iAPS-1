...
extension WeakSynchronizedSet {
    private func identifier(for element: Element) -> ObjectIdentifier {
        ObjectIdentifier(element as AnyObject)
    }
    ...
    @discardableResult private func insert(_ newMember: ElementContainer)
        -> (inserted: Bool, memberAfterInsert: ElementContainer?)
    {
    ...
    @discardableResult private func removeValue(forKey key: Identifier) -> ElementContainer? {
