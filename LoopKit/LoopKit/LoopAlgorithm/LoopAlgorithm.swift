...
//    }
    ...
    public static func generatePrediction(input: LoopPredictionInput, startDate: Date? = nil) throws -> LoopPrediction {
        ...
        if let doseStart = input.doses.first?.startDate {
            ...
            precondition(
                basalStart <= doseStart,
                "Basal history must cover historic dose range. First dose date: \(doseStart) < \(basalStart)"
            )
        }
        ...
        guard let curSensitivity = settings.sensitivity.closestPrior(to: start)?.value,
              ...
              let curTarget = settings.target.closestPrior(to: start)?.value
        else {
        ...
        if settings.algorithmEffectsOptions.contains(.momentum) {
            let momentumInputData = input.glucoseHistory.filterDateRange(
                start.addingTimeInterval(-GlucoseMath.momentumDataInterval),
                start
            )
