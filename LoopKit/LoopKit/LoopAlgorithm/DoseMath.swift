...
private extension InsulinCorrection {
...
extension TempBasalRecommendation {
    ...
    private func matchesRate(_ unitsPerHour: Double) -> Bool {
        abs(self.unitsPerHour - unitsPerHour) < .ulpOfOne
    }
...
public extension Collection where Element: GlucoseValue {
    ...
    private func insulinCorrection(
        ...
        let timeline =
            [AbsoluteScheduleValue(startDate: date, endDate: date.addingTimeInterval(effectDuration), value: sensitivity)]
    ...
    private func insulinCorrection(
        ...
        for prediction in self {
            ...
            let effectedSensitivity = isfSegments.reduce(0) { _, segment in
    ...
    func recommendedTempBasal(
        ...
        if let additionalActiveInsulinClamp {
            let maxThirtyMinuteRateToKeepIOBBelowLimit = additionalActiveInsulinClamp * 2.0 +
                scheduledBasalRate // 30 minutes of a U/hr rate
    ...
    func recommendedAutomaticDose(
        ...
        guard let correction = insulinCorrection(
            to: correctionRange,
            at: date,
            suspendThreshold: suspendThreshold ?? correctionRange.quantityRange(at: date).lowerBound,
            sensitivity: sensitivity.quantity(at: date),
