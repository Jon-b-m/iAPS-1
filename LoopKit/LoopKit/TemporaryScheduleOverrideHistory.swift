...
public final class TemporaryScheduleOverrideHistory {
    ...
    public var relevantTimeWindow = TimeInterval.hours(10)
    ...
    private var lastUndeletedEvent: OverrideEvent? {
        recentEvents.reversed().first { $0.override.actualEnd != .deleted }
    }
    ...
    public func resolvingRecentBasalSchedule(
        _ base: BasalRateSchedule,
    ...
    public func resolvingRecentInsulinSensitivitySchedule(
        _ base: InsulinSensitivitySchedule,
        relativeTo referenceDate: Date = Date()
    ...
    public func resolvingRecentCarbRatioSchedule(
        _ base: CarbRatioSchedule,
        relativeTo referenceDate: Date = Date()
    ) -> CarbRatioSchedule {
        filterRecentEvents(relativeTo: referenceDate)
        return overridesReflectingEnabledDuration(relativeTo: referenceDate).reduce(base) { base, override in
    ...
    public func getEvents(relativeTo referenceDate: Date = Date()) -> [TemporaryScheduleOverride] {
        filterRecentEvents(relativeTo: referenceDate)
        return recentEvents.map(\.override)
    }
    ...
    private func overridesReflectingEnabledDuration(relativeTo referenceDate: Date) -> [TemporaryScheduleOverride] {
        var overrides = recentEvents.filter({ $0.override.actualEnd != .deleted }).map { event -> TemporaryScheduleOverride in
            ...
            if case let .early(endDate) = event.override.actualEnd {
    ...
    public func queryByAnchor(_ anchor: QueryAnchor?)
        -> (resultOverrides: [TemporaryScheduleOverride], deletedOverrides: [TemporaryScheduleOverride], newAnchor: QueryAnchor)
    {
...
extension OverrideEvent: RawRepresentable {
    ...
    init?(rawValue: RawValue) {
        ...
        modificationCounter = rawValue["modificationCounter"] as? Int64 ?? 0
    ...
    var rawValue: RawValue {
        ...
        if case let .early(endDate) = override.actualEnd {
            raw["endDate"] = endDate
        }
...
extension TemporaryScheduleOverrideHistory: RawRepresentable {
    ...
    public var rawValue: RawValue {
        [
            "recentEvents": recentEvents.map(\.rawValue),
            "taintedEventLog": taintedEventLog.map(\.rawValue),
            "modificationCounter": modificationCounter
        ]
    }
}
...
extension TemporaryScheduleOverrideHistory: CustomDebugStringConvertible {
    public var debugDescription: String {
        "TemporaryScheduleOverrideHistory(recentEvents: \(recentEvents), taintedEventLog: \(taintedEventLog))"
    }
}

private extension Date {
    var nearestPrevious: Date {
        Date(timeIntervalSince1970: timeIntervalSince1970.nextDown)
    }
}
