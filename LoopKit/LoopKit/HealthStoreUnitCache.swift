...
public extension Notification.Name {
    ...
    static let HealthStorePreferredGlucoseUnitDidChange = Notification
        .Name(rawValue: "com.loopKit.notification.HealthStorePreferredGlucoseUnitDidChange")
}
...
public class HealthStoreUnitCache {
    ...
    private init(healthStore: HKHealthStore) {
        ...
        userPreferencesChangeObserver = NotificationCenter.default.addObserver(
    ...
    @discardableResult private func getHealthStoreUnitAndUpdateCache(for quantityTypeIdentifier: HKQuantityTypeIdentifier)
        -> HKUnit?
    {
        ...
        healthStore.preferredUnits(for: [quantityType]) { results, error in
            if let error = error {
                ...
                OSLog(category: "HealthStoreUnitCache").info(
                    "Error fetching unit for %{public}@: %{public}@",
                    quantityTypeIdentifier.rawValue,
