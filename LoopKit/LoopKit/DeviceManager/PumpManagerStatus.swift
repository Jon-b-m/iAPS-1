...
extension PumpManagerStatus: Codable {
    public init(from decoder: Decoder) throws {
        ...
        device = try container.decode(CodableDevice.self, forKey: .device).device
        pumpBatteryChargeRemaining = try container.decodeIfPresent(Double.self, forKey: .pumpBatteryChargeRemaining)
        basalDeliveryState = try container.decodeIfPresent(BasalDeliveryState.self, forKey: .basalDeliveryState)
        bolusState = try container.decode(BolusState.self, forKey: .bolusState)
        insulinType = try container.decodeIfPresent(InsulinType.self, forKey: .insulinType)
        deliveryIsUncertain = try container.decode(Bool.self, forKey: .deliveryIsUncertain)
    }
...
extension PumpManagerStatus.BolusState: Codable {
    public init(from decoder: Decoder) throws {
        if let string = try? decoder.singleValueContainer().decode(String.self) {
            switch string {
            case CodableKeys.noBolus.rawValue,
                 "none": // included for backward compatibility. BolusState.none -> BolusState.noBolus
                self = .noBolus
