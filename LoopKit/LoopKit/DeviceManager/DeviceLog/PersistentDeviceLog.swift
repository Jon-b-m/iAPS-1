...
public class PersistentDeviceLog {
    ...
    public var earliestLogEntryDate: Date {
        Date(timeIntervalSinceNow: -maxEntryAge)
    }
    ...
    public func getLogEntries(
        startDate: Date,
        endDate: Date? = nil,
        completion: @escaping (_ result: Result<[StoredDeviceLogEntry], Error>) -> Void
    ) {
        managedObjectContext.perform {
    ...
    public func purgeLogEntries(before date: Date, completion: ((Error?) -> Void)? = nil) {
        ...
        managedObjectContext.performAndWait {
            do {
                let count = try managedObjectContext.purgeObjects(
                    of: DeviceLogEntry.self,
                    matching: NSPredicate(format: "timestamp < %@", date as NSDate)
                )
...
public extension PersistentDeviceLog {
    func addStoredDeviceLogEntries(entries: [StoredDeviceLogEntry]) -> Error? {
        ...
        log.info("Added %d StoredDeviceLogEntries", entries.count)
        return nil
    }
}
