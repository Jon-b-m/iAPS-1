...
extension CGMManagerStatus: Codable {
    public init(from decoder: Decoder) throws {
        ...
        hasValidSensorSession = try container.decode(Bool.self, forKey: .hasValidSensorSession)
        lastCommunicationDate = try container.decodeIfPresent(Date.self, forKey: .lastCommunicationDate)
        device = try container.decodeIfPresent(CodableDevice.self, forKey: .device)?.device
    }
...
public extension CGMManager {
    ...
    func addStatusObserver(_: CGMManagerStatusObserver, queue _: DispatchQueue) {
        // optional since a CGM manager may not support status observers
    }

    func removeStatusObserver(_: CGMManagerStatusObserver) {
        // optional since a CGM manager may not support status observers
    }
