...
public extension GlucoseEffectVelocity {
    internal static let perSecondUnit = HKUnit.milligramsPerDeciliter.unitDivided(by: .second())
    ...
    func effect(from start: Date, to end: Date) -> GlucoseEffect? {
        guard
            start <= end,
            startDate <= start,
            end <= endDate
