...
public protocol PersistedAlertStore {
    ...
    func lookupAllUnacknowledgedUnretracted(
        ...
        completion: @escaping (Swift.Result<[PersistedAlert], Error>) -> Void
    )
...
public extension Alert.Sound {
    var filename: String? {
        switch self {
        case let .sound(name): return name
...
extension Decoder {
    var enumDecodingError: DecodingError {
        DecodingError.dataCorrupted(DecodingError.Context(codingPath: codingPath, debugDescription: "invalid enumeration"))
    }
}
