...
public extension GlucoseRangeSchedule {
    func applyingOverride(_ override: TemporaryScheduleOverride) -> GlucoseRangeSchedule {
        guard let targetRange = override.settings.targetRange else {
            return self
        }

        let doubleRange = targetRange.doubleRange(for: unit)
        let rangeOverride = GlucoseRangeSchedule.Override(
            value: doubleRange,
...
private extension DailyValueSchedule where T == Double {
    func applyingOverride(
