...
private extension Collection where Element == (x: Double, y: Double) {
    /**
...
public extension BidirectionalCollection where Element: GlucoseSampleValue, Index == Int {
    /// Whether the collection contains any calibration entries
    /// Runtime: O(n)
    func containsCalibrations() -> Bool {
        !filter({ $0.isDisplayOnly }).isEmpty
    }
    ...
    internal func isContinuous(within interval: TimeInterval = TimeInterval(minutes: 5)) -> Bool {
        if let first = first,
           let last = last,
    ...
    func linearMomentumEffect(
        ...
        let velocityMax = velocityMaximum ?? HKQuantity(
            unit: HKUnit.milligramsPerDeciliter.unitDivided(by: .minute()),
            doubleValue: 4.0
        )

        guard
            count > 2, // Linear regression isn't much use without 3 or more entries.
            isContinuous(), !containsCalibrations(), hasSingleProvenance,
            let firstSample = first,
            let lastSample = last,
        ...
        let (slope: slope, intercept: _) = map { (
...
extension Collection where Element: GlucoseSampleValue, Index == Int {
    ...
    var hasSingleProvenance: Bool {
        let firstProvenance = first?.provenanceIdentifier
    ...
    public func counteractionEffects(to effects: [GlucoseEffect]) -> [GlucoseEffectVelocity] {
        ...
        guard !isEmpty, !effects.isEmpty else {
            return []
        }

        let startGlucoseIdx = firstIndex { $0.startDate >= effects.first!.startDate }
        ...
        while endGlucoseIdx != endIndex {
            ...
            let effect = GlucoseEffectVelocity(
                startDate: startGlucose.startDate,
                endDate: endGlucose.startDate,
                quantity: averageVelocity
            )
