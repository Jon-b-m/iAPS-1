...
public final class CgmEventStore {
    ...
    public var cacheStartDate: Date {
        Date().addingTimeInterval(-cacheLength)
    }
    ...
    public init(
        ...
        cacheStore.onReady { error in
            ...
            cacheStore.fetchAnchor(key: GlucoseStore.healthKitQueryAnchorMetadataKey) { _ in
                self.queue.async {
...
public extension CgmEventStore {
     ...
     */
    func add(events: [PersistedCgmEvent]) async throws {
    ...
    enum CgmEventQueryResult {
        case success(QueryAnchor, [PersistedCgmEvent])
        case failure(Error)
    }

    func executeCgmEventQuery(fromQueryAnchor queryAnchor: QueryAnchor?, completion: @escaping (CgmEventQueryResult) -> Void) {
        ...
        cacheStore.managedObjectContext.performAndWait {
            ...
            do {
                ...
                queryResult.append(contentsOf: stored.compactMap(\.persistedCgmEvent))
            } catch {
                queryError = error
            }
        }
