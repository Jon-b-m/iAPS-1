...
extension StoredGlucoseSample: Codable {
    public init(from decoder: Decoder) throws {
        ...
        let provenanceIdentifier = try container.decodeIfPresent(String.self, forKey: .provenanceIdentifier) ?? Self
            .defaultProvenanceIdentifier
        ...
        self.init(
            ...
            trendRate: try container.decodeIfPresent(Double.self, forKey: .trendRate)
                .map { HKQuantity(unit: .milligramsPerDeciliterPerMinute, doubleValue: $0) },
            isDisplayOnly: isDisplayOnly,
            wasUserEntered: wasUserEntered,
            device: try container.decodeIfPresent(CodableDevice.self, forKey: .device).map(\.device),
