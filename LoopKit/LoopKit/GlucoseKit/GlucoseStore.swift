        ...
        queue.async {
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                do {
                    ...
                    let count = try self.deleteGlucoseSamples(withUUIDs: deleted.map(\.uuid))
                    if count > 0 {
            ...
            if let newestStartDate = added.map(\.startDate).max() {
...
extension GlucoseStore {
    ...
    public func getGlucoseSamples(
        start: Date? = nil,
        end: Date? = nil,
        completion: @escaping (_ result: Result<[StoredGlucoseSample], Error>) -> Void
    ) {
        queue.async {
                ...
                do {
                    ...
                    let samples: [NewGlucoseSample] = try samples.compactMap { sample in
                        guard syncIdentifiers.insert(sample.syncIdentifier).inserted else {
                            self.log.default(
                                "Skipping adding glucose sample due to duplicate sync identifier: %{public}@",
                        ...
                        guard try self.cacheStore.managedObjectContext.count(for: request) == 0 else {
                            self.log.default(
                                "Skipping adding glucose sample due to existing cached sync identifier: %{public}@",
                                sample.syncIdentifier
                            )
                            return nil
                        }
    ...
    private func saveSamplesToHealthKit() {
        ...
        cacheStore.managedObjectContext.performAndWait {
            do {
                ...
                request
                    .sortDescriptors =
                    [NSSortDescriptor(key: "modificationCounter", ascending: true)] // Maintains modificationCounter order
                ...
                if objects.contains(where: { $0.uuid != nil }) {
                    self.log
                        .error(
                            "Found CachedGlucoseObjects with non-nil uuid. Should never happen, but HealthKit should be able to resolve it."
                        )
                ...
                let quantitySamples = objects.map(\.quantitySample)
        ...
        if let error = error {
            log.error("Error saving samples to HealthKit: %{public}@", String(describing: error))
        }
    }
...
public extension GlucoseStore {
    ...
    func getSyncGlucoseSamples(
        ...
        queue.async {
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                do {
                    samples = try self.getCachedGlucoseObjects(start: start, end: end)
                        .map { StoredGlucoseSample(managedObject: $0) }
                } catch let coreDataError {
                    error = coreDataError
                }
            }
    ...
    func setSyncGlucoseSamples(_ objects: [StoredGlucoseSample], completion: @escaping (Error?) -> Void) {
        queue.async {
            var error: Error?
...
extension GlucoseStore {
    public var earliestCacheDate: Date {
        Date(timeIntervalSinceNow: -cacheLength)
    }
    ...
    public func purgeAllGlucoseSamples(healthKitPredicate: NSPredicate, completion: @escaping (Error?) -> Void) {
        queue.async {
            ...
            if let hkSampleStore = self.hkSampleStore {
                hkSampleStore.healthStore
                    .deleteObjects(of: HealthKitSampleStore.glucoseType, predicate: healthKitPredicate) { _, _, healthKitError in
                        self.queue.async {
    ...
    @discardableResult private func purgeCachedGlucoseObjects(before date: Date? = nil) -> Error? {
        dispatchPrecondition(condition: .onQueue(queue))
    ...
    private func purgeExpiredManagedDataFromHealthKit(before date: Date) {
        ...
        hkSampleStore.healthStore
            .deleteObjects(of: HealthKitSampleStore.glucoseType, predicate: predicate) { (success, count, _) -> Void in
                // error is expected and ignored if protected data is unavailable
                if success {
                    self.log.debug(
                        "Successfully purged %d HealthKit objects older than %{public}@",
                        count,
    ...
    private func handleUpdatedGlucoseData() {
        ...
        purgeExpiredCachedGlucoseObjects()
        updateLatestGlucose()
        saveSamplesToHealthKit()
...
public extension GlucoseStore {
    ...
    func getRecentMomentumEffect(
        for date: Date? = nil,
        _ completion: @escaping (_ result: Result<[GlucoseEffect], Error>) -> Void
    ) {
    ...
    func getCounteractionEffects(
        start: Date,
        end: Date? = nil,
        to effects: [GlucoseEffect],
        _ completion: @escaping (_ result: Result<[GlucoseEffectVelocity], Error>) -> Void
    ) {
    ...
    func counteractionEffects<Sample: GlucoseSampleValue>(
        for samples: [Sample],
        to effects: [GlucoseEffect]
...
public extension GlucoseStore {
    ...
    enum GlucoseQueryResult {
        case success(QueryAnchor, [StoredGlucoseSample])
        case failure(Error)
    }

    func executeGlucoseQuery(
        fromQueryAnchor queryAnchor: QueryAnchor?,
        limit: Int,
        completion: @escaping (GlucoseQueryResult) -> Void
    ) {
        queue.async {
...
public extension GlucoseStore {
    func addNewGlucoseSamples(samples: [NewGlucoseSample], completion: @escaping (Error?) -> Void) {
        guard !samples.isEmpty else {
            completion(nil)
            return
        }
