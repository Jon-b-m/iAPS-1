...
class CachedGlucoseObject: NSManagedObject {
    ...
    var device: HKDevice? {
        ...
        set {
            ...
            primitiveDevice = newValue
                .flatMap { try? NSKeyedArchiver.archivedData(withRootObject: $0, requiringSecureCoding: false) }
        }
    }
    ...
    var condition: GlucoseCondition? {
        ...
        set {
            ...
            primitiveCondition = newValue.map(\.rawValue)
        }
    }
...
extension CachedGlucoseObject {
    ...
    func create(from sample: NewGlucoseSample, provenanceIdentifier: String, healthKitStorageDelay: TimeInterval?) {
        ...
        healthKitEligibleDate = healthKitStorageDelay.map { sample.date.addingTimeInterval($0) }
    }
...
extension CachedGlucoseObject {
    func update(from sample: StoredGlucoseSample) {
        ...
        trend = sample.trend
        trendRate = sample.trendRate
        healthKitEligibleDate = sample.healthKitEligibleDate
    }
}
