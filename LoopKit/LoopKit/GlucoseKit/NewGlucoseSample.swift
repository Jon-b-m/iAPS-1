...
public struct NewGlucoseSample: Equatable, RawRepresentable {
    ...
    public init?(rawValue: [String: Any]) {
        guard let dateInterval = rawValue["dateInterval"] as? TimeInterval,
              ...
              let syncVersion = rawValue["syncVersion"] as? Int
        else {
            return nil
        }

        date = Date(timeIntervalSince1970: dateInterval)
        quantity = HKQuantity(unit: .milligramsPerDeciliter, doubleValue: quantityValue)
        condition = rawValue["condition"] as? GlucoseCondition ?? nil
        trend = rawValue["trend"] as? GlucoseTrend ?? nil
