...
public class SettingsStore {
    ...
    public init(store: PersistenceController, expireAfter: TimeInterval) {
        ...
        lockedLatestSettings = Locked(nil)
    ...
    public var expireDate: Date {
        Date(timeIntervalSinceNow: -expireAfter)
    }
    ...
    private func purgeSettingsObjects(before date: Date, completion: ((Error?) -> Void)? = nil) {
        ...
        store.managedObjectContext.performAndWait {
            do {
                let count = try self.store.managedObjectContext.purgeObjects(
                    of: SettingsObject.self,
                    matching: NSPredicate(format: "date < %@", date as NSDate)
                )
                if count > 0 {
    ...
    private func encodeSettings(_ settings: StoredSettings) -> Data? {
        do {
            ...
            log.error("Error encoding StoredSettings: %@", String(describing: error))
            return nil
        }
    }
    ...
    private func decodeSettings(fromData data: Data) -> StoredSettings? {
        do {
            ...
            log.error("Error decoding StoredSettings: %@", String(describing: error))
            return nil
        }
    }
}
...
public extension SettingsStore {
    ...
    func getStoredSettings(start: Date? = nil, end: Date? = nil, limit: Int? = nil) async throws -> [StoredSettings] {
    ...
    func executeSettingsQuery(
        fromQueryAnchor queryAnchor: QueryAnchor?,
        limit: Int,
        completion: @escaping (SettingsQueryResult) -> Void
    ) {
        dataAccessQueue.async {
...
extension StoredSettings: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.init(
            ...
            glucoseTargetRangeSchedule: try container.decodeIfPresent(
                GlucoseRangeSchedule.self,
                forKey: .glucoseTargetRangeSchedule
            ),
            preMealTargetRange: try container.decodeIfPresent(DoubleRange.self, forKey: .preMealTargetRange)?
                .quantityRange(for: bloodGlucoseUnit),
            workoutTargetRange: try container.decodeIfPresent(DoubleRange.self, forKey: .workoutTargetRange)?
                .quantityRange(for: bloodGlucoseUnit),
            overridePresets: try container.decodeIfPresent(
                [TemporaryScheduleOverridePreset].self,
                forKey: .overridePresets
            ),
            ...
            defaultRapidActingModel: try container.decodeIfPresent(
                StoredInsulinModel.self,
            ...
            insulinSensitivitySchedule: try container.decodeIfPresent(
                InsulinSensitivitySchedule.self,
                forKey: .insulinSensitivitySchedule
            ),
            ...
            automaticDosingStrategy: try container.decodeIfPresent(
                AutomaticDosingStrategy.self,
                forKey: .automaticDosingStrategy
            ) ?? .tempBasalOnly,
...
public extension SettingsStore {
    func addStoredSettings(settings: [StoredSettings], completion: @escaping (Error?) -> Void) {
