...
public extension Guardrail where Value == HKQuantity {
    static let suspendThreshold = Guardrail(
        absoluteBounds: 66.1 ... 110.9,
        recommendedBounds: 73.1 ... 80.9,
        unit: .milligramsPerDeciliter,
        startingSuggestion: 80
    )

    static func maxSuspendThresholdValue(
        correctionRangeSchedule: GlucoseRangeSchedule?,
        preMealTargetRange: ClosedRange<HKQuantity>?,
        workoutTargetRange: ClosedRange<HKQuantity>?
    ) -> HKQuantity {
        [
    ...
    static let correctionRange = Guardrail(
        absoluteBounds: 86.1 ... 180.5,
        recommendedBounds: 99.1 ... 115.9,
        unit: .milligramsPerDeciliter,
    ...
    fileprivate static func preMealCorrectionRange(
        correctionRangeScheduleRange: ClosedRange<HKQuantity>,
        suspendThreshold: GlucoseThreshold?
    ) -> Guardrail<HKQuantity> {
        ...
        return Guardrail(
            ...
            recommendedBounds: absoluteLowerBound ... min(
                max(absoluteLowerBound, correctionRangeScheduleRange.lowerBound),
                premealCorrectionRangeMaximum
            )
        )
    }

    static func correctionRangeOverride(
        for preset: CorrectionRangeOverrides.Preset,
        correctionRangeScheduleRange: ClosedRange<HKQuantity>,
        suspendThreshold: GlucoseThreshold?
    ) -> Guardrail {
        switch preset {
        case .workout:
            return workoutCorrectionRange(
                correctionRangeScheduleRange: correctionRangeScheduleRange,
                suspendThreshold: suspendThreshold
            )
        case .preMeal:
            return preMealCorrectionRange(
                correctionRangeScheduleRange: correctionRangeScheduleRange,
                suspendThreshold: suspendThreshold
            )
        }
    ...
    static func maximumBasalRate(
        ...
        let absoluteUpperBound = maximumUpperBound.matchingOrTruncatedValue(
            from: supportedBasalRates,
            withinDecimalPlaces: decimalPlaces
        )
        ...
        if let highestScheduledBasalRate = scheduledBasalRange?.upperBound {
            recommendedLowerBound = (recommendedLowScheduledBasalScaleFactor * highestScheduledBasalRate)
    ...
    static func selectableMaxBasalRates(
        supportedBasalRates: [Double],
        scheduledBasalRange: ClosedRange<Double>?,
        lowestCarbRatio: Double?,
        maximumBasalRatePrecision _: Int = 3
    ) -> [Double] {
        let basalGuardrail = Guardrail.maximumBasalRate(
