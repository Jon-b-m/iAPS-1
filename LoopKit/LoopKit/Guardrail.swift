...
public struct Guardrail<Value: Comparable> {
    ...
    public init(absoluteBounds: ClosedRange<Value>, recommendedBounds: ClosedRange<Value>, startingSuggestion: Value? = nil) {
        precondition(
            absoluteBounds.lowerBound <= recommendedBounds.lowerBound,
            "The minimum value must be less than or equal to the smallest recommended value"
        )
        precondition(
            absoluteBounds.upperBound >= recommendedBounds.upperBound,
            "The maximum value must be greater than or equal to the greatest recommended value"
        )
...
public extension Guardrail where Value: Strideable {
    func allValues(stridingBy increment: Value.Stride) -> StrideThrough<Value> {
...
public extension Guardrail where Value == HKQuantity {
    init(
        absoluteBounds: ClosedRange<Double>,
        recommendedBounds: ClosedRange<Double>,
        unit: HKUnit,
        startingSuggestion: Double? = nil
    ) {
        let absoluteBoundsWithUnit = HKQuantity(unit: unit, doubleValue: absoluteBounds.lowerBound) ...
            HKQuantity(unit: unit, doubleValue: absoluteBounds.upperBound)
        let recommendedBoundsWithUnit = HKQuantity(unit: unit, doubleValue: recommendedBounds.lowerBound) ...
            HKQuantity(unit: unit, doubleValue: recommendedBounds.upperBound)
        ...
        self.init(
            absoluteBounds: absoluteBoundsWithUnit,
            recommendedBounds: recommendedBoundsWithUnit,
            startingSuggestion: startingSuggestionQuantity
        )
    }

    /// if fractionDigits is nil, defaults to the unit maxFractionDigits
    func allQuantities(forUnit unit: HKUnit, usingFractionDigits fractionDigits: Int? = nil) -> [HKQuantity] {
        allValues(forUnit: unit, usingFractionDigits: fractionDigits ?? unit.maxFractionDigits)
            .map { HKQuantity(unit: unit, doubleValue: $0) }
    }

    /// if fractionDigits is nil, defaults to the unit maxFractionDigits
    func allValues(forUnit unit: HKUnit, usingFractionDigits fractionDigits: Int? = nil) -> [Double] {
        unit.allValues(
            from: absoluteBounds.lowerBound,
            through: absoluteBounds.upperBound,
            usingFractionDigits: fractionDigits ?? unit.maxFractionDigits
        )
    }
}
