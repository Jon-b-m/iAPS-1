...
public extension CarbAbsorptionComputable {
    ...
    internal func timeToAbsorb(forPercentAbsorbed percentAbsorption: Double, totalAbsorptionTime: TimeInterval) -> TimeInterval {
...
public struct PiecewiseLinearAbsorption: CarbAbsorptionComputable {
    ...
    var scale: Double {
        2.0 / (1.0 + percentStartOfFall - percentEndOfRise)
    }
    ...
    public func percentAbsorptionAtPercentTime(_ percentTime: Double) -> Double {
        ...
        case let t where t >= percentStartOfFall && t < 1.0:
            return scale * (
                percentStartOfFall - 0.5 * percentEndOfRise +
...
extension CarbEntry {
    func carbsOnBoard(
        ...
        if time >= 0 {
            value = absorptionModel.unabsorbedCarbs(
                of: quantity.doubleValue(for: HKUnit.gram()),
                atTime: time - delay,
                absorptionTime: absorptionTime ?? defaultAbsorptionTime
            )
        } else {
            value = 0
        }
    ...
    fileprivate func glucoseEffect(
        ...
        insulinSensitivity.doubleValue(for: HKUnit.milligramsPerDeciliter) / carbRatio.doubleValue(for: .gram()) * absorbedCarbs(
            at: date,
            absorptionTime: absorptionTime ?? defaultAbsorptionTime,
            delay: delay,
    ...
    private func estimatedAbsorptionTime(
        forAbsorbedCarbs carbs: Double,
        at date: Date,
        absorptionModel: CarbAbsorptionComputable
    ) -> TimeInterval {
        let time = date.timeIntervalSince(startDate)
        ...
        return max(
            time,
            absorptionModel.absorptionTime(forPercentAbsorption: carbs / quantity.doubleValue(for: .gram()), atTime: time)
        )
    }
}
    ...
    fileprivate func simulationDateRange(
        ...
        if let start = start, let end = end {
            ...
            for sample in self {
                ...
                let endDate = sample.endDate.addingTimeInterval(sample.absorptionTime ?? defaultAbsorptionTime)
...
public extension Collection {
    func dynamicCarbsOnBoard<T>(
        ...
        repeat {
            let value = reduce(0.0) { (value, entry) -> Double in
                value + entry.dynamicCarbsOnBoard(
                    at: date,
                    defaultAbsorptionTime: defaultAbsorptionTime,
                    delay: delay,
                    delta: delta,
    ...
    func dynamicGlucoseEffects<T>(
        ...
        repeat {
            let value = reduce(0.0) { (value, entry) -> Double in
                guard let isf = insulinSensitivities.closestPrior(to: entry.startDate),
                      let cr = carbRatios.closestPrior(to: entry.startDate)
                else {
    ...
    func getClampedCarbsOnBoard<T>() -> CarbValue? where Element == CarbStatus<T> {
...
private class CarbStatusBuilder<T: CarbEntry> {
    // MARK: Model settings
    ...
    private var adaptiveRateStandbyInterval: TimeInterval {
        initialAbsorptionTime * adaptiveRateStandbyIntervalFraction
    }
    ...
    private var observedAbsorptionDates: DateInterval {
        DateInterval(start: entry.startDate, end: observedCompletionDate ?? lastEffectDate)
    }
    ...
    private var timeToAbsorbObservedCarbs: TimeInterval {
        ...
        if adaptiveAbsorptionRateEnabled, time > adaptiveRateStandbyInterval {
            ...
            timeToAbsorb = absorptionModel.timeToAbsorb(
                forPercentAbsorbed: percentAbsorbed,
                totalAbsorptionTime: initialAbsorptionTime
            )
        }
    ...
    private var clampedTimeline: [CarbValue]? {
        observedGrams >= minPredictedGrams ? observedTimeline : nil
    }
    ...
    init(
        ...
        initialObservedEffect: Double = 0
    ) {
        self.entry = entry
        ...
        self.delay = delay
        observedEffect = initialObservedEffect
        self.absorptionModel = absorptionModel
        self.adaptiveAbsorptionRateEnabled = adaptiveAbsorptionRateEnabled
        self.adaptiveRateStandbyIntervalFraction = adaptiveRateStandbyIntervalFraction
        entryGrams = entry.quantity.doubleValue(for: carbUnit)
        entryEffect = entryGrams * carbohydrateSensitivityFactor
        maxEndDate = entry.startDate.addingTimeInterval(maxAbsorptionTime + delay)
...
public extension Collection where Element: CarbEntry {
    ...
    func map(
        ...
        let builders: [CarbStatusBuilder<Element>] = map { entry in
            guard
                ...
                let entryInsulinSensitivity = insulinSensitivity.closestPrior(to: entry.startDate)
            else {
        ...
        for dxEffect in effectVelocities {
            ...
            let activeBuilders = builders.filter { (builder) -> Bool in
                dxEffect.startDate < builder.maxEndDate && dxEffect.startDate >= builder.entry.startDate
            }
            ...
            for builder in activeBuilders {
                ...
                if totalRate > 0 {
                    partialEffectValue = Swift.min(
                        builder.remainingEffect,
                        (absorptionRateAtEffectTime / totalRate) * effectValue
                    )
