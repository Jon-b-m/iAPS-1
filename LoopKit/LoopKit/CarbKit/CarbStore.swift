...
public final class CarbStore {
    ...
    public enum CarbStoreError: Error {
        ...
        init?(error: PersistenceController.PersistenceControllerError?) {
            guard let error = error, case let .coreDataError(coreDataError) = error else {
                return nil
            }
    ...
    public var carbSensitivitySchedule: CarbSensitivitySchedule? {
        ...
        return .carbSensitivitySchedule(
    ...
    public init(
        ...
        lockedCarbRatioSchedule = Locked(carbRatioSchedule)
        ...
        case .linear:
            settings = CarbModelSettings(
                absorptionModel: LinearAbsorption(),
                initialAbsorptionTimeOverrun: absorptionTimeOverrun,
                adaptiveAbsorptionRateEnabled: false
            )
        case .nonlinear:
            settings = CarbModelSettings(
...
public extension CarbStore {
    ...
    func getCarbEntries(start: Date? = nil, end: Date? = nil) async throws -> [StoredCarbEntry] {
    ...
    func getCarbEntries(
        ...
        completion: @escaping (_ result: CarbStoreResult<[StoredCarbEntry]>) -> Void
    ) {
        queue.async {
    ...
    func getCarbStatus(
        ...
        let now = Date()
        guard let carbRatio = carbRatioScheduleApplyingOverrideHistory?.between(start: start, end: end ?? now),
              let insulinSensitivity = insulinSensitivityScheduleApplyingOverrideHistory?.quantitiesBetween(
                  start: start,
                  end: end ?? now
              )
        else {
...
extension CarbStore {
    ...
    public func replaceCarbEntry(
        _ oldEntry: StoredCarbEntry,
        withEntry newEntry: NewCarbEntry,
        completion: @escaping (_ result: CarbStoreResult<StoredCarbEntry>) -> Void
    ) {
        ...
        queue.async {
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                do {
                    guard let oldObject = try self.cacheStore.managedObjectContext.cachedCarbObjectFromStoredCarbEntry(oldEntry)
                    else {
                        error = .noData
                        return
                    }
    ...
    private func saveEntryToHealthKit(_ object: CachedCarbObject) {
        ...
        if let error = error {
            log.error("Error saving HealthKit object: %@", String(describing: error))
            return
        }
        ...
        if let error = cacheStore.save() {
            log.error("Error updating CachedCarbObject after saving HealthKit object: %@", String(describing: error))
            object.uuid = nil
        }
    }
    ...
    private func deleteObjectFromHealthKit(_ object: CachedCarbObject) {
        ...
        hkSampleStore.healthStore.deleteObjects(
            of: HealthKitSampleStore.carbType,
            predicate: HKQuery.predicateForObject(with: object.uuid!)
        ...
        if let error = error {
            log.error("Error deleting HealthKit object: %@", String(describing: error))
            return
        }
        ...
        object.uuid = nil
        if let error = cacheStore.save() {
            log.error("Error updating CachedCarbObject after deleting HealthKit object: %@", String(describing: error))
        }
    }
    ...
    private func addCarbEntry(for sample: HKQuantitySample, on date: Date) throws -> Bool {
        ...
        let replacedObjects = try fetchRelatedCarbObjects(
    ...
    public func fetchLatestCarbEntry() async throws -> StoredCarbEntry? {
        try await withCheckedThrowingContinuation({ continuation in
            queue.async {
                ...
                do {
                    let entries = try self.cacheStore.managedObjectContext.fetch(request)
                        .compactMap { StoredCarbEntry(managedObject: $0) }
...
public extension CarbStore {
    ...
    func getSyncCarbObjects(
        start: Date? = nil,
        end: Date? = nil,
        completion: @escaping (_ result: CarbStoreResult<[SyncCarbObject]>) -> Void
    ) {
        queue.async {
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                do {
                    objects = try self.getActiveCachedCarbObjects(start: start, end: end)
                        .map { SyncCarbObject(managedObject: $0) }
                } catch let coreDataError {
                    error = .coreDataError(coreDataError)
                }
            }
    ...
    func syncCarbObjects(_ objects: [SyncCarbObject]) async throws {
    ...
    func syncCarbObjects(_ objects: [SyncCarbObject], completion: @escaping (CarbStoreError?) -> Void) {
        queue.async {
            var error: CarbStoreError?
            ...
            self.cacheStore.managedObjectContext.performAndWait {
                ...
                do {
                    try objects.forEach {
                        let object = try self.fetchRelatedCarbObjects(
                            syncIdentifier: $0.syncIdentifier,
    ...
    func setSyncCarbObjects(_ objects: [SyncCarbObject], completion: @escaping (CarbStoreError?) -> Void) {
        queue.async {
...
extension CarbStore {
    ...
    @discardableResult private func purgeCachedCarbObjects(before date: Date) -> CarbStoreError? {
        dispatchPrecondition(condition: .onQueue(queue))
        ...
        if let error = error {
            log.error("Unable to purge CachedCarbObjects: %{public}@", String(describing: error))
            return error
        }
...
public extension CarbStore {
    ...
    func carbsOnBoard(
        at date: Date,
        effectVelocities: [GlucoseEffectVelocity]? = nil,
        completion: @escaping (_ result: CarbStoreResult<CarbValue>) -> Void
    ) {
    ...
    func getCarbsOnBoardValues(
        ...
        getCarbEntries(start: foodStart, end: end) { result in
            ...
            case let .failure(error):
                completion(.failure(error))
            case let .success(entries):
                let carbsOnBoard = self.carbsOnBoard(
                    from: entries,
                    startingAt: start,
                    endingAt: end,
                    effectVelocities: effectVelocities
                )
    ...
    func carbsOnBoard<Sample: CarbEntry>(
        from samples: [Sample],
        ...
        if let velocities = effectVelocities
        {
            ...
            let carbDates = samples.map(\.startDate)
            let maxCarbDate = carbDates.max()!
            let minCarbDate = carbDates.min()!

            guard let carbRatio = carbRatioScheduleApplyingOverrideHistory?.between(start: minCarbDate, end: maxCarbDate),
                  let insulinSensitivity = insulinSensitivityScheduleApplyingOverrideHistory?.quantitiesBetween(
                      start: minCarbDate,
                      end: maxCarbDate
                  )
    ...
    func carbsOnBoard<Sample: CarbEntry>(
        ...
        let values = carbsOnBoard(
            from: samples,
            startingAt: date.addingTimeInterval(-delta),
            endingAt: date,
            effectVelocities: effectVelocities
        )
    ...
    func getGlucoseEffects(
        ...
        completion: @escaping (_ result: CarbStoreResult<(entries: [StoredCarbEntry], effects: [GlucoseEffect])>) -> Void
    ) {
        queue.async {
            ...
            self.getCarbEntries(start: foodStart, end: end) { result in
                ...
                case let .success(entries):
                    do {
                        let effects = try self.glucoseEffects(
                            of: entries,
                            startingAt: start,
                            endingAt: end,
                            effectVelocities: effectVelocities
    ...
    ///   - end: The latest date of effects to retrieve, if provided
    ///   - effectVelocities: A timeline of glucose effect velocities, ordered by start date
    func glucoseEffects<Sample: CarbEntry>(
        of samples: [Sample],
        startingAt start: Date,
        endingAt end: Date? = nil,
        effectVelocities: [GlucoseEffectVelocity]
...
public extension CarbStore {
    struct QueryAnchor: Equatable, RawRepresentable {
        ...
        public init?(rawValue: RawValue) {
            guard let anchorKey = (rawValue["anchorKey"] ?? rawValue["storedModificationCounter"]) as? Int64
            else { // Backwards compatibility with storedModificationCounter
                return nil
            }
            ...
            cacheStore.managedObjectContext.performAndWait {
                do {
                    ...
                    request
                        .predicate =
                        NSCompoundPredicate(andPredicateWithSubpredicates: [
                            NSPredicate(format: "anchorKey > %d", anchorKey),
    ...
    private func exportDatePredicate(startDate: Date, endDate: Date? = nil) -> NSPredicate {
        ...
        if let endDate = endDate {
            addedDatePredicate =
                NSCompoundPredicate(andPredicateWithSubpredicates: [
                    addedDatePredicate,
                    NSPredicate(format: "addedDate < %@", endDate as NSDate)
                ])
            supercededDatePredicate =
                NSCompoundPredicate(andPredicateWithSubpredicates: [
                    supercededDatePredicate,
                    NSPredicate(format: "supercededDate < %@", endDate as NSDate)
                ])
        }
...
public extension CarbStore {
    func addNewCarbEntries(entries: [NewCarbEntry], completion: @escaping (Error?) -> Void) {
