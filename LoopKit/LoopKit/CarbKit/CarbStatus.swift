...
extension CarbStatus: CarbEntry {
    public var absorptionTime: TimeInterval? {
        absorption?.estimatedDate.duration ?? entry.absorptionTime
    }
}
...
extension CarbStatus {
    func dynamicCarbsOnBoard(
        ...
        guard date <= observationEnd else {
            ...
            let total = absorption.total.doubleValue(for: unit)
            let unabsorbedAtEffectiveTime = absorptionModel.unabsorbedCarbs(
                of: total,
        ...
        return max(observedTimeline.filter({ $0.endDate <= date }).reduce(total) { (total, value) -> Double in
            total - value.quantity.doubleValue(for: unit)
        }, 0)
    }

    func dynamicAbsorbedCarbs(
        at date: Date,
        ...
        guard date <= observationEnd else {
            ...
            let effectiveAbsorptionTime = absorption.timeToAbsorbObservedCarbs + absorption.estimatedTimeRemaining
            let total = absorption.total.doubleValue(for: unit)
            let absorbedAtEffectiveTime = absorptionModel.absorbedCarbs(
                of: total,
                atTime: effectiveTime,
                absorptionTime: effectiveAbsorptionTime
            )
        ...
        return min(beforeDate.reduce(sum) { (sum, value) -> Double in
            sum + value.quantity.doubleValue(for: unit)
