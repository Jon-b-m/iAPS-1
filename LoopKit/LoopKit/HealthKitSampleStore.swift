...
extension HKHealthStore: HKHealthStoreProtocol {
    public func cachedPreferredUnits(for quantityTypeIdentifier: HKQuantityTypeIdentifier) async -> HKUnit? {
        HealthStoreUnitCache.unitCache(for: self).preferredUnit(for: quantityTypeIdentifier)
    }
}
...
public protocol HealthKitSampleStoreDelegate {
    ...
    func processResults(
        from query: HKAnchoredObjectQuery,
        added: [HKSample],
...
public extension Notification.Name {
    static let StoreAuthorizationStatusDidChange = Notification
        .Name(rawValue: "com.loudnate.LoopKit.AuthorizationStatusDidChangeNotification")
}
...
public class HealthKitSampleStore {
    ...
    public init(
        ...
        lockedQueryState = Locked<QueryState>(QueryState(anchorState: .uninitialized, authorizationDetermined: false))

        log = OSLog(category: String(describing: Swift.type(of: self)))
    }
    ...
    private enum QueryAnchorState: Equatable {
        ...
        var anchor: HKQueryAnchor? {
            ...
            case .uninitialized:
                return nil
            case let .initializationComplete(anchor):
                return anchor
            }
        }
    }
    ...
    internal var queryAnchor: HKQueryAnchor? {
        get {
            if case let .initializationComplete(anchor) = queryState.anchorState {
                return anchor
            } else {
                return nil
            }
    ...
    private struct QueryState: Equatable {
        ...
        var readyToQuery: Bool {
            anchorState != .uninitialized && authorizationDetermined
        }
    }
    ...
    private var queryState: QueryState {
        lockedQueryState.value
    }

    @discardableResult private func mutateQueryState(_ changes: (_ state: inout QueryState) -> Void) -> QueryState {
        setQueryStateWithResult({ (state) -> QueryState in
            changes(&state)
            return state
        })
    }
    ...
    internal final func observerQueryHandler(
        query: HKObserverQuery,
        observerQueryCompletionHandler: @escaping HKObserverQueryCompletionHandler,
        error: Error?
    ) {
        if let error = error {
        ...
        log.default(
            "%{public}@ notified with changes. Fetching from: %{public}@",
            query,
            queryAnchor.map(String.init(describing:)) ?? "0"
        )
        executeAnchorQuery(observerQuery: query, observerQueryCompletionHandler: observerQueryCompletionHandler)
    }

    internal final func executeAnchorQuery(
        observerQuery: HKObserverQuery,
        observerQueryCompletionHandler: @escaping HKObserverQueryCompletionHandler
    ) {
        let batchSize = 500

        let anchoredObjectQuery = anchoredObjectQueryType.init(
            type: sampleType,
            predicate: observerQuery.predicate,
            anchor: queryAnchor,
            limit: batchSize
        ) { [weak self] query, newSamples, deletedSamples, anchor, error in
            ...
            if let error = error {
                self.log.error(
                    "HKQuery: Error from anchoredObjectQuery: anchor: %{public}@ error: %{public}@",
            ...
            self.log.default(
                "AnchorQuery results new: %{public}d deleted: %{public}d anchor: %{public}@",
                newSamples.count,
                ...
                delegate.processResults(from: query, added: newSamples, deleted: deletedSamples, anchor: anchor) { success in
                    if success {
                        ...
                        if newSamples.count + deletedSamples.count >= batchSize {
                            self.executeAnchorQuery(
                                observerQuery: observerQuery,
                                observerQueryCompletionHandler: observerQueryCompletionHandler
                            )
                        } else {
                            observerQueryCompletionHandler()
                        }
...
extension HealthKitSampleStore {
    func createQuery(from: String) {
        ...
        observerQuery = observerQueryType
            .init(sampleType: sampleType, predicate: predicate) { [weak self] query, completionHandler, error in
...
public extension HealthKitSampleStore {
    ...
    var sharingDenied: Bool {
        healthStore.authorizationStatus(for: sampleType) == .sharingDenied
    }
