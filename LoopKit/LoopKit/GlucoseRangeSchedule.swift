...
public struct DoubleRange {
    ...
    public var isZero: Bool {
        abs(minValue) < .ulpOfOne && abs(maxValue) < .ulpOfOne
    }
}
...
extension DoubleRange: Equatable {
    public static func == (lhs: DoubleRange, rhs: DoubleRange) -> Bool {
        abs(lhs.minValue - rhs.minValue) < .ulpOfOne &&
...
public struct GlucoseRangeSchedule: DailySchedule, Equatable {
    ...
    public struct Override: Equatable {
        ...
        public func isActive(at date: Date = Date()) -> Bool {
            activeDates.contains(date) && !value.isZero
        }
    }
    ...
    public init?(unit: HKUnit, dailyItems: [RepeatingScheduleValue<DoubleRange>], timeZone: TimeZone? = nil) {
        guard let rangeSchedule = DailyQuantitySchedule<DoubleRange>(unit: unit, dailyItems: dailyItems, timeZone: timeZone)
        else {
            return nil
        }
    ...
    public func between(start startDate: Date, end endDate: Date) -> [AbsoluteScheduleValue<DoubleRange>] {
        rangeSchedule.between(start: startDate, end: endDate)
    }
    ...
    public func quantityRange(at time: Date) -> ClosedRange<HKQuantity> {
        value(at: time).quantityRange(for: unit)
    }
    ...
    public var quantityRanges: [RepeatingScheduleValue<ClosedRange<HKQuantity>>] {
        items.map {
            RepeatingScheduleValue<ClosedRange<HKQuantity>>(
    ...
    public var rawValue: RawValue {
        rangeSchedule.rawValue
    }

    public func minLowerBound() -> HKQuantity {
        let minDoubleValue = items.lazy.map(\.value.minValue).min()!
    ...
    public func scheduleRange() -> ClosedRange<HKQuantity> {
        ...
        let maxDoubleValue = items.lazy.map(\.value.maxValue).max()!
...
public extension ClosedRange where Bound == HKQuantity {
    func doubleRange(for unit: HKUnit) -> DoubleRange {
        DoubleRange(minValue: lowerBound.doubleValue(for: unit), maxValue: upperBound.doubleValue(for: unit))
    }

    func glucoseRange(for unit: HKUnit) -> GlucoseRange {
        GlucoseRange(range: doubleRange(for: unit), unit: unit)
    }
}
