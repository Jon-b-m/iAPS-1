...
class MasterViewController: UITableViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        {
            dataManager.healthStore.requestAuthorization(toShare: sampleTypes, read: sampleTypes) { success, _ in
                if success {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .configuration:
            ...
            case .correctionRange:
                cell.textLabel?.text = NSLocalizedString(
                    "Correction Range",
                    comment: "The title text for the glucose correction range schedule"
                )
            case .insulinSensitivity:
                cell.textLabel?.text = NSLocalizedString(
                    "Insulin Sensitivity",
                    comment: "The title text for the insulin sensitivity schedule"
                )
            case .pumpID:
        ...
        case .data:
            ...
            case .carbs:
                cell.textLabel?.text = NSLocalizedString(
                    "Carbs",
                    comment: "The title for the cell navigating to the carbs screen"
                )
            case .reservoir:
                cell.textLabel?.text = NSLocalizedString(
                    "Reservoir",
                    comment: "The title for the cell navigating to the reservoir screen"
                )
            case .diagnostic:
                cell.textLabel?.text = NSLocalizedString(
                    "Diagnostic",
                    comment: "The title for the cell displaying diagnostic data"
                )
            case .generate:
                cell.textLabel?.text = NSLocalizedString(
                    "Generate Data",
                    comment: "The title for the cell displaying data generation"
                )
            case .reset:
...
//                show(scheduleVC, sender: sender)
            ...
            case .correctionRange:
                ...
                therapySettings.glucoseTargetRangeSchedule = dataManager?.glucoseTargetRangeSchedule
                let therapySettingsViewModel = TherapySettingsViewModel(therapySettings: therapySettings)

                let view = CorrectionRangeScheduleEditor(
                    mode: .settings,
                    therapySettingsViewModel: therapySettingsViewModel,
                    didSave: {
                        self.dataManager?.glucoseTargetRangeSchedule = therapySettingsViewModel.therapySettings
                ...
                let scheduleVC = DismissibleHostingController(
                    content: view,
                    dismissalMode: .pop(to: type(of: self)),
                    isModalInPresentation: false
                )
                show(scheduleVC, sender: sender)
            case .insulinSensitivity:
                let unit = dataManager?.insulinSensitivitySchedule?.unit ?? HKUnit.milligramsPerDeciliter
                let scheduleVC = InsulinSensitivityScheduleViewController(
                    allowedValues: unit.allowedSensitivityValues,
                    unit: unit
                )
                ...
                scheduleVC.title = NSLocalizedString(
                    "Insulin Sensitivity",
                    comment: "The title of the insulin sensitivity schedule screen"
                )
                show(scheduleVC, sender: sender)
...
//                textFieldVC.delegate = self
                ...
                textFieldVC.placeholder = NSLocalizedString(
                    "Enter the 6-digit pump ID",
                    comment: "The placeholder text instructing users how to enter a pump ID"
                )
                textFieldVC.value = dataManager?.pumpID
                textFieldVC.keyboardType = .numberPad
                textFieldVC.contextHelp = NSLocalizedString(
                    "The pump ID can be found printed on the back, or near the bottom of the STATUS/Esc screen. It is the strictly numerical portion of the serial number (shown as SN or S/N).",
                    comment: "Instructions on where to find the pump ID on a Minimed pump"
                )
            ...
            case .generate:
                let vc = CommandResponseViewController(command: { [weak self] (completionHandler) -> String in
                    ...
                    reservoir: for index in sequence(first: TimeInterval(hours: -6), next: { $0 + .minutes(5) }) {
                        ...
                        group.enter()
                        dataManager.doseStore.addReservoirValue(unitVolume, at: Date(timeIntervalSinceNow: index)) { _, _, _, _ in
                            group.leave()
                        }
                    }

                    group.enter()
                    dataManager.glucoseStore.addGlucoseSamples(
                        [NewGlucoseSample(
                            date: Date(),
                            quantity: HKQuantity(unit: .milligramsPerDeciliter, doubleValue: 101),
                            condition: nil,
                            trend: nil,
                            trendRate: nil,
                            isDisplayOnly: false,
...
extension MasterViewController: DailyValueScheduleTableViewControllerDelegate {
    func dailyValueScheduleTableViewControllerWillFinishUpdating(_: DailyValueScheduleTableViewController) {
...
extension MasterViewController: InsulinSensitivityScheduleStorageDelegate {
    func saveSchedule(
        _ schedule: InsulinSensitivitySchedule,
        for _: InsulinSensitivityScheduleViewController,
        completion: @escaping (SaveInsulinSensitivityScheduleResult) -> Void
    ) {
        dataManager?.insulinSensitivitySchedule = schedule
        completion(.success)
    }
}
