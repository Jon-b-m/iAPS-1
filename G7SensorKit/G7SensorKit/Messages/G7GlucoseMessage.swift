...
public struct G7GlucoseMessage: SensorMessage, Equatable {
    ...
    public var hasReliableGlucose: Bool {
        algorithmState.hasReliableGlucose
    }
...
extension G7GlucoseMessage: CustomDebugStringConvertible {
    public var debugDescription: String {
        "G7GlucoseMessage(glucose:\(String(describing: glucose)), sequence:\(sequence) glucoseIsDisplayOnly:\(glucoseIsDisplayOnly) state:\(String(describing: algorithmState)) messageTimestamp:\(messageTimestamp) age:\(age), data:\(data.hexadecimalString))"
    }
}
