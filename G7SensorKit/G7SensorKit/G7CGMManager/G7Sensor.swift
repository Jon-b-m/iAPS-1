...
public final class G7Sensor: G7BluetoothManagerDelegate {
    ...
    private func handleGlucoseMessage(message: G7GlucoseMessage, peripheralManager: G7PeripheralManager) {
        ...
        if sensorID == nil, let name = peripheralManager.peripheral.name, let activationDate = activationDate {
            ...
            log.error("Dropping unhandled glucose message: %{public}@", String(describing: message))
        }
    }

    // MARK: - BluetoothManagerDelegate

    func bluetoothManager(_: G7BluetoothManager, readied peripheralManager: G7PeripheralManager) -> Bool {
        ...
        peripheralManager.perform { peripheral in
            self.log.info(
                "Listening for authentication responses for %{public}@",
                String(describing: peripheralManager.peripheral.name)
            )
            do {
    ...
    func bluetoothManager(_: G7BluetoothManager, readyingFailed _: G7PeripheralManager, with error: Error) {
        delegateQueue.async {
            self.delegate?.sensor(self, didError: error)
        }
    }

    func peripheralDidDisconnect(_: G7BluetoothManager, peripheralManager: G7PeripheralManager, wasRemoteDisconnect: Bool) {
        if let sensorID = sensorID, sensorID == peripheralManager.peripheral.name {
            let suspectedEndOfSession: Bool

            log.info("Sensor disconnected: wasRemoteDisconnect:%{public}@", String(describing: wasRemoteDisconnect))
    ...
    func bluetoothManager(
        ...
        didReceiveControlResponse response: Data
    ) {
            ...
            if let glucoseMessage = G7GlucoseMessage(data: response) {
                ...
                delegateQueue.async {
                    self.delegate?.sensor(
                        self,
                        didError: G7SensorError.observationError("Unable to handle glucose control response")
                    )
                }
            }
    ...
    func bluetoothManager(_: G7BluetoothManager, didReceiveBackfillResponse response: Data) {
    ...
    func bluetoothManagerScanningStatusDidChange(_: G7BluetoothManager) {
        delegateQueue.async {
...
private extension G7PeripheralManager {
