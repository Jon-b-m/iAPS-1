...
public class G7CGMManager: CGMManager {
    ...
    private func setState(_ changes: (_ state: inout G7CGMManagerState) -> Void) {
        setStateWithResult(changes)
    }

    @discardableResult private func mutateState(_ changes: (_ state: inout G7CGMManagerState) -> Void) -> G7CGMManagerState {
        setStateWithResult({ (state) -> G7CGMManagerState in
            changes(&state)
            return state
        })
    }
    ...
    public func acknowledgeAlert(alertIdentifier _: LoopKit.Alert.AlertIdentifier, completion: @escaping (Error?) -> Void) {
        completion(nil)
    }
    ...
    func logDeviceCommunication(_ message: String, type: DeviceLogEntryType = .send) {
        cgmManagerDelegate?.deviceManager(
            self,
            logEventForDeviceIdentifier: state.sensorID,
            type: type,
            message: message,
            completion: nil
        )
...
public extension G7CGMManager {
    // MARK: - G7StateObserver

    func addStateObserver(_ observer: G7StateObserver, queue: DispatchQueue) {
        g7StateObservers.insert(observer, queue: queue)
    }

    func removeStateObserver(_ observer: G7StateObserver) {
        g7StateObservers.removeElement(observer)
    }
}

extension G7CGMManager: G7SensorDelegate {
    public func sensor(_: G7Sensor, didDiscoverNewSensor name: String, activatedAt: Date) -> Bool {
    ...
    public func sensorDidConnect(_: G7Sensor, name _: String) {
        mutateState { state in
    ...
    public func sensorDisconnected(_: G7Sensor, suspectedEndOfSession: Bool) {
    ...
    public func sensor(_: G7Sensor, logComms comms: String) {
        logDeviceCommunication("Sensor comms \(comms)", type: .receive)
    }

    public func sensor(_: G7Sensor, didError error: Error) {
    ...
    public func sensorConnectionStatusDidUpdate(_: G7Sensor) {
        g7StateObservers.forEach { observer in
...
public extension G7BackfillMessage {
