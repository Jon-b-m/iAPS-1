...
extension GlucosePageError: LocalizedError {
    public var errorDescription: String? {
        ...
        case let .unknownEventType(eventType):
            return String(
    ...
    public init(pageData: Data) throws {
        ...
        func initialTimestamp() -> DateComponents? {
            ...
            while tempOffset < length {
                ...
                {
                    let offsetDate = sensorTimestampEvent.timestamp.date!
                        .addingTimeInterval(TimeInterval(minutes: relativeEventCount * 5))
                    return calendar.dateComponents([.year, .month, .day, .hour, .minute, .calendar], from: offsetDate)
                } else if !(
                    event is NineteenSomethingGlucoseEvent /* seems to be a filler byte */ ||
                        event is DataEndGlucoseEvent
                ) {
                    return nil
                }
