...
public struct MySentryAckMessageBody: MessageBody {
    ...
    public var txData: Data {
        ...
        buffer.replaceSubrange(5 ..< 5 + responseMessageTypes.count, with: responseMessageTypes.map(\.rawValue))

        return Data(buffer)
    }

    public var description: String {
        "MySentryAck(\(sequence), \(mySentryID.hexadecimalString), \(responseMessageTypes))"
    }
}
