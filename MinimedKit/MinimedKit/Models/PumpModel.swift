...
public enum PumpModel: String {
    ...
    public func estimateTempBasalProgress(
        unitsPerHour: Double,
        duration: TimeInterval,
        elapsed: TimeInterval
    ) -> (deliveredUnits: Double, progress: Double) {
    ...
    public func estimateBolusProgress(
        elapsed: TimeInterval,
        programmedUnits: Double
    ) -> (deliveredUnits: Double, progress: Double) {
        ...
        if isDeliveryRateVariable {
            if programmedUnits < 1 {
                ...
                unroundedVolume = (min(elapsed, baseDuration) + min(elapsed, overlay1Duration) + min(elapsed, overlay2Duration)) *
                    baseDeliveryRate
