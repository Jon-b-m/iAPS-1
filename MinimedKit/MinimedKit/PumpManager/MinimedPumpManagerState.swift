...
public struct MinimedPumpManagerState: RawRepresentable, Equatable {
    ...
    public var pumpSettings: PumpSettings {
        PumpSettings(pumpID: pumpID, pumpRegion: pumpRegion)
    }
    ...
    public init(
        ...
        pumpID: String,
        pumpModel: PumpModel,
        pumpFirmwareVersion: String,
        pumpRegion: PumpRegion,
        rileyLinkConnectionState: RileyLinkConnectionState?,
        timeZone: TimeZone,
    ...
    public init?(rawValue: RawValue) {
        ...
        if version == 1
        {
            ...
            {
                rileyLinkConnectionState = RileyLinkConnectionState(autoConnectIDs: Set(connectedPeripheralIDs))
            }
        } else {
            if let rawState = rawValue["rileyLinkConnectionManagerState"] as? RileyLinkConnectionState.RawValue {
                rileyLinkConnectionState = RileyLinkConnectionState(rawValue: rawState)
            }
        }
        ...
        if let isPumpSuspended = rawValue["isPumpSuspended"] as? Bool {
            ...
            if isPumpSuspended {
                ...
                suspendState = .resumed(Date())
                  ...
                  let storedSuspendState = SuspendState(rawValue: rawSuspendState)
        {
        ...
        if let rawRecentlyReconciledEvents = rawValue["recentlyReconciledEvents"] as? [ReconciledDoseMapping.RawValue] {
            ...
            recentlyReconciledEvents = Dictionary(mappings.map { ($0.eventRaw, $0) }, uniquingKeysWith: { _, new in new })
        } else {
    ...
    public var rawValue: RawValue {
        var value: [String: Any] = [
            ...
            "pendingDoses": pendingDoses.map(\.rawValue),
            "recentlyReconciledEvents": reconciliationMappings.values.map(\.rawValue)
        ]
...
extension MinimedPumpManagerState {
    static let idleListeningEnabledDefaults: RileyLinkBluetoothDevice.IdleListeningState = .enabled(
        timeout: .minutes(4),
        channel: 0
    )
}
