...
public class MinimedPumpManager: RileyLinkPumpManager {
    ...
    public init(
        ...
        self.rileyLinkDeviceProvider.idleListeningState = idleListeningEnabled ? MinimedPumpManagerState
            .idleListeningEnabledDefaults : .disabled
    }

    public required convenience init?(rawState: PumpManager.RawStateValue) {
        guard let state = MinimedPumpManagerState(rawValue: rawState),
              let connectionManagerState = state.rileyLinkConnectionState
        else {
            return nil
        }
    ...
    private func setState(_ changes: (_ state: inout MinimedPumpManagerState) -> Void) {
    ...
    private func mutateState(_ changes: (_ state: inout MinimedPumpManagerState) -> Void) -> MinimedPumpManagerState {
        setStateWithResult({ (state) -> MinimedPumpManagerState in
            changes(&state)
            return state
        })
    }
    ...
    private func logDeviceCommunication(_ message: String, type: DeviceLogEntryType = .send) {
        ...
        pumpDelegate.delegate?.deviceManager(
            self,
            logEventForDeviceIdentifier: state.pumpID,
            type: type,
            message: message,
            completion: nil
        )
    ...
    override public var rileyLinkConnectionManagerState: RileyLinkConnectionState? {
        get {
            state.rileyLinkConnectionState
        }
        set {
            setState { state in
    ...
    override public func device(_ device: RileyLinkDevice, didReceivePacket packet: RFPacket) {
        ...
        case let body as MySentryAlertMessageBody:
            log.default("MySentry Alert: %{public}@", String(describing: body))
        case let body as MySentryAlertClearedMessageBody:
            log.default("MySentry Alert Cleared: %{public}@", String(describing: body))
        default:
            log.error("Unknown MySentry Message: %d: %{public}@", message.messageType.rawValue, message.txData.hexadecimalString)
        }
    }
...
extension MinimedPumpManager {
        ...
        if lastTuned.timeIntervalSinceNow <= -tuneTolerance {
            pumpOps.runSession(withName: "Tune pump", using: device) { session in
                do {
                    ...
                    self.log.default(
                        "Device %{public}@ auto-tuned to %{public}@ MHz",
                        device.name ?? "",
                        String(describing: scanResult.bestFrequency)
                    )
                } catch {
                    self.log.error(
                        "Device %{public}@ auto-tune failed with error: %{public}@",
                        device.name ?? "",
                    ...
                    if let error = error as? LocalizedError {
                        self.pumpDelegate.notify { delegate in
                            delegate?.pumpManager(
                                self,
                                didError: PumpManagerError.communication(MinimedPumpManagerError.tuneFailed(error))
                            )
                        }
                    }
                }
    ...
    private func runSuspendResumeOnSession(
        suspendResumeState: SuspendResumeMessageBody.SuspendResumeState,
        session: PumpOpsSession,
        insulinType: InsulinType
    ) throws {
        defer { self.recents.suspendEngageState = .stable }
        recents.suspendEngageState = suspendResumeState == .suspend ? .engaging : .disengaging
    ...
    private func setSuspendResumeState(
        state: SuspendResumeMessageBody.SuspendResumeState,
        insulinType: InsulinType,
        completion: @escaping (MinimedPumpManagerError?) -> Void
    ) {
    ...
    private func updatePumpStatus(_ status: MySentryPumpStatusMessageBody, from device: RileyLinkDevice) {
        ...
        guard abs(pumpDate.timeIntervalSinceNow) < .minutes(5) else {
            log.error(
                "Ignored MySentry status due to date mismatch: %{public}@ in %{public}",
                String(describing: pumpDate),
        ...
        case let .active(glucose: glucose):
    ...
    public func buildPumpStatusHighlight(
        for state: MinimedPumpManagerState,
    ...
    private static var pumpBatteryLowAlertIdentifier: Alert.Identifier {
        Alert.Identifier(managerIdentifier: pluginIdentifier, alertIdentifier: "PumpBatteryLow")
    }
    ...
    private var pumpBatteryLowAlert: Alert {
        ...
        let body = LocalizedString(
            "Change the pump battery immediately",
            comment: "The notification alert describing a low pump battery"
        )
        let content = Alert.Content(
            title: title,
            body: body,
            acknowledgeActionButtonLabel: LocalizedString("Dismiss", comment: "Default alert dismissal")
        )
        return Alert(
            identifier: Self.pumpBatteryLowAlertIdentifier,
            foregroundContent: content,
            backgroundContent: content,
            trigger: .immediate
        )
    ...
    private func checkPumpBattery(oldBatteryPercentage: Double?, newBatteryPercentage: Double?) {
        ...
        if let oldBatteryPercentage = oldBatteryPercentage,
           newBatteryPercentage - oldBatteryPercentage >= batteryReplacementDetectionThreshold
        {
            pumpDelegate.notify { delegate in
    ...
    private static var pumpReservoirEmptyAlertIdentifier: Alert.Identifier {
        Alert.Identifier(managerIdentifier: pluginIdentifier, alertIdentifier: "PumpReservoirEmpty")
    }
    ...
    private var pumpReservoirEmptyAlert: Alert {
        ...
        let body = LocalizedString(
            "Change the pump reservoir now",
            comment: "The notification alert describing an empty pump reservoir"
        )
        let content = Alert.Content(
            title: title,
            body: body,
            acknowledgeActionButtonLabel: LocalizedString("Ok", comment: "Default alert dismissal")
        )
        return Alert(
            identifier: Self.pumpReservoirEmptyAlertIdentifier,
            foregroundContent: content,
            backgroundContent: content,
            trigger: .immediate
        )
    ...
    private static var pumpReservoirLowAlertIdentifier: Alert.Identifier {
        Alert.Identifier(managerIdentifier: pluginIdentifier, alertIdentifier: "PumpReservoirLow")
    }
    ...
    private func pumpReservoirLowAlertForAmount(_ units: Double, andTimeRemaining remaining: TimeInterval?) -> Alert {
        ...
        if let remaining = remaining, let timeString = intervalFormatter.string(from: remaining) {
            body = String(
                format: LocalizedString(
                    "%1$@ U left: %2$@",
                    comment: "Low reservoir alert with time remaining format string. (1: Number of units remaining)(2: approximate time remaining)"
                ),
                unitsString,
                timeString
            )
            ...
            body = String(
                format: LocalizedString(
                    "%1$@ U left",
                    comment: "Low reservoir alert format string. (1: Number of units remaining)"
                ),
                unitsString
            )
        }
        ...
        return Alert(
            identifier: Self.pumpReservoirLowAlertIdentifier,
            foregroundContent: content,
            backgroundContent: content,
            trigger: .immediate
        )
    ...
    private func evaluateReservoirAlerts(
        lastValue: ReservoirValue?,
        newValue: ReservoirValue,
        areStoredValuesContinuous _: Bool
    ) {
    ...
    static func reconcilePendingDosesWith(
        ...
        )
    {
        ...
        func addReconciliationMapping(startTime: Date, uuid: UUID, eventRaw: Data, index _: Int) {
        ...
        let remainingPumpEvents = events.filter { (event) -> Bool in
            newReconciliationMapping[event.raw] == nil
        }
    ...
    private func reconcilePendingDosesWith(_ events: [NewPumpEvent], fetchedAt: Date) -> [NewPumpEvent] {
        // Must be called from the sessionQueue
        setStateWithResult { (state) -> [NewPumpEvent] in
            let allPending = (state.pendingDoses + [state.unfinalizedTempBasal, state.unfinalizedBolus]).compactMap({ $0 })
            let result = MinimedPumpManager.reconcilePendingDosesWith(
                events,
                reconciliationMappings: state.reconciliationMappings,
                pendingDoses: allPending
            )
            ...
            state.reconciliationMappings = result.reconciliationMappings.filter { (_, value) -> Bool in
                value.startTime >= expirationCutoff
            }
            ...
            self.pumpOps.runSession(withName: "Fetch Pump History", using: device) { session in
                do {
                    guard let startDate = self.pumpDelegate.call({ delegate in
                        delegate?.startDateToFilterNewPumpEvents(for: self)
                    }) else {
                    ...
                    self.log.default(
                        "Fetching history since %{public}@",
                        String(describing: startDate.addingTimeInterval(.minutes(-1)))
                    )
                    ...
                    let remainingHistoryEvents = self.reconcilePendingDosesWith(newPumpEvents, fetchedAt: self.dateGenerator())
                        .map { (event) -> NewPumpEvent in
                            NewPumpEvent(
                                date: event.date,
                    ...
                    self.pumpDelegate.notify({ delegate in
                        ...
                        let pendingEvents = (
                            self.state
                                .pendingDoses + [self.state.unfinalizedBolus, self.state.unfinalizedTempBasal]
                        )
                        .compactMap({ $0?.newPumpEvent() })

                        self.log.default(
                            "Reporting new pump events: %{public}@",
                            String(describing: remainingHistoryEvents + pendingEvents)
                        )

                        delegate.pumpManager(
                            self,
                            hasNewPumpEvents: remainingHistoryEvents + pendingEvents,
                            ...
                            if error == nil {
                                ...
                                self.setState({ state in
                                    ...
                                    if let tempBasal = state.unfinalizedTempBasal, tempBasal.isReconciledWithHistory,
    ...
    private func storePendingPumpEvents(
        forceFinalization: Bool = false,
        _ completion: @escaping (_ error: MinimedPumpManagerError?) -> Void
    ) {
        // Must be called from the sessionQueue
        let events = (state.pendingDoses + [state.unfinalizedBolus, state.unfinalizedTempBasal])
            .compactMap({ $0?.newPumpEvent(forceFinalization: forceFinalization) })
    ...
    private var isPumpDataStale: Bool {
        ...
        let pumpStatusAgeTolerance = rileyLinkDeviceProvider
            .idleListeningEnabled ? TimeInterval(minutes: 6) : TimeInterval(minutes: 4)
    ...
    public var preferredInsulinDataSource: InsulinDataSource {
        get {
            state.preferredInsulinDataSource
        }
        set {
            setState { state in
    ...
    public var useMySentry: Bool {
        ...
        set {
            ...
            if oldValue != newValue {
                ...
                rileyLinkDeviceProvider.idleListeningState = useIdleListening ? MinimedPumpManagerState
                    .idleListeningEnabledDefaults : .disabled
            }
        }
    }
...
extension MinimedPumpManager: PumpManager {
    ...
    public var localizedTitle: String {
        String(format: LocalizedString("Minimed %@", comment: "Pump title (1: model number)"), state.pumpModel.rawValue)
    }
    ...
    public var supportedBasalRates: [Double] {
        state.pumpModel.supportedBasalRates
    }
    ...
    public var supportedBolusVolumes: [Double] {
        state.pumpModel.supportedBolusVolumes
    }
    ...
    public var maximumBasalScheduleEntryCount: Int {
        state.pumpModel.maximumBasalScheduleEntryCount
    }

    public var minimumBasalScheduleEntryDuration: TimeInterval {
        state.pumpModel.minimumBasalScheduleEntryDuration
    }

    public var pumpRecordsBasalProfileStartEvents: Bool {
        state.pumpModel.recordsBasalProfileStartEvents
    }

    public var pumpReservoirCapacity: Double {
        Double(state.pumpModel.reservoirCapacity)
    }
    ...
    private func lastSync(for state: MinimedPumpManagerState, recents: MinimedPumpManagerRecents) -> Date? {
        [state.lastReconciliation, recents.lastContinuousReservoir].compactMap { $0 }.max()
    }

    public var lastSync: Date? {
        lastSync(for: state, recents: recents)
    }
    ...
    public func ensureCurrentPumpData(completion: ((Date?) -> Void)?) {
        ...
        guard isPumpDataStale else {
            log.default("Pump data is not stale: lastSync = %{public}@", String(describing: lastSync))
            completion?(lastSync)
            return
        }
        ...
        rileyLinkDeviceProvider.getDevices { devices in
            ...
            self.pumpOps.runSession(withName: "Get Pump Status", using: device) { session in
                do {
                    ...
                    if self.state.basalSchedule.entries.isEmpty, let basalSchedule = try? session.getBasalSchedule() {
                        self.setState { state in
                    ...
                    self.updateReservoirVolume(status.reservoir, at: date) { result in
                        ...
                        case .failure:
                            completion?(self.lastSync)
                        case let .success(areStoredValuesContinuous):
    ...
    public func enactBolus(
        ...
        pumpOps.runSession(withName: "Bolus", usingSelector: rileyLinkDeviceProvider.firstConnectedDevice) { session in
            ...
            var uncertainBolusError: PumpManagerError?

            if let error = session.setNormalBolus(units: enactUnits) {
                switch error {
                case let .certain(certainError):
                    ...
                    return
                case let .uncertain(uncertainError):
    ...
    public func cancelBolus(completion: @escaping (PumpManagerResult<DoseEntry?>) -> Void) {
        ...
        recents.bolusEngageState = .disengaging
    ...
    public func syncBasalRateSchedule(
        ...
        pumpOps
            .runSession(withName: "Save Basal Profile", usingSelector: rileyLinkDeviceProvider.firstConnectedDevice) { session in
                guard let session = session else {
    ...
    public func syncDeliveryLimits(
        ...
        pumpOps.runSession(withName: "Save Settings", usingSelector: rileyLinkDeviceProvider.firstConnectedDevice) { session in
            ...
            do {
                ...
                let storedDeliveryLimits = DeliveryLimits(
                    maximumBasalRate: HKQuantity(unit: .internationalUnitsPerHour, doubleValue: settings.maxBasal),
    ...
    public func deletePump(completion: @escaping () -> Void) {
        storePendingPumpEvents(forceFinalization: true) { _ in
...
extension MinimedPumpManager: PumpOpsDelegate {
    ...
    public func pumpOps(_: PumpOps, didChange state: PumpState) {
    ...
    public func fetchNewDataIfNeeded(_ completion: @escaping (CGMReadingResult) -> Void) {
        rileyLinkDeviceProvider.getDevices { devices in
            ...
            let latestGlucoseDate = self.cgmDelegate.call({ (delegate) -> Date in
                delegate?.startDateToFilterNewData(for: self) ?? Date(timeIntervalSinceNow: TimeInterval(hours: -24))
            })
                    ...
                    let glucoseValues: [NewGlucoseSample] = events
                        ...
                        .map {
                            ...
                            return NewGlucoseSample(
                                ...
                                quantity: quantity,
                                condition: nil,
                                trend: glucoseEvent.trendType,
                                trendRate: glucoseEvent.trendRate,
                                isDisplayOnly: false,
                                wasUserEntered: false,
                                syncIdentifier: glucoseEvent.glucoseSyncIdentifier ?? UUID().uuidString,
                                device: self.device
                            )
                        }
