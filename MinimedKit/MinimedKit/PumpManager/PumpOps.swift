...
public extension PumpOps {
    func runSession(
        withName name: String,
        usingSelector deviceSelector: @escaping (_ completion: @escaping (_ device: RileyLinkDevice?) -> Void) -> Void,
        _ block: @escaping (_ session: PumpOpsSession?) -> Void
    ) {
...
public class MinimedPumpOps: PumpOps {
    ...
    public func runSession(
        ...
        device.runSession(withName: name) { commandSession in
            ...
            let session = PumpOpsSession(
                settings: self.pumpSettings,
                pumpState: self.pumpState.value,
                messageSender: minimedPumpMessageSender,
                delegate: self
            )
...
extension MinimedPumpOps: PumpOpsSessionDelegate {
    public func pumpOpsSessionDidChangeRadioConfig(_: PumpOpsSession) {
        if let sessionDevice = self.sessionDevice {
            configuredDevices.value = [sessionDevice.peripheralIdentifier]
        }
    }

    public func pumpOpsSession(_: PumpOpsSession, didChange state: PumpState) {
...
public extension MinimedPumpOps {
    static let notificationPumpStateKey = "com.rileylink.RileyLinkKit.PumpOps.PumpState"
}

public extension Notification.Name {
    static let PumpOpsStateDidChange = Notification.Name(rawValue: "com.rileylink.RileyLinkKit.PumpOpsStateDidChange")
}
