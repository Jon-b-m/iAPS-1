        ...
        for event in self {
            ...
            case let bolus as BolusNormalPumpEvent:
                ...
                dose = DoseEntry(
                    ...
                    endDate: bolusEndDate,
                    value: bolus.programmed,
                    unit: .units,
                    deliveredUnits: bolus.amount,
                    automatic: automatic,
                    isMutable: bolus.isMutable(atDate: now, forPump: model),
                    wasProgrammedByPumpUI: !bolus.wasRemotelyTriggered
                )
            ...
            case let temp as TempBasalPumpEvent:
                if case .Absolute = temp.rateType {
                    lastTempBasal = DoseEntry(
                        ...
                        unit: .unitsPerHour,
                        isMutable: false,
                        wasProgrammedByPumpUI: !temp.wasRemotelyTriggered
                    )
                    continue
                } else {
