...
extension SetBolusError: LocalizedError {
    ...
    public var helpAnchor: String? {
        ...
        case .uncertain:
            return String(
                format: LocalizedString(
                    "%1$@ sent a bolus command to the pump, but was unable to confirm that the pump received the command. For safety, %2$@ will assume the bolus was delivered. When %3$@ eventually fetches history from the pump, and the estimated bolus finish time is passed, %4$@ will update its records of delivery to match what the pump reports.",
                    comment: "Help anchor for uncertain bolus (1: appName) (2: appName) (3: appName) (4: appName)"
                ),
                appName,
                appName,
                appName,
...
public class PumpOpsSession {
    public private(set) var pump: PumpState {
        didSet {
    ...
    public init(
        ...
        pumpState: PumpState,
        messageSender: PumpMessageSender,
        delegate: PumpOpsSessionDelegate
    ) {
...
extension PumpOpsSession {
    ...
    private func sendWakeUpBurst() throws {
        ...
        guard pump.lastWakeAttempt == nil || pump.lastWakeAttempt!.timeIntervalSinceNow <= -PumpOpsSession
            .minimumTimeBetweenWakeAttempts
        else {
            return
        }
        ...
        if pump.pumpModel == nil || !pump.pumpModel!.hasMySentry {
            ...
            do {
                let _: PumpAckMessageBody = try messageSender.getResponse(
                    to: shortPowerMessage,
                    responseType: .pumpAck,
                    repeatCount: 255,
    ...
    private func isPumpResponding() -> Bool {
        do {
            let _: GetPumpModelCarelinkMessageBody = try messageSender.getResponse(
                to: PumpMessage(pumpID: settings.pumpID, type: .getPumpModel),
                responseType: .getPumpModel,
                repeatCount: 0,
                timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
                retryCount: 1
            )
            return true
        } catch {
    ...
    private func wakeup(_ duration: TimeInterval = TimeInterval(minutes: 1)) throws {
        ...
        do {
            let longPowerMessage = PumpMessage(
                ...
                type: .powerOn,
                body: PowerOnCarelinkMessageBody(duration: duration)
            )
            let _: PumpAckMessageBody = try messageSender.getResponse(
                to: longPowerMessage,
                responseType: .pumpAck,
...
public extension PumpOpsSession {
    ...
    ///     - PumpOpsError.unexpectedResponse
    ///     - PumpOpsError.unknownResponse
    func getPumpModel(usingCache: Bool = true) throws -> PumpModel {
        if usingCache, let pumpModel = pump.pumpModel {
            return pumpModel
        }

        try wakeup()
        let body: GetPumpModelCarelinkMessageBody = try messageSender.getResponse(
            to: PumpMessage(pumpID: settings.pumpID, type: .getPumpModel),
            responseType: .getPumpModel,
            repeatCount: 0,
            timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
    ...
    ///     - PumpOpsError.unexpectedResponse
    ///     - PumpOpsError.unknownResponse
    func getPumpFirmwareVersion() throws -> String {
        try wakeup()
        let body: GetPumpFirmwareVersionMessageBody = try messageSender.getResponse(
            to: PumpMessage(pumpID: settings.pumpID, type: .readFirmwareVersion),
            responseType: .readFirmwareVersion,
            repeatCount: 0,
            timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
            retryCount: 3
        )
    ...
    ///     - PumpOpsError.unexpectedResponse
    ///     - PumpOpsError.unknownResponse
    func getBatteryStatus() throws -> GetBatteryCarelinkMessageBody {
        try wakeup()
        return try messageSender.getResponse(
            to: PumpMessage(pumpID: settings.pumpID, type: .getBattery),
            responseType: .getBattery,
            repeatCount: 0,
            timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
            retryCount: 3
        )
    ...
    internal func getPumpStatus() throws -> ReadPumpStatusMessageBody {
        try wakeup()
        return try messageSender.getResponse(
            to: PumpMessage(pumpID: settings.pumpID, type: .readPumpStatus),
            responseType: .readPumpStatus,
            repeatCount: 0,
            timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
            retryCount: 3
        )
    ...
    ///     - PumpOpsError.unexpectedResponse
    ///     - PumpOpsError.unknownResponse
    func getSettings() throws -> ReadSettingsCarelinkMessageBody {
        try wakeup()
        return try messageSender.getResponse(
            to: PumpMessage(pumpID: settings.pumpID, type: .readSettings),
            responseType: .readSettings,
            repeatCount: 0,
            timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
            retryCount: 3
        )
    ...
    ///     - PumpOpsError.unexpectedResponse
    ///     - PumpOpsError.unknownResponse
    func getTime() throws -> DateComponents {
        try wakeup()
        let response: ReadTimeCarelinkMessageBody = try messageSender.getResponse(
            to: PumpMessage(pumpID: settings.pumpID, type: .readTime),
            responseType: .readTime,
            repeatCount: 0,
            timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
            retryCount: 3
        )
    ...
    ///     - PumpOpsError.unexpectedResponse
    ///     - PumpOpsError.unknownResponse
    func getBasalSchedule(for profile: BasalProfile = .standard) throws -> BasalSchedule? {
        try wakeup()
        ...
        while !isFinished {
            let body: DataFrameMessageBody = try messageSender.getResponse(
                to: message,
                responseType: profile.readMessageType,
                repeatCount: 0,
                timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
                retryCount: 3
            )
    ...
    ///     - PumpOpsError.unexpectedResponse
    ///     - PumpOpsError.unknownResponse
    func getOtherDevicesIDs() throws -> ReadOtherDevicesIDsMessageBody {
        try wakeup()

        return try messageSender.getResponse(
            to: PumpMessage(pumpID: settings.pumpID, type: .readOtherDevicesIDs),
            responseType: .readOtherDevicesIDs,
            repeatCount: 0,
            timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
            retryCount: 3
        )
    ...
    ///     - PumpOpsError.unexpectedResponse
    ///     - PumpOpsError.unknownResponse
    func getOtherDevicesEnabled() throws -> Bool {
        try wakeup()

        let response: ReadOtherDevicesStatusMessageBody = try messageSender.getResponse(
            to: PumpMessage(pumpID: settings.pumpID, type: .readOtherDevicesStatus),
            responseType: .readOtherDevicesStatus,
            repeatCount: 0,
            timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
            retryCount: 3
        )
    ...
    ///     - PumpOpsError.unexpectedResponse
    ///     - PumpOpsError.unknownResponse
    func getRemoteControlIDs() throws -> ReadRemoteControlIDsMessageBody {
        try wakeup()

        return try messageSender.getResponse(
            to: PumpMessage(pumpID: settings.pumpID, type: .readRemoteControlIDs),
            responseType: .readRemoteControlIDs,
            repeatCount: 0,
            timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
            retryCount: 3
        )
...
public extension PumpOpsSession {
    ...
    ///     - PumpOpsError.noResponse
    ///     - PumpOpsError.unknownResponse
    func getRemainingInsulin() throws -> (units: Double, clock: DateComponents) {
        let pumpModel = try getPumpModel()
        let pumpClock = try getTime()

        let reservoir: ReadRemainingInsulinMessageBody = try messageSender.getResponse(
            to: PumpMessage(pumpID: settings.pumpID, type: .readRemainingInsulin),
            responseType: .readRemainingInsulin,
            repeatCount: 0,
            timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
            retryCount: 3
        )
    ...
    ///     - PumpCommandError
    ///     - PumpOpsError
    func getCurrentPumpStatus() throws -> PumpStatus {
        let pumpModel = try getPumpModel()
...
public extension PumpOpsSession {
    internal enum CommandTestingFailureMode {
    ...
    private func runCommandWithArguments<T: MessageBody>(
        _ message: PumpMessage,
        responseType: MessageType = .pumpAck,
        retryCount: Int = 3,
        testingFailureMode: CommandTestingFailureMode? = nil
    ) throws -> T {
        do {
            try wakeup()

            let shortMessage = PumpMessage(
                packetType: message.packetType,
                address: message.address.hexadecimalString,
                messageType: message.messageType,
                messageBody: CarelinkShortMessageBody()
            )
            let _: PumpAckMessageBody = try messageSender.getResponse(
                to: shortMessage,
                responseType: .pumpAck,
        ...
        do {
            ...
            let response: T = try messageSender.getResponse(
                to: message,
    ...
    /// - Throws: `PumpCommandError` specifying the failure sequence
    func pressButton(_ type: ButtonPressCarelinkMessageBody.ButtonType) throws {
        let message = PumpMessage(
            pumpID: settings.pumpID,
            type: .buttonPress,
            body: ButtonPressCarelinkMessageBody(buttonType: type)
        )
    ...
    /// - Throws: `PumpCommandError` specifying the failure sequence
    func setSuspendResumeState(_ state: SuspendResumeMessageBody.SuspendResumeState) throws {
    ...
    func selectBasalProfile(_ profile: BasalProfile) throws {
        let message = PumpMessage(
            pumpID: settings.pumpID,
            type: .selectBasalProfile,
            body: SelectBasalProfileMessageBody(newProfile: profile)
        )
    ...
    /// - Throws: PumpCommandError
    func setMaxBasalRate(unitsPerHour: Double) throws {
    ...
    /// - Throws: PumpCommandError
    func setMaxBolus(units: Double) throws {
    ...
    func setTempBasal(_ unitsPerHour: Double, duration: TimeInterval) -> Result<Bool, PumpCommandError> {
        let message = PumpMessage(
            pumpID: settings.pumpID,
            type: .changeTempBasal,
            body: ChangeTempBasalCarelinkMessageBody(unitsPerHour: unitsPerHour, duration: duration)
        )
        ...
        } catch let PumpOpsError.pumpError(errorCode) {
            return .failure(.arguments(.pumpError(errorCode)))
        } catch let PumpOpsError.unknownPumpErrorCode(errorCode) {
        ...
        do {
            let response: ReadTempBasalCarelinkMessageBody = try messageSender.getResponse(
                to: PumpMessage(pumpID: settings.pumpID, type: .readTempBasal),
                responseType: .readTempBasal,
                repeatCount: 0,
                timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
                retryCount: 3
            )
    ...
    /// - Throws: PumpCommandError
    func setTime(_ generator: () -> DateComponents) throws {
        try wakeup()
        ...
        do {
            ...
            let message = PumpMessage(
                pumpID: settings.pumpID,
                type: .changeTime,
                body: ChangeTimeCarelinkMessageBody(dateComponents: components)!
            )
            let _: PumpAckMessageBody = try messageSender.getResponse(
                to: message,
                responseType: .pumpAck,
            ...
            pump.timeZone = components.timeZone?.fixed ?? .currentFixed
        } catch let error as PumpOpsError {
            throw PumpCommandError.arguments(error)
        }
    }

    func setTimeToNow(in timeZone: TimeZone? = nil) throws {
    ...
    func setNormalBolus(units: Double) -> SetBolusError? {
        ...
        do {
            let message = PumpMessage(
                pumpID: settings.pumpID,
                type: .bolus,
                body: BolusCarelinkMessageBody(units: units, insulinBitPackingScale: pumpModel.insulinBitPackingScale)
            )
...
//            }
    ...
    /// - Throws: PumpCommandError
    func setRemoteControlEnabled(_ enabled: Bool) throws {
        let message = PumpMessage(
            pumpID: settings.pumpID,
            type: .setRemoteControlEnabled,
            body: SetRemoteControlEnabledMessageBody(enabled: enabled)
        )
    ...
    /// - Throws: PumpCommandError
    func setRemoteControlID(_ id: Data, atIndex index: Int) throws {
    ...
    /// - Throws: PumpCommandError
    func removeRemoteControlID(atIndex index: Int) throws {
    ...
    /// - Throws: `PumpCommandError` specifying the failure sequence
    func setBasalSchedule(_ basalSchedule: BasalSchedule, for profile: BasalProfile) throws {
    ...
    func getStatistics() throws -> RileyLinkStatistics {
        try messageSender.getRileyLinkStatistics()
    }
}
...
public extension PumpOpsSession {
    ...
    ///     - PumpOpsError.unexpectedResponse
    ///     - PumpOpsError.unknownResponse
    func changeWatchdogMarriageProfile(_ watchdogID: Data) throws {
        ...
        guard findMessage.address.hexadecimalString == settings.pumpID, findMessage.packetType == .mySentry,
              let findMessageBody = findMessage.messageBody as? FindDeviceMessageBody,
              let findMessageResponseBody = MySentryAckMessageBody(
                  sequence: findMessageBody.sequence,
                  watchdogID: watchdogID,
                  responseMessageTypes: [findMessage.messageType]
              )
        else {
            throw PumpOpsError.unknownResponse(rx: packet.data, during: "Watchdog listening")
        }
        ...
        let findMessageResponse = PumpMessage(
            packetType: .mySentry,
            address: settings.pumpID,
            messageType: .pumpAck,
            messageBody: findMessageResponseBody
        )

        let linkMessage = try messageSender.sendAndListen(
            findMessageResponse,
            repeatCount: 0,
        ...
        guard let
            ...
            let linkMessageResponseBody = MySentryAckMessageBody(
                sequence: linkMessageBody.sequence,
                watchdogID: watchdogID,
                responseMessageTypes: [linkMessage.messageType]
            )
        else {
        ...
        let linkMessageResponse = PumpMessage(
            packetType: .mySentry,
            address: settings.pumpID,
            messageType: .pumpAck,
            messageBody: linkMessageResponseBody
        )
...
extension PumpOpsSession {
    ...
    public func tuneRadio(attempts: Int = 3) throws -> FrequencyScanResults {
        let region = settings.pumpRegion
    ...
    ///     - PumpOpsError.rfCommsFailure
    ///     - LocalizedError
    private func scanForPump(
        in frequencies: [Measurement<UnitFrequency>],
        fallback: Measurement<UnitFrequency>?,
        tries: Int = 3
    ) throws -> FrequencyScanResults {
        ...
        for freq in frequencies {
            ...
            for _ in 1 ... tries {
                ...
                let rfPacket = try? messageSender.sendAndListenForPacket(
                    PumpMessage(pumpID: settings.pumpID, type: .getPumpModel),
                    repeatCount: 0,
                    timeout: .milliseconds(130),
                    retryCount: 3
                )
...
extension PumpOpsSession {
    ...
    public func getHistoryEvents(since startDate: Date) throws -> ([TimestampedHistoryEvent], PumpModel) {
        ...
        pages: for pageNum in 0 ..< 16 {
            ...
            } catch let PumpCommandError.arguments(error) {
            ...
            while idx < pageData.count {
                ...
                // TODO: Convert logging
                NSLog(
                    String(format: "HistoryPage %02d - (bytes %03d-%03d): ", pageNum, idx, top - 1) + pageData
                        .subdata(in: range).hexadecimalString
                )
                idx = top
            }
            ...
            let (timestampedEvents, hasMoreEvents, _) = page
                .timestampedEvents(after: startDate, timeZone: pump.timeZone, model: pumpModel)
    ...
    private func getHistoryPage(_ pageNum: Int) throws -> Data {
        ...
        let msg = PumpMessage(
            pumpID: settings.pumpID,
            type: .getHistoryPage,
            body: GetHistoryPageCarelinkMessageBody(pageNum: pageNum)
        )
        ...
        while expectedFrameNum == curResp.frameNumber {
            ...
            if !curResp.lastFrame {
                curResp = try messageSender.getResponse(
                    to: msg,
                    responseType: .getHistoryPage,
                    repeatCount: 0,
                    timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
                    retryCount: 3
                )
...
extension PumpOpsSession {
    private func logGlucoseHistory(pageData: Data, pageNum: Int) {
        ...
        while idx < pageData.count {
            ...
            // TODO: Convert logging
            NSLog(
                String(format: "GlucosePage %02d - (bytes %03d-%03d): ", pageNum, idx, top - 1) + pageData.subdata(in: range)
                    .hexadecimalString
            )
            idx = top
        }
    ...
    public func getGlucoseHistoryEvents(since startDate: Date) throws -> [TimestampedGlucoseEvent] {
        ...
        let currentGlucosePage: ReadCurrentGlucosePageMessageBody = try messageSender.getResponse(
            to: PumpMessage(pumpID: settings.pumpID, type: .readCurrentGlucosePage),
            responseType: .readCurrentGlucosePage,
            repeatCount: 0,
            timeout: MinimedPumpMessageSender.standardPumpResponseWindow,
            retryCount: 3
        )
        ...
        pages: for pageNum in stride(from: startPage, to: endPage - 1, by: -1) {
            ...
            do {
                ...
                if page.needsTimestamp && pageNum == startPage {
                    ...
                    _ = try writeGlucoseHistoryTimestamp()
