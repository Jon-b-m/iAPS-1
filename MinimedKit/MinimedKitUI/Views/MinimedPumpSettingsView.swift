...
struct MinimedPumpSettingsView: View {
    ...
    init(
        viewModel: MinimedPumpSettingsViewModel,
        supportedInsulinTypes: [InsulinType],
        handleRileyLinkSelection: @escaping (RileyLinkDevice) -> Void,
    ...
    var body: some View {
        List {
            ...
            Section(header: SectionHeader(label: LocalizedString(
                "Configuration",
                comment: "The title of the configuration section in MinimedPumpManager settings"
            )))
                {
                    ...
                    NavigationLink(destination: BatteryTypeSelectionView(batteryType: $viewModel.batteryChemistryType)) {
                        HStack {
                            Text(LocalizedString("Pump Battery Type", comment: "Text for medtronic pump battery type"))
                                .foregroundColor(Color.primary)
                            Spacer()
                    ...
                    if viewModel.pumpManager.state.pumpModel.hasMySentry {
                        NavigationLink(destination: UseMySentrySelectionView(mySentryConfig: $viewModel.mySentryConfig)) {
                            HStack {
                                Text(LocalizedString("Use MySentry", comment: "Text for medtronic pump to use MySentry"))
                                    .foregroundColor(Color.primary)
                                Spacer()
                                Text(
                                    viewModel.mySentryConfig == .useMySentry ?
                                        LocalizedString(
                                            "Yes",
            ...
            Section {
                LabeledValueView(
                    label: LocalizedString("Pump ID", comment: "The title text for the pump ID config value"),
                ...
                LabeledValueView(
                    label: LocalizedString("Region", comment: "The title of the cell showing the pump region"),
        ...
        .alert(item: $viewModel.activeAlert, content: { alert in
            ...
            case let .suspendError(error):
                return Alert(
                    title: Text(LocalizedString("Error Suspending", comment: "The alert title for a suspend error")),
                    message: Text(errorText(error))
                )
            case let .resumeError(error):
                return Alert(
                    title: Text(LocalizedString("Error Resuming", comment: "The alert title for a resume error")),
                    message: Text(errorText(error))
                )
        ...
        .navigationBarTitle(String(
            ...
            viewModel.pumpManager.state.pumpModel.description
        ))
    }
    ...
    var reservoirStatus: some View {
        VStack(alignment: .leading, spacing: 5) {
            ...
            {
                HStack {
                    MinimedReservoirView(
                        filledPercent: reservoirPercent,
