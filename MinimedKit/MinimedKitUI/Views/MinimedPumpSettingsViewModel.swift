...
class MinimedPumpSettingsViewModel: ObservableObject {
    ...
    init(pumpManager: MinimedPumpManager) {
        ...
        reservoirReading = pumpManager.state.lastReservoirReading
        batteryChemistryType = pumpManager.batteryChemistry
        preferredDataSource = pumpManager.preferredInsulinDataSource
        mySentryConfig = pumpManager.useMySentry ? .useMySentry : .doNotUseMySentry
    ...
    var pumpImage: UIImage {
        pumpManager.state.largePumpImage
    }
