            ...
            ops?.runSession(withName: "Get Basal Settings", using: device) { session in
                ...
                do {
                    ...
                    var str = String(
                        ...
                        integerFormatter.string(from: NSNumber(value: schedule?.entries.count ?? 0))!
                    )
            ...
            ops?.runSession(withName: "Read pump status", using: device) { session in
                ...
                do {
                    ...
                    var str = String(
                        ...
                        measurementFormatter.numberFormatter.string(from: NSNumber(value: status.reservoir))!
                    )
                    str += String(
                        format: LocalizedString(
                            "Battery: %1$@ volts\n",
                        ...
                        measurementFormatter.string(from: status.batteryVolts)
                    )
                    str += String(
                        format: LocalizedString(
                            "Suspended: %1$@\n",
                            comment: "The format string describing pump suspended state: (1: suspended)"
                        ),
                        String(describing: status.suspended)
                    )
                    str += String(
                        format: LocalizedString(
                            "Bolusing: %1$@\n",
                        ...
                        String(describing: status.bolusing)
                    )
        ...
        T { (completionHandler) -> String in
            ops?.runSession(withName: "Tune pump", using: device) { session in
                ...
                do {
                    ...
                    let formatString = LocalizedString(
                        "%1$@  %2$@/%3$@  %4$@",
                    ...
                    resultDict[LocalizedString("Best Frequency", comment: "The label indicating the best radio frequency")] =
                        measurementFormatter.string(from: scanResult.bestFrequency)
                    resultDict[LocalizedString("Trials", comment: "The label indicating the results of each frequency trial")] =
                        scanResult.trials.map({ (trial) -> String in
