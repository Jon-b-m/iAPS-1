...
extension PumpModel {
    func largeImage(in color: PumpColor?) -> UIImage {
        UIImage.pumpImage(in: color, isLargerModel: reservoirCapacity > 200, isSmallImage: false)
    }

    func smallImage(in color: PumpColor?) -> UIImage {
        UIImage.pumpImage(in: color, isLargerModel: reservoirCapacity > 200, isSmallImage: true)
    }
}

extension MinimedPumpManagerState {
    var largePumpImage: UIImage {
        UIImage.pumpImage(in: pumpColor, isLargerModel: pumpModel.reservoirCapacity > 200, isSmallImage: false)
    }

    var smallPumpImage: UIImage {
        UIImage.pumpImage(in: pumpColor, isLargerModel: pumpModel.reservoirCapacity > 200, isSmallImage: true)
    }
}
