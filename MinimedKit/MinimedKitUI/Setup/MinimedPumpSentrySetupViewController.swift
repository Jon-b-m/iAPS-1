...
class MinimedPumpSentrySetupViewController: SetupTableViewController {
    ...
    private var continueState: State = .notStarted {
        didSet {
            ...
            case .notStarted:
                ...
                footerView.primaryButton.setTitle(
                    LocalizedString("Retry", comment: "Button title to retry sentry setup"),
                    for: .normal
                )
            case .listening:
                lastError = nil
    ...
    private func listenForPairing() {
        ...
        pumpManager.pumpOps.runSession(
            withName: "MySentry Pairing",
            usingSelector: pumpManager.rileyLinkDeviceProvider.firstConnectedDevice
        ) { session in
