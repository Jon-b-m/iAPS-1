...
class MinimedPumpIDSetupViewController: SetupTableViewController {
    ...
    @IBOutlet var pumpIDTextField: UITextField!

    @IBOutlet fileprivate var regionAndColorPickerCell: RegionAndColorPickerTableViewCell!

    @IBOutlet var activityIndicator: SetupIndicatorView!
    ...
    private var lastError: Error? {
        didSet {
            ...
            if let error = lastError as? LocalizedError {
                let localizedText = [error.errorDescription, error.failureReason, error.recoverySuggestion].compactMap({ $0 })
                    .joined(separator: ". ")
    ...
    private func setupPump(with settings: PumpSettings) {
        ...
        pumpOps.runSession(
            withName: "Pump ID Setup",
            usingSelector: rileyLinkPumpManager.rileyLinkDeviceProvider.firstConnectedDevice,
            { session in
    ...
    override func continueButtonPressed(_ sender: Any) {
        if case .completed = continueState {
            ...
            if isSentrySetUpNeeded {
                ...
                self.continueState = .completed
                self.pumpState = PumpState(timeZone: .currentFixed, pumpModel: PumpModel(
                    rawValue:
                    "523"
                )!, useMySentry: false)
...
extension MinimedPumpIDSetupViewController: PumpOpsDelegate {
    ...
    func willSend(_: String) {}
    func didReceive(_: String) {}
    func didError(_: String) {}

    func pumpOps(_: PumpOps, didChange state: PumpState) {
        DispatchQueue.main.async {
...
class RegionAndColorPickerTableViewCell: UITableViewCell {
    ...
    @IBOutlet var pumpImageView: UIImageView!
}
