...
class PumpOpsSynchronousTests: XCTestCase {
    ...
    lazy var datePast2007: Date = {
        self.dateComponents2017.date!.addingTimeInterval(TimeInterval(minutes: 60))
    }()
    ...
    var ack: PumpMessage {
        PumpMessage(pumpID: pumpID, type: .pumpAck)
    }
    ...
    func testSetNormalBolus() {
        mockMessageSender.responses = [
            .readPumpStatus: [
                mockMessageSender
                    .makeMockResponse(.readPumpStatus, ReadPumpStatusMessageBody(bolusing: false, suspended: false))
            ],
            .bolus: [ack, ack]
        ]
    ...
    func testSetNormalBolusWhileBolusing() {
        mockMessageSender.responses = [
            .readPumpStatus: [
                mockMessageSender
                    .makeMockResponse(.readPumpStatus, ReadPumpStatusMessageBody(bolusing: true, suspended: false))
            ],
            .bolus: [ack, ack]
        ]
    ...
    func testSetNormalBolusWhileSuspended() {
        mockMessageSender.responses = [
            .readPumpStatus: [
                mockMessageSender
                    .makeMockResponse(.readPumpStatus, ReadPumpStatusMessageBody(bolusing: false, suspended: true))
            ],
            .bolus: [ack, ack]
        ]
    ...
    func testShouldFinishIfTimestampBeforeStartDateEncountered() {
        ...
        let (_, hasMoreEvents, _) = page
            .timestampedEvents(after: afterBatteryEventDate, timeZone: pumpState.timeZone, model: pumpModel)
    ...
    func testEventBeforeStartDateIsFiltered() {
        ...
        let (events, hasMoreEvents, cancelled) = page
            .timestampedEvents(after: datePast2007, timeZone: pumpState.timeZone, model: pumpModel)
    ...
    func testPumpLostTimeCancelsFetchEarly() {
        ...
        let (events, hasMoreEvents, cancelledEarly) = page.timestampedEvents(
            after: Date.distantPast,
            timeZone: pumpState.timeZone,
            model: pumpModel
        )
    ...
    func testNonMutableSquareWaveBolusFor522IsReturned() {
        ...
        // 120 minute duration
        let squareWaveBolus = BolusNormalPumpEvent(
            availableData: Data(hexadecimalString: "010080048000240009a24a1510")!,
            pumpModel: pumpModel
        )!

        let page = HistoryPage(events: [squareWaveBolus])

        let (timeStampedEvents, _, _) = page
            .timestampedEvents(after: .distantPast, timeZone: pumpState.timeZone, model: pumpModel)
    ...
    func testDelayedAppendEventDoesNotCauseValidEventsToBeFilteredOut() {
        ...
        let (timeStampedEvents, hasMoreEvents, cancelled) = page.timestampedEvents(
            after: .distantPast,
            timeZone: pumpState.timeZone,
            model: pumpModel
        )
    ...
    func createBatteryEvent(withDateComponent dateComponents: DateComponents) -> BatteryPumpEvent {
        createBatteryEvent(atTime: dateComponents.date!)
    }
    ...
    func createBatteryEvent(atTime date: Date = Date()) -> BatteryPumpEvent {
        ...
        let secondByte = UInt8(second) & 0b0011_1111
        let minuteByte = UInt8(minute) & 0b0011_1111
        let hourByte = UInt8(hour) & 0b0001_1111
        let dayByte = UInt8(day) & 0b0001_1111
        let monthUpperComponent = (UInt8(month) & 0b0000_1100) << 4
        let monthLowerComponent = (UInt8(month) & 0b0000_0011) << 6
        let secondMonthByte = secondByte | monthUpperComponent
        let minuteMonthByte = minuteByte | monthLowerComponent
        let yearByte = UInt8(year) & 0b0111_1111
    ...
    func createSquareBolusEvent2010() -> BolusNormalPumpEvent {
        ...
        return BolusNormalPumpEvent(
            ...
            type: .square,
            duration: TimeInterval(minutes: 120),
            wasRemotelyTriggered: false
        )
    }
    ...
    func createTempEventBasal2016() -> TempBasalPumpEvent {
        ...
        let tempEventBasal = TempBasalPumpEvent(
            availableData: Data(hexadecimalString: "338c4055145d1000")!,
            pumpModel: pumpModel
        )!
    ...
    func createBolusEvent2009() -> BolusNormalPumpEvent {
        ...
        let timeInterval = TimeInterval(minutes: 2)
