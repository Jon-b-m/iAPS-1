...
class MinimedPumpManagerTests: XCTestCase {
    ...
    private func dateGenerator() -> Date {
        simulatedDate + dateSimulationOffset
    }
    ...
    override func setUpWithError() throws {
        ...
        pumpManager = MinimedPumpManager(
            state: state,
            rileyLinkDeviceProvider: rlProvider,
            pumpOps: pumpOps,
    ...
    func testPendingBolusRemovedIfMissingFromHistory() {
        ...
        let pumpStatusResponse = mockMessageSender.makeMockResponse(
            .readPumpStatus,
            ReadPumpStatusMessageBody(bolusing: false, suspended: false)
        )
        ...
        mockMessageSender.responses = [
            .getBattery: [
                mockMessageSender
                    .makeMockResponse(.getBattery, GetBatteryCarelinkMessageBody(status: .normal, volts: 1.44))
            ],
            .readPumpStatus: [pumpStatusResponse, pumpStatusResponse],
            .readTime: [
                mockMessageSender
                    .makeMockResponse(.readTime, ReadTimeCarelinkMessageBody(dateComponents: dateComponents))
            ],
            .readRemainingInsulin: [mockMessageSender.makeMockResponse(
                .readRemainingInsulin,
                ReadRemainingInsulinMessageBody(
                    reservoirVolume: 115.7,
                    insulinBitPackingScale: PumpModel.model522.insulinBitPackingScale
                )
            )],
