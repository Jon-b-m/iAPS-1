...
final class ReconciliationTests: XCTestCase {
    ...
    func testPendingDoseUpdatesWithActualDeliveryFromHistoryDose() {
        ...
        let unfinalizedBolus = UnfinalizedDose(
            ...
            insulinType: .novolog,
            automatic: false,
            isReconciledWithHistory: false
        )

        // 5.4 bolus interrupted at 1.0 units
        let eventDose = DoseEntry(
            type: .bolus,
            startDate: bolusEventTime,
            endDate: cancelTime,
            value: unfinalizedBolus.units,
            unit: .units,
            deliveredUnits: 1.0
        )
    ...
    func testReconciledDosesShouldOnlyAppearInReturnedPendingDoses() {
        ...
        let eventDose = DoseEntry(
            type: .bolus,
            startDate: bolusEventTime,
            endDate: bolusEventTime.addingTimeInterval(bolusDuration),
            value: bolusAmount,
            unit: .units,
            deliveredUnits: bolusAmount
        )
    ...
    func testReconciledDosesShouldNotAppearInReturnedPumpEvents() {
        ...
        let result = MinimedPumpManager.reconcilePendingDosesWith(
            [bolusEvent],
            reconciliationMappings: reconciliationMappings,
            pendingDoses: []
        )
