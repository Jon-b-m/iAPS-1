...
class MainViewController: RileyLinkSettingsViewController {
    ...
    init(deviceDataManager: DeviceDataManager, insulinTintColor: Color, guidanceColors: GuidanceColors) {
        ...
        title = LocalizedString("RileyLink Testing", comment: "Title for RileyLink Testing main view controller")
    }
    ...
    private var shouldAllowAddingPump: Bool {
        rileyLinkManager.connectingCount > 0
    }
            ...
            if let pumpManager = deviceDataManager.pumpManager {
                ...
                case .addMinimedPump:
                    ...
                    cell.textLabel?.text = LocalizedString(
                        "Add Minimed Pump",
                        comment: "Title text for button to set up a new minimed pump"
                    )
                case .setupOmnipod:
                    cell = tableView.dequeueReusableCell(withIdentifier: TextButtonTableViewCell.className, for: indexPath)
                    let textButtonCell = cell as? TextButtonTableViewCell
                ...
                if let rileyLinkManagerViewController = setupViewController as? RileyLinkManagerSetupViewController {
                    rileyLinkManagerViewController
                        .rileyLinkPumpManager = RileyLinkPumpManager(
                            rileyLinkDeviceProvider: deviceDataManager
                                .rileyLinkConnectionManager.deviceProvider
                        )
                }
...
extension MainViewController: PumpManagerCreateDelegate {
    func pumpManagerOnboarding(_: PumpManagerCreateNotifying, didCreatePumpManager pumpManager: PumpManagerUI) {
        deviceDataManager.pumpManager = pumpManager
    }
}

extension MainViewController: PumpManagerOnboardingDelegate {
    func pumpManagerOnboarding(
        _: PumpManagerOnboarding,
        didOnboardPumpManager pumpManager: PumpManagerUI,
        withSettings _: PumpManagerSetupSettings
    ) {
