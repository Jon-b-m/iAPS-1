...
class RileyLinkListTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        dataManagerObserver = NSNotificationCenter.defaultCenter()
            .addObserverForName(nil, object: dataManager, queue: nil) { [weak self = self] (note) -> Void in
                if let deviceManager = self?.dataManager.rileyLinkManager {
                    switch note.name {
                    case RileyLinkDeviceManager.DidDiscoverDeviceNotification:
                        self?.tableView.insertRowsAtIndexPaths(
                            [NSIndexPath(forRow: deviceManager.devices.count - 1, inSection: 0)],
                    ...
                    case RileyLinkDeviceManager.ConnectionStateDidChangeNotification,
                        ...
                        if let device = note.userInfo?[RileyLinkDeviceManager.RileyLinkDeviceKey] as? RileyLinkDevice,
                           index = deviceManager.devices.indexOf({ $0 === device })
                        {
    ...
    override func tableView(tableView _: UITableView, numberOfRowsInSection _: Int) -> Int {
        dataManager.rileyLinkManager.devices.count
    }
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let deviceCell = tableView
            .dequeueReusableCellWithIdentifier(RileyLinkDeviceTableViewCell.className) as! RileyLinkDeviceTableViewCell
