...
public class RileyLinkBluetoothDeviceProvider: NSObject {
    ...
    private let centralQueue = DispatchQueue(
        label: "com.rileylink.RileyLinkBLEKit.BluetoothManager.centralQueue",
        qos: .unspecified
    )

    internal let sessionQueue = DispatchQueue(
        label: "com.rileylink.RileyLinkBLEKit.RileyLinkDeviceManager.sessionQueue",
        qos: .unspecified
    )
    ...
    public var idleListeningState: RileyLinkBluetoothDevice.IdleListeningState {
        get {
            lockedIdleListeningState.value
        }
        set {
...
extension RileyLinkBluetoothDeviceProvider {
    ...
    private var hasDiscoveredAllAutoConnectDevices: Bool {
        ...
        return autoConnectIDs.isSubset(of: devices.map(\.peripheralIdentifier.uuidString))
    }
    ...
    private func addPeripheral(_ peripheral: CBPeripheral, rssi: Int? = nil) {
        ...
        if let device = device {
            ...
            device = RileyLinkBluetoothDevice(
...
extension RileyLinkBluetoothDeviceProvider: RileyLinkDeviceProvider {
    ...
    public func shouldConnect(to deviceID: String) -> Bool {
        autoConnectIDs.contains(deviceID)
    }
}
...
public extension RileyLinkBluetoothDeviceProvider {
...
public extension Notification.Name {
    static let ManagerDevicesDidChange = Notification.Name("com.rileylink.RileyLinkBLEKit.DevicesDidChange")
}

public extension RileyLinkBluetoothDeviceProvider {
    static let autoConnectIDsStateKey = "com.rileylink.RileyLinkBLEKit.AutoConnectIDs"
}
