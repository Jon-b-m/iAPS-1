...
public struct RileyLinkStatistics {
    ...
    public init(
        uptime: TimeInterval,
        radioRxOverflowCount: UInt16,
        radioRxFifoOverflowCount: UInt16,
        packetRxCount: UInt16,
        packetTxCount: UInt16,
        crcFailureCount: UInt16,
        spiSyncFailureCount: UInt16
    ) {
        self.uptime = uptime
...
public struct CommandSession {
    ...
    private func writeCommand<C: Command>(_ command: C, timeout: TimeInterval) throws -> C.ResponseType {
        let response = try manager.writeCommand(
            command,
    ...
    /// - Throws: RileyLinkDeviceError
    private func writeCommand<C: Command>(_ command: C, timeout: TimeInterval) throws -> RFPacket
        where C.ResponseType == PacketResponse
    {
    ...
    public func sendAndListen(
        ...
        retryCount: Int,
        preambleExtension: TimeInterval?
    ) throws -> RFPacket {
        ...
        let singlePacketSendTime: TimeInterval = (Double(data.count * 8) / 16384)
        let preambleTime = preambleExtension ?? 0
        let totalRepeatSendTime: TimeInterval = (singlePacketSendTime + timeBetweenPackets + preambleTime) *
    ...
    public func sendAndListen(_ data: Data, repeatCount: Int, timeout: TimeInterval, retryCount: Int) throws -> RFPacket {
        try sendAndListen(data, repeatCount: repeatCount, timeout: timeout, retryCount: retryCount, preambleExtension: nil)
    }

    /// - Throws: RileyLinkDeviceError
    public func listen(onChannel _: Int, timeout: TimeInterval) throws -> RFPacket? {
