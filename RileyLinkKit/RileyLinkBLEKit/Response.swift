...
struct UpdateRegisterResponse: Response {
    ...
    init?(data: Data) {
        guard !data.isEmpty, let code = ResponseCode(rawValue: data[data.startIndex]) else {
            return nil
        }
    ...
    init?(legacyData data: Data) {
        guard !data.isEmpty, let code = LegacyCode(rawValue: data[data.startIndex])?.responseCode else {
            return nil
        }
...
struct GetStatisticsResponse: Response {
    ...
    private init?(code: ResponseCode, data: Data) {
        ...
        statistics = RileyLinkStatistics(
            uptime: uptime,
            radioRxOverflowCount: radioRxOverflowCount,
            radioRxFifoOverflowCount: radioRxFifoOverflowCount,
            packetRxCount: packetRxCount,
            packetTxCount: packetTxCount,
            crcFailureCount: crcFailureCount,
            spiSyncFailureCount: spiSyncFailureCount
        )
    }
}
