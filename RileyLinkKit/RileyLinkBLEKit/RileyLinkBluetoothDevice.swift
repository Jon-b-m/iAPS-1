...
public class RileyLinkBluetoothDevice: RileyLinkDevice {
    ...
    public var isConnected: Bool {
        manager.peripheral.state == .connected
    }
    ...
    public var hasOrangeLinkService: Bool {
        manager.peripheral.services?.itemWithUUID(RileyLinkServiceUUID.orange.cbUUID) != nil
    }

    public var hardwareType: RileyLinkHardwareType? {
        guard let services = manager.peripheral.services else {
            return nil
        }

        guard let bleComponents = bleFirmwareVersion else {
            return nil
        }
    ...
    init(peripheralManager: PeripheralManager, rssi: Int?) {
        ...
        sessionQueueOperationCountObserver = sessionQueue.observe(\.operationCount, options: [.new]) { [weak self] _, change in
...
public extension RileyLinkBluetoothDevice {
    var name: String? {
        manager.peripheral.name
    }

    var deviceURI: String {
        "rileylink://\(name ?? peripheralIdentifier.uuidString)"
    }

    var peripheralIdentifier: UUID {
        manager.peripheral.identifier
    }

    var peripheralState: CBPeripheralState {
        manager.peripheral.state
    }
...
public extension RileyLinkBluetoothDevice {
    func runSession(withName name: String, _ block: @escaping (_ session: CommandSession) -> Void) {
        log.default("Scheduling session %{public}@", name)
        sessionQueue.addOperation(manager.configureAndRun({ [weak self] manager in
            ...
            if bleFirmwareVersion == nil || radioFirmwareVersion == nil {
                self?.log.error(
                    "Running session with incomplete configuration: bleFirmwareVersion %{public}@, radioFirmwareVersion: %{public}@",
                    String(describing: bleFirmwareVersion),
                    String(describing: radioFirmwareVersion)
                )
            }

            block(CommandSession(
                manager: manager,
                responseType: bleFirmwareVersion?.responseType ?? .buffered,
...
public extension RileyLinkBluetoothDevice {
    enum IdleListeningState {
        case enabled(timeout: TimeInterval, channel: UInt8)
        case disabled
    }

    internal func setIdleListeningState(_ state: IdleListeningState) {
        os_unfair_lock_lock(&lock)
    ...
    func assertIdleListening(forceRestart: Bool = false) {
        os_unfair_lock_lock(&lock)
        guard case let .enabled(timeout: timeout, channel: channel) = idleListeningState else {
            os_unfair_lock_unlock(&lock)
            return
        }
        ...
        guard forceRestart || (lastIdle ?? .distantPast).timeIntervalSinceNow < -timeout else {
            os_unfair_lock_unlock(&lock)
            return
        }

        guard !isIdleListeningPending else {
            os_unfair_lock_unlock(&lock)
            return
        }

        isIdleListeningPending = true
        os_unfair_lock_unlock(&lock)

        manager.startIdleListening(idleTimeout: timeout, channel: channel) { error in
...
extension RileyLinkBluetoothDevice {
    ...
    func assertTimerTick() {
        ...
        if isTimerTickEnabled != manager.timerTickEnabled {
            manager.setTimerTickEnabled(isTimerTickEnabled)
        }
    }
}
...
extension RileyLinkBluetoothDevice: PeripheralManagerDelegate {
    func peripheralManager(_: PeripheralManager, didUpdateNotificationStateFor characteristic: CBCharacteristic) {
    ...
    func peripheralManager(_: PeripheralManager, didUpdateValueFor characteristic: CBCharacteristic) {
        let characteristicService: CBService? = characteristic.service
        guard let cbService = characteristicService,
              let service = RileyLinkServiceUUID(rawValue: cbService.uuid.uuidString)
        else {
    ...
    func peripheralManager(_: PeripheralManager, didReadRSSI RSSI: NSNumber, error _: Error?) {
    ...
    func completeConfiguration(for manager: PeripheralManager) throws {
        ...
        let radioVersionString = try manager.readRadioFirmwareVersion(
            timeout: 1,
            responseType: bleFirmwareVersion?.responseType ?? .buffered
        )
...
public extension RileyLinkBluetoothDevice {
    static let notificationPacketKey = "com.rileylink.RileyLinkBLEKit.RileyLinkDevice.NotificationPacket"

    static let notificationRSSIKey = "com.rileylink.RileyLinkBLEKit.RileyLinkDevice.NotificationRSSI"

    static let batteryLevelKey = "com.rileylink.RileyLinkBLEKit.RileyLinkDevice.BatteryLevel"
}

public extension Notification.Name {
    static let DeviceConnectionStateDidChange = Notification
        .Name(rawValue: "com.rileylink.RileyLinkBLEKit.ConnectionStateDidChange")

    static let DeviceDidStartIdle = Notification.Name(rawValue: "com.rileylink.RileyLinkBLEKit.DidStartIdle")

    static let DeviceNameDidChange = Notification.Name(rawValue: "com.rileylink.RileyLinkBLEKit.NameDidChange")

    static let DevicePacketReceived = Notification.Name(rawValue: "com.rileylink.RileyLinkBLEKit.PacketReceived")

    static let DeviceRSSIDidChange = Notification.Name(rawValue: "com.rileylink.RileyLinkBLEKit.RSSIDidChange")

    static let DeviceTimerDidTick = Notification.Name(rawValue: "com.rileylink.RileyLinkBLEKit.TimerTickDidChange")

    static let DeviceStatusUpdated = Notification.Name(rawValue: "com.rileylink.RileyLinkBLEKit.DeviceStatusUpdated")

    static let DeviceBatteryLevelUpdated = Notification.Name(rawValue: "com.rileylink.RileyLinkBLEKit.BatteryLevelUpdated")
}
