...
extension CBUUIDRawValue where RawValue == String {
    var cbUUID: CBUUID {
        CBUUID(string: rawValue.uppercased())
    }
}
...
private extension CBPeripheral {
    ...
    func getCharacteristicWithUUID(
        _ uuid: MainServiceCharacteristicUUID,
        serviceUUID: RileyLinkServiceUUID = .main
    ) -> CBCharacteristic? {
...
extension PeripheralManager {
        ...
        perform { _ in
            do {
                guard
                    ...
                    let mode = RileyLinkLEDMode(rawValue: data[0])
                else {
                    completion(nil)
                    return
                }
    ...
    var timerTickEnabled: Bool {
        peripheral.getCharacteristicWithUUID(.timerTick)?.isNotifying ?? false
    }

    func setTimerTickEnabled(
        _ enabled: Bool,
        timeout: TimeInterval = expectedMaxBLELatency,
        completion: ((_ error: RileyLinkDeviceError?) -> Void)? = nil
    ) {
        perform { manager in
            do {
    ...
    func setLEDMode(mode: RileyLinkLEDMode) {
        perform { manager in
            do {
                ...
                try manager.writeValue(
                    value,
                    for: characteristic,
                    type: .withResponse,
                    timeout: PeripheralManager.expectedMaxBLELatency
                )
            } catch {
    ...
    func startIdleListening(
        idleTimeout: TimeInterval,
        channel: UInt8,
        timeout: TimeInterval = expectedMaxBLELatency,
        completion: @escaping (_ error: RileyLinkDeviceError?) -> Void
    ) {
        perform { manager in
...
extension PeripheralManager {
    ...
    func writeCommand<C: Command>(_ command: C, timeout: TimeInterval, responseType: ResponseType) throws -> C.ResponseType {
        guard let characteristic = peripheral.getCharacteristicWithUUID(.data) else {
            throw RileyLinkDeviceError
                .peripheralManagerError(PeripheralManagerError.unknownCharacteristic(MainServiceCharacteristicUUID.data.cbUUID))
        }
    ...
    ///     - RileyLinkDeviceError.peripheralManagerError
    ///     - RileyLinkDeviceError.writeSizeLimitExceeded
    private func writeCommandWithoutResponse<C: Command>(_ command: C, timeout: TimeInterval) throws {
        guard let characteristic = peripheral.getCharacteristicWithUUID(.data) else {
            throw RileyLinkDeviceError
                .peripheralManagerError(PeripheralManagerError.unknownCharacteristic(MainServiceCharacteristicUUID.data.cbUUID))
        }
        ...
        guard let characteristic = peripheral.getCharacteristicWithUUID(.firmwareVersion) else {
            throw RileyLinkDeviceError
                .peripheralManagerError(
                    PeripheralManagerError
                        .unknownCharacteristic(MainServiceCharacteristicUUID.firmwareVersion.cbUUID)
                )
        }
...
extension PeripheralManager {
    ...
    ///     - RileyLinkDeviceError.invalidResponse
    ///     - RileyLinkDeviceError.peripheralManagerError
    private func writeCommand<R: Response>(
        _ data: Data,
    ...
    ) throws -> R
    {
        ...
        do {
            try runCommand(timeout: timeout) {
                ...
                addCondition(.valueUpdate(characteristic: characteristic, matching: { value in
                    ...
                    guard let code = ResponseCode(rawValue: value[0]) else {
                        ...
                        log.error(
                            "Unknown response code from RileyLink: %{public}@. Continuing to listen for command response.",
                            unknownCode
                        )
                        return false
                    }
            ...
            if case let .timeout(unmetConditions) = error,
    ...
    ///     - RileyLinkDeviceError.invalidResponse
    ///     - RileyLinkDeviceError.peripheralManagerError
    private func writeLegacyCommand<R: Response>(
        _ data: Data,
        for characteristic: CBCharacteristic,
        type: CBCharacteristicWriteType = .withResponse,
        timeout: TimeInterval,
                ...
                addCondition(.valueUpdate(characteristic: characteristic, matching: { value in
                    ...
                    for response in buffer.responses {
                        ...
                        case .commandInterrupted:
                            // This is expected in cases where an "Idle" GetPacket command is running
                            log.debug(
                                "writeLegacyCommand response (commandInterrupted): %{public}@",
