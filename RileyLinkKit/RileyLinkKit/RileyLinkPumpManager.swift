...
open class RileyLinkPumpManager {
    ...
    public init(rileyLinkDeviceProvider: RileyLinkDeviceProvider) {
        ...
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(receivedRileyLinkPacketNotification(_:)),
            name: .DevicePacketReceived,
            object: nil
        )
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(receivedRileyLinkTimerTickNotification(_:)),
            name: .DeviceTimerDidTick,
            object: nil
        )
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(receivedRileyLinkBatteryUpdate(_:)),
    ...
    open func device(_: RileyLinkDevice, didReceivePacket _: RFPacket) {}

    open func deviceTimerDidTick(_: RileyLinkDevice) {}

    open func device(_: RileyLinkDevice, didUpdateBattery _: Int) {}
...
extension RileyLinkPumpManager {
    ...
    @objc private func receivedRileyLinkTimerTickNotification(_ note: Notification) {
        ...
        lastTimerTick.value = Date()
        deviceTimerDidTick(device)
    }
