...
public class RileyLinkDeviceTableViewController: UITableViewController {
    ...
    private func observe() {
        ...
        notificationObservers = [
            center.addObserver(forName: .DeviceNameDidChange, object: device, queue: mainQueue) { [weak self] (_) -> Void in
            ...
            center
                .addObserver(
                    forName: .DeviceConnectionStateDidChange,
                    object: device,
                    queue: mainQueue
                ) { [weak self] (_) -> Void in
            ...
            center.addObserver(forName: .DeviceDidStartIdle, object: device, queue: mainQueue) { [weak self] _ in
                self?.updateDeviceStatus()
            },
            center.addObserver(forName: .DeviceStatusUpdated, object: device, queue: mainQueue) { [weak self] _ in
                self?.updateDeviceStatus()
            }
        ]
    }
    ...
    private func cellForRow(_ row: DeviceRow) -> UITableViewCell? {
        guard let rowIndex = deviceRows.firstIndex(of: row),
              let sectionIndex = sections.firstIndex(of: Section.device)
        else {
            return nil
        }
    ...
    private func cellForRow(_ row: OrangeConfigureCommandRow) -> UITableViewCell? {
        guard let sectionIndex = sections.firstIndex(of: Section.orangeLinkCommands)
        else {
            return nil
        }
    ...
    private func cellForRow(_ row: RileyLinkCommandRow) -> UITableViewCell? {
        guard let sectionIndex = sections.firstIndex(of: Section.rileyLinkCommands)
        else {
            return nil
        }
        return tableView.cellForRow(at: IndexPath(row: row.rawValue, section: sectionIndex))
    ...
    @objc func switchAction(sender: RileyLinkSwitch) {
    ...
    override public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .device:
            ...
            case .connection:
                cell.textLabel?.text = LocalizedString(
                    "Connection State",
                    comment: "The title of the cell showing BLE connection state"
                )
                cell.detailTextLabel?.text = device.peripheralState.description
            case .rssi:
                cell.textLabel?.text = LocalizedString(
                    "Signal Strength",
                    comment: "The title of the cell showing BLE signal strength (RSSI)"
                )
            ...
            case .frequency:
                cell.textLabel?.text = LocalizedString(
                    "Frequency",
                    comment: "The title of the cell showing current rileylink frequency"
                )
        ...
        case .alert:
            ...
            case .battery:
                ...
                cell.textLabel?.text = LocalizedString(
                    "Low Battery Alert",
                    comment: "The title of the cell showing battery level"
                )
        ...
        case .rileyLinkCommands:
            ...
            case .diagnosticLEDSMode:
                cell.textLabel?.text = LocalizedString(
                    ...
                    comment: "The title of the command to update diagnostic LEDs"
                )
                cell.setLEDMode(ledMode)
            case .getStatistics:
                cell.textLabel?.text = LocalizedString(
                    "Get RileyLink Statistics",
                    comment: "The title of the command to fetch RileyLink statistics"
                )
            }
        case .orangeLinkCommands:
            cell.accessoryType = .disclosureIndicator
            ...
            case .yellow:
                ...
                cell.textLabel?.text = LocalizedString(
                    "Lighten Yellow LED",
                    comment: "The title of the cell showing Lighten Yellow LED"
                )
            case .red:
                switchView.isHidden = false
                cell.accessoryType = .none
                ...
                cell.textLabel?.text = LocalizedString(
                    "Lighten Red LED",
                    comment: "The title of the cell showing Lighten Red LED"
                )
            case .shake:
                switchView.isHidden = false
            ...
            case .findDevice:
                cell.textLabel?.text = LocalizedString(
                    "Find Device",
                    comment: "The title of the cell for sounding device finding piezo"
                )
                cell.detailTextLabel?.text = nil
            }
        case .configureCommand:
            ...
            case .connectionVibrate:
                ...
                cell.textLabel?.text = LocalizedString(
                    ...
                    comment: "The title of the cell for connection vibration"
                )
            }
        }
...
private extension UITableViewCell {
    ...
    func setBatteryAlert(_ level: Int?, formatter: NumberFormatter) {
        detailTextLabel?.text = formatter
            .percentString(from: level) ?? LocalizedString("Off", comment: "Detail text when battery alert disabled.")
    }
}
