...
public class RileyLinkDevicesTableViewDataSource: NSObject {
    ...
    public var tableView: UITableView! {
        didSet {
            ...
            tableView.register(
                RileyLinkDevicesHeaderView.self,
                forHeaderFooterViewReuseIdentifier: RileyLinkDevicesHeaderView.className
            )
    ...
    public private(set) var devices: [RileyLinkDevice] = [] {
        didSet {
            ...
            let insertedPaths = (oldValue.count ..< devices.count).map { (index) -> IndexPath in
                IndexPath(row: index, section: devicesSectionIndex)
            }
    ...
    private func preferenceStateForDevice(_ device: RileyLinkDevice) -> CBPeripheralState? {
        let isAutoConnectDevice = rileyLinkPumpManager.rileyLinkDeviceProvider
            .shouldConnect(to: device.peripheralIdentifier.uuidString)
    ...
    @objc private func deviceDidUpdate(_ note: Notification) {
        DispatchQueue.main.async {
            if let device = note.object as? RileyLinkDevice,
               let index = self.devices.firstIndex(where: { $0.peripheralIdentifier == device.peripheralIdentifier })
            {
                ...
                if let cell = self.tableView
                    .cellForRow(at: IndexPath(row: index, section: self.devicesSectionIndex)) as? RileyLinkDeviceTableViewCell
                {
...
extension RileyLinkDevicesTableViewDataSource: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let deviceCell = tableView
            .dequeueReusableCell(withIdentifier: RileyLinkDeviceTableViewCell.className) as! RileyLinkDeviceTableViewCell
        let device = devices[indexPath.row]

        deviceCell.configureCellWithName(
            device.name,
            signal: decimalFormatter.decibleString(from: deviceRSSI[device.peripheralIdentifier]),
            peripheralState: preferenceStateForDevice(device)
        )
    ...
    public func tableView(_: UITableView, titleForHeaderInSection _: Int) -> String? {
        LocalizedString("Devices", comment: "The title of the devices table section in RileyLink settings")
    }
}

extension RileyLinkDevicesTableViewDataSource: UITableViewDelegate {
    public func tableView(_ tableView: UITableView, viewForHeaderInSection _: Int) -> UIView? {
        tableView.dequeueReusableHeaderFooterView(withIdentifier: RileyLinkDevicesHeaderView.className)
    }
    ...
    public func tableView(_: UITableView, estimatedHeightForHeaderInSection _: Int) -> CGFloat {
        55
    }
