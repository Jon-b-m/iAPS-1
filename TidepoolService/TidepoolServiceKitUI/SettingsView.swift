...
public struct SettingsView: View {
    ...
    public init(service: TidepoolService, login: ((TEnvironment) async throws -> Void)?, dismiss: (() -> Void)?)
    {
        ...
        _selectedEnvironment = State(
            initialValue: service.session?.environment ?? defaultEnvironment ?? TEnvironment
    ...
    public var body: some View {
                    ...
                    VStack(spacing: 20) {
                        ...
                        if selectedEnvironment != TEnvironment.productionEnvironment {
                            VStack {
                                ...
                                if isLoggedIn {
                                    Button(
                                        LocalizedString("Revoke token", comment: "Button title to revoke oauth tokens"),
                                        action: {
                                            Task {
                                                do {
        ...
        .alert(
            LocalizedString(
                ...
                comment: "Confirmation message for deleting a service"
            ),
    ...
    private var environmentActionSheet: ActionSheet {
        ...
        return ActionSheet(
            title: Text(LocalizedString("Environment", comment: "Tidepool login environment action sheet title")),
            message: Text(selectedEnvironment.description),
            buttons: buttons
        )
    }
    ...
    private var closeButtonTitle: String {
        LocalizedString("Close", comment: "Close navigation button title of an onboarding section page view") }
}
