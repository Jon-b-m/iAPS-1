...
class StoredDosingDecisionTests: XCTestCase {
    func testDatumDosingDecision() {
        let data = try! Self.encoder
            .encode(
                StoredDosingDecision.test
                    .datumDosingDecision(for: "1234567890", hostIdentifier: "Loop", hostVersion: "1.2.3")
            )
        XCTAssertEqual(
    ...
    func testDatumControllerStatus() {
        let data = try! Self.encoder
            .encode(
                StoredDosingDecision.test
                    .datumControllerStatus(for: "1234567890", hostIdentifier: "Loop", hostVersion: "1.2.3")
            )
        XCTAssertEqual(
            String(data: data, encoding: .utf8),
            """
    ...
    func testDatumPumpStatus() {
        let data = try! Self.encoder
            .encode(StoredDosingDecision.test.datumPumpStatus(for: "1234567890", hostIdentifier: "Loop", hostVersion: "1.2.3"))
        XCTAssertEqual(
            String(data: data, encoding: .utf8),
            """
            {
