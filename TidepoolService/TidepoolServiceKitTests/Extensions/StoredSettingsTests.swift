...
class StoredSettingsTests: XCTestCase {
    func testDatumControllerSettings() {
        let data = try! Self.encoder
            .encode(StoredSettings.test.datumControllerSettings(for: "1234567890", hostIdentifier: "Loop", hostVersion: "1.2.3"))
        XCTAssertEqual(
    ...
    func testDatumCGMSettings() {
        let data = try! Self.encoder
            .encode(StoredSettings.test.datumCGMSettings(for: "1234567890", hostIdentifier: "Loop", hostVersion: "1.2.3"))
        XCTAssertEqual(
            String(data: data, encoding: .utf8),
            """
            {
    ...
    func testDatumPumpSettings() {
        let data = try! Self.encoder
            .encode(StoredSettings.test.datumPumpSettings(for: "1234567890", hostIdentifier: "Loop", hostVersion: "1.2.3"))
        XCTAssertEqual(
            String(data: data, encoding: .utf8),
            """
            {
    ...
    func testDatumPumpSettingsOverrideDeviceEvent() {
        let data = try! Self.encoder
            .encode(
                StoredSettings.test
                    .datumPumpSettingsOverrideDeviceEvent(for: "1234567890", hostIdentifier: "Loop", hostVersion: "1.2.3")
            )
        XCTAssertEqual(
            String(data: data, encoding: .utf8),
            """
