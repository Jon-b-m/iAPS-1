...
class TidepoolServiceTests: XCTestCase {
    ...
    func testCalculateDosingDecisionData() {
        ...
        let created = tidepoolService.calculateDosingDecisionData(
            dosingDecisions,
            for: userID,
            hostIdentifier: "Loop",
            hostVersion: "1.2.3"
    ...
    func testCalculateSettingsDataIndividual() {
        ...
        let (
            created,
            updated,
            lastControllerSettings,
            lastCGMSettings,
            lastPumpSettings,
            lastPumpSettingsOverride
        ) = tidepoolService.calculateSettingsData(
            settings,
            for: userID,
            hostIdentifier: "Loop",
            hostVersion: "1.2.3"
    ...
    func testCalculateSettingsDataUpdateControllerSettings() {
        let settings = [
            ...
            StoredSettings(
                controllerDevice: StoredSettings.ControllerDevice(name: "Controller #2"),
                cgmDevice: HKDevice(name: "CGM #1"),
                pumpDevice: HKDevice(name: "Pump #1")
            )
        ]
    ...
    func testCalculateSettingsDataPumpOverrideMultipleAllCreated() {
        let settings = [
            StoredSettings(pumpDevice: HKDevice(name: "Pump #1")),
            StoredSettings(
                scheduleOverride: TemporaryScheduleOverride(duration: .minutes(30)),
                pumpDevice: HKDevice(name: "Pump #1")
            ),
            StoredSettings(
...
private extension StoredSettings.ControllerDevice {
    init(name: String) {
        self.init(name: name, systemName: "", systemVersion: "", model: "", modelIdentifier: "")
    }
}

private extension HKDevice {
