...
extension SyncCarbObject: IdentifiableHKDatum {
    func datum(for userId: String, hostIdentifier: String, hostVersion: String) -> TFoodDatum? {
        ...
        return TFoodDatum(time: datumTime, name: datumName, nutrition: datumNutrition)
            .adornWith(id: id, payload: datumPayload, origin: origin)
    }
    ...
    private var datumNutrition: TFoodDatum.Nutrition {
        TFoodDatum.Nutrition(carbohydrate: datumCarbohydrate, estimatedAbsorptionDuration: absorptionTime)
    }

    private var datumCarbohydrate: TFoodDatum.Nutrition.Carbohydrate {
        TFoodDatum.Nutrition.Carbohydrate(net: quantity.doubleValue(for: .gram()), units: .grams)
    }
