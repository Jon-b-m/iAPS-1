...
extension StoredDosingDecision: IdentifiableDatum {
    func datumDosingDecision(for userId: String, hostIdentifier: String, hostVersion: String) -> TDosingDecisionDatum {
        ...
        let datum = TDosingDecisionDatum(
            time: datumTime,
            reason: datumReason,
        ...
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TDosingDecisionDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        return datum.adornWith(
            id: datumId(for: userId, type: TDosingDecisionDatum.self),
    ...
    func datumControllerStatus(for userId: String, hostIdentifier: String, hostVersion: String) -> TControllerStatusDatum {
        let datum = TControllerStatusDatum(
            time: datumTime,
            battery: datumControllerBattery
        )
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TControllerStatusDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        return datum.adornWith(
            id: datumId(for: userId, type: TControllerStatusDatum.self),
            timeZone: datumTimeZone,
            timeZoneOffset: datumTimeZoneOffset,
            payload: datumPayload,
            origin: origin
    ...
    func datumPumpStatus(for userId: String, hostIdentifier: String, hostVersion: String) -> TPumpStatusDatum {
        ...
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TPumpStatusDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        return datum.adornWith(
            id: datumId(for: userId, type: TPumpStatusDatum.self),
            timeZone: datumTimeZone,
            timeZoneOffset: datumTimeZoneOffset,
            payload: datumPayload,
            origin: origin
    ...
    private var datumOriginalFood: TDosingDecisionDatum.Food? {
        ...
        let carbohydrate = TDosingDecisionDatum.Nutrition.Carbohydrate(
            net: originalCarbEntry.quantity.doubleValue(for: .gram()),
            units: .grams
        )
        let nutrition = TDosingDecisionDatum.Nutrition(
            carbohydrate: carbohydrate,
            estimatedAbsorptionDuration: originalCarbEntry.absorptionTime
        )
    ...
    private var datumFood: TDosingDecisionDatum.Food? {
        ...
        let carbohydrate = TDosingDecisionDatum.Nutrition.Carbohydrate(
            net: carbEntry.quantity.doubleValue(for: .gram()),
            units: .grams
        )
        let nutrition = TDosingDecisionDatum.Nutrition(
            carbohydrate: carbohydrate,
            estimatedAbsorptionDuration: carbEntry.absorptionTime
        )
    ...
    private var datumSelfMonitoredBloodGlucose: TDosingDecisionDatum.BloodGlucose? {
        ...
        return TDosingDecisionDatum.BloodGlucose(
            time: manualGlucoseSample.startDate,
    ...
    private var datumCarbohydratesOnBoard: TDosingDecisionDatum.CarbohydratesOnBoard? {
        ...
        return TDosingDecisionDatum.CarbohydratesOnBoard(
            time: carbsOnBoard.startDate,
            amount: carbsOnBoard.quantity.doubleValue(for: .gram()).rounded(decimalPlaces: 4)
        )
    }
    ...
    private var datumInsulinOnBoard: TDosingDecisionDatum.InsulinOnBoard? {
        ...
        return TDosingDecisionDatum.InsulinOnBoard(
            time: insulinOnBoard.startDate,
            amount: insulinOnBoard.value.rounded(decimalPlaces: 4)
        )
    }
    ...
    private var datumBloodGlucoseTargetSchedule: [TDosingDecisionDatum.BloodGlucoseStartTarget]? {
        ...
        return glucoseTargetRangeSchedule.items(for: .milligramsPerDeciliter)
            .map {
                TDosingDecisionDatum
                    .BloodGlucoseStartTarget(start: $0.startTime, low: $0.value.minValue, high: $0.value.maxValue) }
    }
    ...
    private var datumHistoricalBloodGlucose: [TDosingDecisionDatum.BloodGlucose]? {
        ...
        return historicalGlucose
            .map {
                TDosingDecisionDatum
                    .BloodGlucose(time: $0.startDate, value: $0.quantity.doubleValue(for: .milligramsPerDeciliter)) }
    }
    ...
    private var datumForecastBloodGlucose: [TDosingDecisionDatum.BloodGlucose]? {
        ...
        return predictedGlucose.map { TDosingDecisionDatum.BloodGlucose(
            time: $0.startDate,
            value: $0.quantity.doubleValueClampedAndRounded(for: .milligramsPerDeciliter)
        ) }
    }
    ...
    private var datumUnits: TDosingDecisionDatum.Units {
        TDosingDecisionDatum.Units(bloodGlucose: .milligramsPerDeciliter, carbohydrate: .grams, insulin: .units)
    }

    private var datumControllerBattery: TControllerStatusDatum.Battery? {
        guard let controllerStatus = controllerStatus,
              controllerStatus.batteryState != nil || controllerStatus.batteryLevel != nil
        else {
            return nil
        }
        return TControllerStatusDatum.Battery(
            state: controllerStatus.batteryState?.datum,
    ...
    private var datumBasalDelivery: TPumpStatusDatum.BasalDelivery? {
        pumpManagerStatus?.basalDeliveryState?.datum
    }
    ...
    private var datumBolusDelivery: TPumpStatusDatum.BolusDelivery? {
        pumpManagerStatus?.bolusState.datum
    }
...
private extension StoredDosingDecision.ControllerStatus.BatteryState {
...
private extension PumpManagerStatus.BolusState {
...
private extension DoseEntry {
    var temporaryBasalDatum: TPumpStatusDatum.BasalDelivery.Dose {
        TPumpStatusDatum.BasalDelivery.Dose(
            startTime: startDate,
            endTime: endDate,
            rate: unitsPerHour,
            amountDelivered: deliveredUnits
        )
    }

    var bolusDatum: TPumpStatusDatum.BolusDelivery.Dose {
        TPumpStatusDatum.BolusDelivery.Dose(startTime: startDate, amount: programmedUnits, amountDelivered: deliveredUnits)
    }
}
