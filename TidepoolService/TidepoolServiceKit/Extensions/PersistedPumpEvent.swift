...
extension PersistedPumpEvent: IdentifiableDatum {
    ...
    private func dataForAlarm(for userId: String, hostIdentifier: String, hostVersion: String) -> [TDatum] {
        ...
        if let alarmType = alarmType, case let .other(details) = alarmType {
        ...
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TAlarmDeviceEventDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        datum = datum.adornWith(
            id: datumId(for: userId, type: TAlarmDeviceEventDatum.self),
            payload: payload,
            origin: origin
        )
        data.append(datum)
    ...
    private func dataForPrime(for userId: String, hostIdentifier: String, hostVersion: String) -> [TDatum] {
        ...
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TPrimeDeviceEventDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        datum = datum.adornWith(
            id: datumId(for: userId, type: TPrimeDeviceEventDatum.self),
    ...
    private func dataForResume(for userId: String, hostIdentifier: String, hostVersion: String) -> [TDatum] {
        ...
        var datum = TStatusDeviceEventDatum(
            time: datumTime,
            name: .resumed,
            reason: reason
        )
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TStatusDeviceEventDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        datum = datum.adornWith(
            id: datumId(for: userId, type: TStatusDeviceEventDatum.self),
            payload: datumPayload,
            origin: origin
        )
        return [datum]
    ...
    private func dataForRewind(for userId: String, hostIdentifier: String, hostVersion: String) -> [TDatum] {
        ...
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TReservoirChangeDeviceEventDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        datum = datum.adornWith(
            id: datumId(for: userId, type: TReservoirChangeDeviceEventDatum.self),
            payload: datumPayload,
            origin: origin
        )
        data.append(datum)
...
private extension PumpAlarmType {
...
extension TReservoirChangeDeviceEventDatum: TypedDatum {
    static var resolvedType: String {
        "\(TDatum.DatumType.deviceEvent.rawValue)/\(TDeviceEventDatum.SubType.reservoirChange.rawValue)" }
}
