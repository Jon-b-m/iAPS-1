...
extension DoseEntry: IdentifiableDatum {
    ...
    var syncIdentifierAsString: String { syncIdentifier!.md5hash!
    } // Actual sync identifier may be human readable and of variable length
    ...
    private func dataForBasalManual(for userId: String, hostIdentifier: String, hostVersion: String) -> [TDatum] {
        ...
        var datum = TScheduledBasalDatum(
            time: datumTime,
        ...
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TScheduledBasalDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        datum = datum.adornWith(
            id: datumId(for: userId, type: TScheduledBasalDatum.self),
    ...
    private func dataForBasalAutomatic(for userId: String, hostIdentifier: String, hostVersion: String) -> [TDatum] {
        ...
        var datum = TAutomatedBasalDatum(
            time: datumTime,
            duration: !isMutable ? datumDuration : 0,
            expectedDuration: !isMutable && datumDuration < basalDatumExpectedDuration ?
                basalDatumExpectedDuration : nil,
            rate: datumScheduledBasalRate,
            scheduleName: StoredSettings.activeScheduleNameDefault,
            insulinFormulation: datumInsulinFormulation
        )
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TAutomatedBasalDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        datum = datum.adornWith(
            id: datumId(for: userId, type: TAutomatedBasalDatum.self),
            annotations: datumAnnotations,
            payload: payload,
            origin: origin
        )
...
//        } else
    ...
    private func dataForBolusManuallyEntered(for userId: String, hostIdentifier: String, hostVersion: String) -> [TDatum] {
        ...
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TInsulinDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        datum = datum.adornWith(
            id: datumId(for: userId, type: TInsulinDatum.self),
            annotations: datumAnnotations,
            payload: payload,
            origin: origin
        )
    ...
    private func dataForBolusManual(for userId: String, hostIdentifier: String, hostVersion: String) -> [TDatum] {
        ...
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TNormalBolusDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        datum = datum.adornWith(
            id: datumId(for: userId, type: TNormalBolusDatum.self),
            annotations: datumAnnotations,
            payload: payload,
            origin: origin
        )
    ...
    private func dataForBolusAutomatic(for userId: String, hostIdentifier: String, hostVersion: String) -> [TDatum] {
        ...
        var datum = TAutomatedBolusDatum(
            time: datumTime,
            normal: !isMutable ? deliveredUnits : programmedUnits,
            expectedNormal: !isMutable && programmedUnits != deliveredUnits ? programmedUnits : nil,
            insulinFormulation: datumInsulinFormulation
        ...
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TAutomatedBolusDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        datum = datum.adornWith(
            id: datumId(for: userId, type: TAutomatedBolusDatum.self),
            annotations: datumAnnotations,
            payload: payload,
            origin: origin
        )
    ...
    private func dataForSuspend(for userId: String, hostIdentifier: String, hostVersion: String) -> [TDatum] {
        var datum = TSuspendedBasalDatum(
            time: datumTime,
            duration: datumDuration
        )
        datum.suppressed = datumSuppressed
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TSuspendedBasalDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        datum = datum.adornWith(
            id: datumId(for: userId, type: TSuspendedBasalDatum.self),
            annotations: datumAnnotations,
    ...
    private func dataForTempBasalManual(for userId: String, hostIdentifier: String, hostVersion: String) -> [TDatum] {
        ...
        var datum = TTemporaryBasalDatum(
            time: datumTime,
            duration: !isMutable ? datumDuration : 0,
            expectedDuration: !isMutable && datumDuration < basalDatumExpectedDuration ?
                basalDatumExpectedDuration : nil,
        ...
        datum.suppressed = datumSuppressed
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TTemporaryBasalDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        datum = datum.adornWith(
            id: datumId(for: userId, type: TTemporaryBasalDatum.self),
            annotations: datumAnnotations,
            payload: payload,
            origin: origin
        )
    ...
    private var datumSuppressed: TScheduledBasalDatum.Suppressed? {
        ...
        return TScheduledBasalDatum.Suppressed(
            rate: datumScheduledBasalRate,
            scheduleName: StoredSettings.activeScheduleNameDefault
        )
    }
