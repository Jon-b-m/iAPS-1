...
extension StoredSettings: IdentifiableDatum {
    func datumControllerSettings(for userId: String, hostIdentifier: String, hostVersion: String) -> TControllerSettingsDatum {
        let datum = TControllerSettingsDatum(
            time: datumTime,
            device: datumControllerDevice,
            notifications: datumControllerNotifications
        )
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TControllerSettingsDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        return datum.adornWith(
            id: datumId(for: userId, type: TControllerSettingsDatum.self),
    ...
    func datumCGMSettings(for userId: String, hostIdentifier: String, hostVersion: String) -> TCGMSettingsDatum {
        let datum = TCGMSettingsDatum(
            ...
            scheduledAlerts: nil
        ) // TODO: https://tidepool.atlassian.net/browse/LOOP-3929
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TCGMSettingsDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        return datum.adornWith(
            id: datumId(for: userId, type: TCGMSettingsDatum.self),
            timeZone: datumTimeZone,
            timeZoneOffset: datumTimeZoneOffset,
            payload: datumPayload,
            origin: origin
    ...
    func datumPumpSettings(for userId: String, hostIdentifier: String, hostVersion: String) -> TPumpSettingsDatum {
        ...
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TPumpSettingsDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        return datum.adornWith(
            id: datumId(for: userId, type: TPumpSettingsDatum.self),
            timeZone: datumTimeZone,
            timeZoneOffset: datumTimeZoneOffset,
            payload: datumPayload,
            origin: origin
    ...
    func datumPumpSettingsOverrideDeviceEvent(
        for userId: String,
        ...
        let datum = TPumpSettingsOverrideDeviceEventDatum(
            time: activeOverride.datumTime,
            ...
            carbohydrateRatioScaleFactor: activeOverride
                .datumCarbohydrateRatioScaleFactor,
            insulinSensitivityScaleFactor: activeOverride
                .datumInsulinSensitivityScaleFactor,
            units: activeOverride.datumUnits
        )
        let origin = datumOrigin(
            for: resolvedIdentifier(for: TPumpSettingsOverrideDeviceEventDatum.self),
            hostIdentifier: hostIdentifier,
            hostVersion: hostVersion
        )
        return datum.adornWith(
            id: datumId(for: userId, type: TPumpSettingsOverrideDeviceEventDatum.self),
            timeZone: datumTimeZone,
            timeZoneOffset: datumTimeZoneOffset,
            payload: datumPayload,
            origin: origin
    ...
    private var datumControllerDevice: TControllerSettingsDatum.Device? {
        ...
        return TControllerSettingsDatum.Device(
            manufacturers: ["Apple"],
    ...
    private var datumPumpActiveScheduleName: String? {
        Self.activeScheduleNameDefault
    }
    ...
    private var datumPumpBasalRateSchedules: [String: [TPumpSettingsDatum.BasalRateStart]]? {
        ...
        return [
            Self.activeScheduleNameDefault: basalRateSchedule.items
                .map { TPumpSettingsDatum.BasalRateStart(start: $0.startTime, rate: $0.value) }
        ]
    }
    ...
    private var datumPumpBloodGlucoseTargetPhysicalActivity: TPumpSettingsDatum.BloodGlucoseTarget? {
        ...
        return TPumpSettingsDatum.BloodGlucoseTarget(
            low: workoutTargetRange.lowerBound.doubleValue(for: .milligramsPerDeciliter),
            high: workoutTargetRange.upperBound
    ...
    private var datumPumpBloodGlucoseTargetPreprandial: TPumpSettingsDatum.BloodGlucoseTarget? {
        ...
        return TPumpSettingsDatum.BloodGlucoseTarget(
            low: preMealTargetRange.lowerBound.doubleValue(for: .milligramsPerDeciliter),
            high: preMealTargetRange.upperBound
                .doubleValue(for: .milligramsPerDeciliter)
        )
    }
    ...
    private var datumPumpBloodGlucoseTargetSchedules: [String: [TPumpSettingsDatum.BloodGlucoseStartTarget]]? {
        ...
        return [
            Self.activeScheduleNameDefault: glucoseTargetRangeSchedule.items(for: .milligramsPerDeciliter)
                .map {
                    TPumpSettingsDatum.BloodGlucoseStartTarget(
                        start: $0.startTime,
                        low: $0.value.minValue,
                        high: $0.value.maxValue
                    ) }
        ]
    }
    ...
    private var datumPumpCarbohydrateRatioSchedules: [String: [TPumpSettingsDatum.CarbohydrateRatioStart]]? {
        ...
        return [
            Self.activeScheduleNameDefault: carbRatioSchedule.items(for: .gram())
                .map { TPumpSettingsDatum.CarbohydrateRatioStart(start: $0.startTime, amount: $0.value) }
        ]
    }
    ...
    private var datumPumpInsulinModel: TPumpSettingsDatum.InsulinModel? {
        ...
        return TPumpSettingsDatum.InsulinModel(
            modelType: modelType,
    ...
    private var datumPumpInsulinSensitivitySchedules: [String: [TPumpSettingsDatum.InsulinSensitivityStart]]? {
        ...
        return [
            Self.activeScheduleNameDefault: insulinSensitivitySchedule.items(for: .milligramsPerDeciliter)
                .map { TPumpSettingsDatum.InsulinSensitivityStart(start: $0.startTime, amount: $0.value) }
        ]
    }
    ...
    private var datumPumpScheduleTimeZoneOffset: TimeInterval? {
        guard let scheduleTimeZone = basalRateSchedule?.timeZone ?? glucoseTargetRangeSchedule?.timeZone ?? carbRatioSchedule?
    ...
    private var datumPumpUnits: TPumpSettingsDatum.Units {
        TPumpSettingsDatum.Units(bloodGlucose: .milligramsPerDeciliter, carbohydrate: .grams, insulin: .units)
    }
    ...
    private var activeOverride: TemporaryScheduleOverride? {
        ...
        case let (preMealOverride?, scheduleOverride?):
...
private extension NotificationSettings {
    var datum: TControllerSettingsDatum.Notifications {
        TControllerSettingsDatum.Notifications(
            authorization: authorizationStatus.datum,
...
private extension NotificationSettings.AuthorizationStatus {
...
private extension NotificationSettings.NotificationSetting {
    var datum: Bool? {
        switch self {
        case .notSupported:
            return false
...
private extension NotificationSettings.AlertStyle {
...
private extension TemporaryScheduleOverridePreset {
    var datum: TPumpSettingsDatum.OverridePreset {
        TPumpSettingsDatum.OverridePreset(
            abbreviation: datumAbbreviation,
...
private extension TemporaryScheduleOverride {
    ...
    var datumOverridePreset: String? {
        guard case let .preset(preset) = context else {
            return nil
        }
        return preset.name
    }
...
private extension TemporaryScheduleOverride.Context {
...
private extension TemporaryScheduleOverrideSettings {
    var datumBloodGlucoseTarget: TPumpSettingsDatum.BloodGlucoseTarget? {
        guard let targetRange = targetRange else {
            return nil
        }
        return TPumpSettingsDatum.BloodGlucoseTarget(
            low: targetRange.lowerBound.doubleValue(for: .milligramsPerDeciliter),
...
extension TPumpSettingsOverrideDeviceEventDatum: TypedDatum {
    static var resolvedType: String {
        "\(TDatum.DatumType.deviceEvent.rawValue)/\(TDeviceEventDatum.SubType.pumpSettingsOverride.rawValue)" }
}
