...
class PodInfoTests: XCTestCase {
    func testFullMessage() {
        ...
        do {
            // Decode
            let infoResponse =
                try PodInfoResponse(encodedData: Data(hexadecimalString: "0216020d0000000000ab6a038403ff03860000285708030d0000")!)
    ...
    func testPodInfoTriggeredAlertsEmpty() {
        ...
        do {
            // Decode
            let decoded =
                try PodInfoTriggeredAlerts(encodedData: Data(hexadecimalString: "01000000000000000000000000000000000000")!)
    ...
    func testPodInfoTriggeredAlertsSuspendStillActive() {
        ...
        do {
            // Decode
            let decoded =
                try PodInfoTriggeredAlerts(encodedData: Data(hexadecimalString: "010000000000000000000000000bd70c400000")!)
            XCTAssertEqual(.triggeredAlerts, decoded.podInfoType)
    ...
    func testPodInfoTriggeredAlertsReplacePodAfter3DaysAnd8Hours() {
        ...
        do {
            let decoded =
                try PodInfoTriggeredAlerts(encodedData: Data(hexadecimalString: "010000000000000000000000000000000010e1")!)
            XCTAssertEqual(.triggeredAlerts, decoded.podInfoType)
    ...
    func testPodInfoTriggeredAlertsReplacePodAfterReservoirEmpty() {
        ...
        do {
            let decoded =
                try PodInfoTriggeredAlerts(encodedData: Data(hexadecimalString: "010000000000001285000011c700000000119c")!)
            XCTAssertEqual(.triggeredAlerts, decoded.podInfoType)
    ...
    func testPodInfoTriggeredAlertsReplacePod() {
        ...
        do {
            let decoded =
                try PodInfoTriggeredAlerts(encodedData: Data(hexadecimalString: "010000000000001284000000000000000010e0")!)
            XCTAssertEqual(.triggeredAlerts, decoded.podInfoType)
    ...
    func testPodInfoNoFaultAlerts() {
        ...
        do {
            // Decode
            let decoded =
                try DetailedStatus(encodedData: Data(hexadecimalString: "02080100000a003800000003ff008700000095ff0000")!)
    ...
    func testPodInfoDeliveryErrorDuringPriming() {
        ...
        do {
            // Decode
            let decoded =
                try DetailedStatus(encodedData: Data(hexadecimalString: "020f0000000900345c000103ff0001000005ae056029")!)
            XCTAssertEqual(.detailedStatus, decoded.podInfoType)
    ...
    func testPodInfoDuringPriming() {
        ...
        do {
            // Decode
            let decoded =
                try DetailedStatus(encodedData: Data(hexadecimalString: "020d0000000600008f000003ff0000000003a20386a0")!)
            XCTAssertEqual(.detailedStatus, decoded.podInfoType)
    ...
    func testPodInfoFaultEventErrorShuttingDown() {
        ...
        do {
            // Decode
            let decoded =
                try DetailedStatus(encodedData: Data(hexadecimalString: "020d0000000407f28609ff03ff0a0200000823080000")!)
            XCTAssertEqual(.detailedStatus, decoded.podInfoType)
            XCTAssertEqual(.faultEventOccurred, decoded.podProgressStatus)
            XCTAssertEqual(.suspended, decoded.deliveryStatus)
    ...
    func testPodInfoFaultEventCheckAboveThreshold() {
        ...
        do {
            // Decode
            let decoded =
                try DetailedStatus(encodedData: Data(hexadecimalString: "020d0000000407eb6a0e0c03ff0e1400002817080000")!)
            XCTAssertEqual(.detailedStatus, decoded.podInfoType)
            XCTAssertEqual(.faultEventOccurred, decoded.podProgressStatus)
            XCTAssertEqual(.suspended, decoded.deliveryStatus)
            XCTAssertEqual(0, decoded.bolusNotDelivered)
    ...
    func testPodInfoFaultEventBolusNotDelivered() {
        ...
        do {
            // Decode
            let decoded =
                try DetailedStatus(encodedData: Data(hexadecimalString: "020f0000010200ec6a026803ff026b000028a7082023")!)
            XCTAssertEqual(.detailedStatus, decoded.podInfoType)
            XCTAssertEqual(.inactive, decoded.podProgressStatus)
            XCTAssertEqual(.suspended, decoded.deliveryStatus)
    ...
    func testPodInfoFaultEventResetDueToLowVoltageDetect() {
        ...
        do {
            // Decode
            let decoded =
                try DetailedStatus(encodedData: Data(hexadecimalString: "020D00000000000012FFFF03FF00160000879A070000")!)
            XCTAssertEqual(.detailedStatus, decoded.podInfoType)
            XCTAssertEqual(.faultEventOccurred, decoded.podProgressStatus)
            XCTAssertEqual(.suspended, decoded.deliveryStatus)
    ...
    func testPodInfoPulseLogPlusPartial() {
        ...
        do {
            let decoded =
                try PodInfoPulseLogPlus(
                    encodedData: Data(
                        hexadecimalString: ""
                    )!
                )
            ...
            XCTAssertEqual(0x0062_2A80, decoded.pulseLog[0])
            XCTAssertEqual(0x0160_2600, decoded.pulseLog[9])
            XCTAssertEqual(0x0161_2780, decoded.pulseLog[19])
            XCTAssertEqual(0x1560_2800, decoded.pulseLog[29])
            XCTAssertEqual(0x3D60_2700, decoded.pulseLog[39])
            XCTAssertEqual(0x1560_2D80, decoded.pulseLog[49])
            XCTAssertEqual(0x2860_2300, decoded.pulseLog[54])
    ...
    func testPodInfoPulseLogPlus() {
        ...
        do {
            let decoded =
                try PodInfoPulseLogPlus(
                    encodedData: Data(
                        hexadecimalString: ""
                    )!
                )
            XCTAssertEqual(.pulseLogPlus, decoded.podInfoType)
            XCTAssertEqual(.noFaults, decoded.faultEventCode.faultType)
            ...
            XCTAssertEqual(4, decoded.entrySize)
            XCTAssertEqual(0x3C, decoded.maxEntries)
            XCTAssertEqual(0x5440_2600, decoded.pulseLog[0])
            XCTAssertEqual(0x1541_2B80, decoded.pulseLog[9])
            XCTAssertEqual(0x3D41_2C00, decoded.pulseLog[19])
            XCTAssertEqual(0x0140_2F80, decoded.pulseLog[29])
            XCTAssertEqual(0x2941_2F00, decoded.pulseLog[39])
            XCTAssertEqual(0x5163_2D00, decoded.pulseLog[49])
            XCTAssertEqual(0x1541_2F80, decoded.pulseLog[59])
    ...
    func testPodInfoPulseLogRecent() {
        ...
        do {
            // Decode
            let decoded =
                try PodInfoPulseLogRecent(
                    encodedData: Data(
                        hexadecimalString: ""
                    )!
                )
            XCTAssertEqual(.pulseLogRecent, decoded.podInfoType)
            XCTAssertEqual(134, decoded.indexLastEntry)
            XCTAssertEqual(0x3421_2E00, decoded.pulseLog[0])
            XCTAssertEqual(0x5920_3080, decoded.pulseLog[9])
            XCTAssertEqual(0x1D21_3180, decoded.pulseLog[19])
            XCTAssertEqual(0x4521_3300, decoded.pulseLog[29])
            XCTAssertEqual(0x0921_3180, decoded.pulseLog[39])
            XCTAssertEqual(0x3121_3400, decoded.pulseLog[49])
        } catch {
            XCTFail("message decoding threw error: \(error)")
        }
    }
    ...
    func testPodInfoPulseLogPrevious() {
        ...
        do {
            // Decode
            let decoded =
                try PodInfoPulseLogPrevious(
                    encodedData: Data(
                        hexadecimalString: ""
                    )!
                )
            XCTAssertEqual(.pulseLogPrevious, decoded.podInfoType)
            XCTAssertEqual(50, decoded.nEntries)
            XCTAssertEqual(0x1460_2500, decoded.pulseLog[0])
            XCTAssertEqual(0x3961_2A80, decoded.pulseLog[9])
            XCTAssertEqual(0x1D60_2680, decoded.pulseLog[19])
            XCTAssertEqual(0x4520_2C00, decoded.pulseLog[29])
            XCTAssertEqual(0x0920_2D80, decoded.pulseLog[39])
            XCTAssertEqual(0x2031_0082, decoded.pulseLog[49])
        } catch {
            XCTFail("message decoding threw error: \(error)")
        }
    }
    ...
    func testPodFault12() {
        ...
        do {
            // Decode
            let faultEvent =
                try DetailedStatus(encodedData: Data(hexadecimalString: "020d00000000000012ffff03ff000000008792070000")!)
