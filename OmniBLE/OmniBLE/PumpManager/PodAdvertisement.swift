...
struct PodAdvertisement {
    ...
    var pairable: Bool {
        serviceUUIDs.count >= 5 && serviceUUIDs[3].uuidString == "FFFF" && serviceUUIDs[4].uuidString == "FFFE"
    }
    ...
    init?(_ advertisementData: [String: Any]) {
        ...
        guard let decodedSeqNo = UInt32(
            lotSeqString[lotSeqString.index(lotSeqString.startIndex, offsetBy: 2) ..< lotSeqString.endIndex],
            radix: 16
        ) else {
            return nil
        }
