...
public class PodComms: CustomDebugStringConvertible {
    ...
    private var isPaired: Bool {
        podState?.ltk != nil && (podState?.ltk.count ?? 0) > 0
    }
    ...
    private func sendPairMessage(transport: PodMessageTransport, message: Message) throws -> VersionResponse {
        ...
        log.debug(
            "sendPairMessage: attempting to use PodMessageTransport %@ to send message %@",
            String(reflecting: transport),
    ...
    private func pairPod(insulinType: InsulinType) throws {
        ...
        let ids = Ids(myId: myId, podId: podId)
        ...
        let transport = PodMessageTransport(manager: manager, myId: myId, podId: podId, state: messageTransportState)
        ...
        log.debug(
            "pairPod: creating PodState for versionResponse %{public}@ and transport %{public}@",
            String(describing: versionResponse),
            String(describing: transport.state)
        )
        podState = PodState(
            address: podId,
            ltk: ltk,
        ...
        log.debug("pairPod: self.PodState messageTransportState now: %@", String(reflecting: podState?.messageTransportState))
    }
    ...
    private func establishSession(ltk: Data, eapSeq: Int, msgSeq: Int = 1) throws -> MessageTransportState? {
        ...
        case let .SessionNegotiationResynchronization(keys):
            ...
            if podState != nil {
                ...
                podState!.messageTransportState.eapSeq = eapSeq
            }
            return nil
        case let .SessionKeys(keys):
            ...
            let omnipodMessageNumber = podState?.messageTransportState.messageNumber ?? 0
            ...
            if podState != nil {
                ...
                podState!.messageTransportState = messageTransportState
            } else {
                log.debug(
                    "Used keys %@ to create messageTransportState: %@",
                    String(reflecting: keys),
                    String(reflecting: messageTransportState)
                )
            }
    ...
    private func establishNewSession() throws {
        ...
        let mts = try establishSession(ltk: podState!.ltk, eapSeq: podState!.incrementEapSeq())
        if mts == nil {
            let mts = try establishSession(ltk: podState!.ltk, eapSeq: podState!.incrementEapSeq())
            if mts == nil {
    ...
    private func setupPod(timeZone: TimeZone) throws {
        ...
        let transport = PodMessageTransport(manager: manager, myId: myId, podId: podId, state: podState!.messageTransportState)
        transport.messageLogger = messageLogger
        ...
        log.debug(
            "setupPod: calling sendPairMessage %@ for message %@",
            String(reflecting: transport),
    ...
    func pairAndSetupPod(
        ...
        manager.runSession(withName: "Pair and setup pod") { [weak self] in
            do {
                ...
                let transport = PodMessageTransport(
                    manager: manager,
                    myId: myId,
                    podId: podId,
                    state: self.podState!.messageTransportState
                )
...
extension PodComms: PeripheralManagerDelegate {
    func completeConfiguration(for manager: PeripheralManager) throws {
        log.default(
            "PodComms completeConfiguration: isPaired=%{public}@ needsSessionEstablishment=%{public}@",
            String(describing: isPaired),
            String(describing: needsSessionEstablishment)
        )

        if isPaired, needsSessionEstablishment {
            let myId = self.myId
