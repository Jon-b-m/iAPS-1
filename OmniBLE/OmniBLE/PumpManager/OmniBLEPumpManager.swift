...
public class OmniBLEPumpManager: DeviceManager {
    ...
    public init(state: OmniBLEPumpManagerState, dateGenerator: @escaping () -> Date = Date.init) {
        ...
        lockedPodComms = Locked(podComms)
    ...
    public var deviceBLEName: String? {
        podComms.manager?.peripheral.name
    }
    ...
    private func setState(_ changes: (_ state: inout OmniBLEPumpManagerState) -> Void) {
    ...
    private func logDeviceCommunication(_ message: String, type: DeviceLogEntryType = .send) {
        let podAddress = String(format: "%04X", state.podId)
    ...
    private func issueHeartbeatIfNeeded() {
        if provideHeartbeat, dateGenerator().timeIntervalSince(lastHeartbeat) > .minutes(2) {
            pumpDelegate.notify { delegate in
    ...
    func omnipodPeripheralDidDisconnect(peripheral: CBPeripheral, error: Error?) {
        logDeviceCommunication(
            "Pod disconnected \(peripheral.identifier.uuidString) \(String(describing: error))",
            type: .connection
        )
    ...
    func omnipodPeripheralDidFailToConnect(peripheral: CBPeripheral, error: Error?) {
        logDeviceCommunication(
            "Pod failed to connect \(peripheral.identifier.uuidString), \(String(describing: error))",
            type: .connection
        )
    }
...
public extension OmniBLEPumpManager {
    ...
    private func basalDeliveryState(for state: OmniBLEPumpManagerState, at date: Date = Date()) -> PumpManagerStatus
        .BasalDeliveryState
    {
    ...
    func buildPumpStatusHighlight(for state: OmniBLEPumpManagerState, andDate date: Date = Date()) -> PumpStatusHighlight? {
    ...
    func isRunningManualTempBasal(for state: OmniBLEPumpManagerState) -> Bool {
    ...
    func buildPumpLifecycleProgress(for _: OmniBLEPumpManagerState) -> PumpLifecycleProgress? {
        switch podCommState {
        case .active:
            if shouldWarnPodEOL,
    ...
    private func prepForNewPod() {
        ...
        podComms.prepForNewPod(myId: state.controllerId, podId: state.podId)
    }
    ...
    // MARK: Testing
        ...
        private func jumpStartPod(
            ...
            var podState = PodState(
                address: state.podId,
                ltk: fakeLtk,
                firmwareVersion: "jumpstarted",
                bleFirmwareVersion: "jumpstarted",
                lotNo: lotNo,
                lotSeq: lotSeq,
    ...
    internal func connectToNewPod(completion: @escaping (Result<OmniBLE, Error>) -> Void) {
    ...
    func pairAndPrime(completion: @escaping (PumpManagerResult<TimeInterval>) -> Void) {
            ...
            DispatchQueue.global(qos: .userInitiated).asyncAfter(deadline: .now() + .seconds(2)) {
                self.jumpStartPod(lotNo: 135_601_809, lotSeq: 0_800_525, mockFault: mockFaultDuringPairing)
    ...
    func resumingPodSetup() {
        ...
        if !isConnected {
            log.debug("### Pod setup resume pod not connected, sleeping %d seconds", sleepTime)
            sleep(sleepTime)
        }
    ...
    func acknowledgePodAlerts(_ alertsToAcknowledge: AlertSet, completion: @escaping (_ alerts: AlertSet?) -> Void) {
        ...
        podComms.runSession(withName: "Acknowledge Alerts") { result in
    ...
    func setTime(completion: @escaping (OmniBLEPumpManagerError?) -> Void) {
    ...
    func setConfirmationBeeps(newPreference: BeepPreference, completion: @escaping (OmniBLEPumpManagerError?) -> Void) {
    ...
    func setSilencePod(silencePod: Bool, completion: @escaping (OmniBLEPumpManagerError?) -> Void) {
...
extension OmniBLEPumpManager: PumpManager {
    ...
    public static var onboardingSupportedBasalRates: [Double] {
        ...
        (0 ... 600).map { Double($0) / Double(Pod.pulsesPerUnit) }
    }
    ...
    public var insulinType: InsulinType? {
        ...
        set {
            if let insulinType = newValue {
                ...
                podComms.updateInsulinType(insulinType)
            }
        }
    }
    ...
    public func enactBolus(
        ...
        podComms.runSession(withName: "Bolus") { result in
            ...
            let result = session.bolus(
                ...
                programReminderInterval: bolusWasAutomaticIndicator
            )
    ...
    public func updateExpirationReminder(
        ...
        completion: @escaping (OmniBLEPumpManagerError?) -> Void
    ) {
        ...
        podComms.runSession(withName: "Update Expiration Reminder") { result in
    ...
    public func updateLowReservoirReminder(_ value: Int, completion: @escaping (OmniBLEPumpManagerError?) -> Void) {
        ...
        podComms.runSession(withName: "Program Low Reservoir Reminder") { result in
    ...
    private func silenceAcknowledgedAlerts() {
        ...
        if let alert = state.alertsWithPendingAcknowledgment.first {
            if let slot = alert.triggeringSlot {
                podComms.runSession(withName: "Silence already acknowledged alert") { result in
                    switch result {
                    case let .success(session):
                        do {
        ...
        pumpDelegate.notify { delegate in
            ...
            delegate?
                .issueAlert(Alert(
                    identifier: Alert
                        .Identifier(
                            managerIdentifier: OmniBLEPumpManager.podAlarmNotificationIdentifier,
...
extension OmniBLEPumpManager: PodCommsDelegate {
    func podCommsDidEstablishSession(_ podComms: PodComms) {
        podComms.runSession(withName: "Post-connect status fetch") { result in
            switch result {
            case let .success(session):
                _ = try? session.getStatus(noSeqGetStatus: true)
        ...
        for alert in state.activeAlerts {
            if alert.alertIdentifier == alertIdentifier || alert.repeatingAlertIdentifier == alertIdentifier {
                ...
                if let slot = alert.triggeringSlot {
                    ...
                    podComms.runSession(withName: "Acknowledge Alert") { result in
                        switch result {
                        case let .success(session):
                            do {
                                let beepBlock = self.beepMessageBlock(beepType: .beep)
