...
public struct PodState: RawRepresentable, Equatable, CustomDebugStringConvertible {
    ...
    public var dosesToStore: [UnfinalizedDose] {
        finalizedDoses + [unfinalizedTempBasal, unfinalizedSuspend, unfinalizedBolus].compactMap { $0 }
    }
    ...
    public mutating func incrementEapSeq() -> Int {
        messageTransportState.eapSeq += 1
    ...
    public var currentNonce: UInt32 {
        let fixedNonceValue: UInt32 = 0x494E_532E // Dash pods require this particular fixed value
        return fixedNonceValue
    }

    public mutating func resyncNonce(syncWord _: UInt16, sentNonce _: UInt32, messageSequenceNum _: Int) {
    ...
    public init?(rawValue: RawValue) {
        ...
        activeTime = rawValue["activeTime"] as? TimeInterval
           ...
           let messageTransportState = MessageTransportState(rawValue: messageTransportStateRaw)
        {
            ...
            messageTransportState = MessageTransportState(ck: nil, noncePrefix: nil)
        }
