        ...
        RoundedCardScrollView {
            RoundedCard(
                ...
                footer: LocalizedString(
                    "A reminder is configured during Pod setup to notify you in advance of Pod expiration. Set the number of hours advance notice you would like to configure by default when pairing a new Pod.",
                    comment: "Footer text for omnipod reminders section"
                )
            ) {
                ...
                RoundedCard(
                ...
                )
                    {
                        Text(LocalizedString(
                            "Scheduled Reminder",
                            comment: "Title of scheduled reminder card on NotificationSettingsView"
                        ))
                        Divider()
            ...
            RoundedCard(footer: LocalizedString(
                "You will be notified when the amount of insulin in the Pod reaches this level.",
            ...
            RoundedCard<EmptyView>(
                ...
                footer: LocalizedString(
                    "The reminders above will not sound on your device when it is in Silent or Do Not Disturb mode. There are other critical Pod alerts that will sound on your device even when set to Silent or Do Not Disturb mode.\n\nThe Pod will also use audible beeps for all Pod reminders and alerts except when the Pod is Silenced.",
                    comment: "Description text for critical alerts"
                )
            )
        }
        ...
        NavigationLink(
        ...
        )
            {
                RoundedCardValueRow(
                    ...
                    value: insulinQuantityFormatter
                        .string(from: HKQuantity(unit: .internationalUnit(), doubleValue: Double(lowReservoirReminderValue))) ??
                        "",
                    highlightValue: false,
                    disclosure: true
                )
        ...
        Group {
            ...
            NavigationView {
                NotificationSettingsView(
                    ...
                    allowedScheduledReminderDates: [now, now - TimeInterval(hours: 2), now - TimeInterval(hours: 3)],
