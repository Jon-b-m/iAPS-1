...
public struct PodInfoPulseLogRecent: PodInfo {
    ...
    public init(encodedData: Data) throws {
        ...
        nEntries = nLogBytesReturned / MemoryLayout<UInt32>.size
        indexLastEntry = Int((UInt16(encodedData[1]) << 8) | UInt16(encodedData[2]))
        pulseLog = createPulseLog(encodedData: encodedData, logStartByteOffset: logStartByteOffset, nEntries: nEntries)
        data = encodedData
    }
}
...
public struct PodInfoPulseLogPrevious: PodInfo {
    ...
    public init(encodedData: Data) throws {
        ...
        nEntries = Int((UInt16(encodedData[1]) << 8) | UInt16(encodedData[2]))
        // verify we actually got all the reported entries
        if nEntries > nEntriesCalculated {
