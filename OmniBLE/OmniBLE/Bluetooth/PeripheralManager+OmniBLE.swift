...
extension PeripheralManager {
    ...
    func sendHello(myId: UInt32) throws {
        ...
        try writeValue(
            Data([PodCommand.HELLO.rawValue, 0x01, 0x04]) + controllerId,
            for: characteristic,
            type: .withResponse,
            timeout: 5
        )
    ...
    func peekForNack() throws {
        ...
        if cmdQueue.contains(where: { cmd in
            cmd[0] == PodCommand.NACK.rawValue
        }) {
    ...
    func waitForCommand(_ command: PodCommand, timeout: TimeInterval = 5) throws {
        ...
        if !cmdQueue.isEmpty {
            ...
            if command.rawValue != value[0] {
                log.error(
                    "waitForCommand failed. rawValue != value[0] (%d != %d); data=%@",
                    command.rawValue,
                    value[0],
