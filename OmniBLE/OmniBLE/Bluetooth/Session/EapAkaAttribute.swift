...
class EapAkaAttribute {
    ...
    static func parseAttributes(payload: Data) throws -> [EapAkaAttribute] {
        var tail = payload
        var ret = [EapAkaAttribute]()
        while !tail.isEmpty {
            if tail.count < 2 {
            ...
            case .AT_CLIENT_ERROR_CODE:
                ret
                    .append(
                        try EapAkaAttributeClientErrorCode
                            .parse(payload: tail.subdata(in: 2 ..< EapAkaAttributeClientErrorCode.SIZE))
                    )
            case .none:
                throw MessageError.notEnoughData
            }
...
class EapAkaAttributeRand: EapAkaAttribute {
    ...
    override func toData() -> Data {
        Data([EapAkaAttributeType.AT_RAND.rawValue, UInt8(EapAkaAttributeRand.SIZE / SIZE_MULTIPLIER), 0x00, 0x00]) + payload
    }
...
class EapAkaAttributeAutn: EapAkaAttribute {
    ...
    override func toData() -> Data {
        Data([EapAkaAttributeType.AT_AUTN.rawValue, UInt8(EapAkaAttributeAutn.SIZE / SIZE_MULTIPLIER), 0x00, 0x00]) + payload
    }
...
class EapAkaAttributeAuts: EapAkaAttribute {
    ...
    override func toData() -> Data {
        Data([EapAkaAttributeType.AT_AUTS.rawValue, UInt8(EapAkaAttributeAuts.SIZE / SIZE_MULTIPLIER), 0x00, 0x00]) + payload
    }
...
class EapAkaAttributeRes: EapAkaAttribute {
    ...
    private static let PAYLOAD_SIZE_BITS = 0x64 // type, size, 2 reserved bytes, payload
}
...
class EapAkaAttributeCustomIV: EapAkaAttribute {
    ...
    override func toData() -> Data {
        Data([EapAkaAttributeType.AT_CUSTOM_IV.rawValue, UInt8(EapAkaAttributeCustomIV.SIZE / SIZE_MULTIPLIER), 0x00, 0x00]) +
            payload
    }
...
class EapAkaAttributeClientErrorCode: EapAkaAttribute {
    ...
    override func toData() -> Data {
        Data([
            EapAkaAttributeType.AT_CLIENT_ERROR_CODE.rawValue,
            UInt8(EapAkaAttributeClientErrorCode.SIZE / SIZE_MULTIPLIER),
            0x00,
            0x00
        ]) + payload
    }
