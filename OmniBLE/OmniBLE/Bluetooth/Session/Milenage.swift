...
class Milenage {
    ...
    init(
        ...
        sqn: Data,
        randParam: Data? = nil,
        auts: Data = Data(repeating: 0x00, count: 14),
        amf: Data = Milenage.MILENAGE_AMF
    ) throws {
        guard k.count == Milenage.KEY_SIZE
        else {
            throw MilenageError.Error("Milenage key has to be \(Milenage.KEY_SIZE) bytes long. Received: \(k.hexadecimalString)")
        }
        guard sqn.count == Milenage.SQN
        else { throw MilenageError.Error("Milenage SQN has to be \(Milenage.SQN) long. Received: \(sqn.hexadecimalString)") }
        guard auts.count == Milenage.AUTS_SIZE
        else {
            throw MilenageError.Error("Milenage AUTS has to be \(Milenage.AUTS_SIZE) long. Received: \(auts.hexadecimalString)") }
        guard amf.count == Milenage.MILENAGE_AMF.count else {
            throw MilenageError.Error(
                "Milenage AMF has to be ${MILENAGE_AMF.count} long." +
                    "Received: ${amf.toHex()}"
            )
        }
        self.k = k
        ...
        var ckInput = [UInt8](repeating: 0x00, count: Milenage.KEY_SIZE)
        ...
        var macAInput = [UInt8](repeating: 0x00, count: Milenage.KEY_SIZE)
        ...
        var akStarInput = [UInt8](repeating: 0x00, count: Milenage.KEY_SIZE)
...
extension Data {
    static func ^ (left: Data, right: Data) -> Data {
        var out = [UInt8](repeating: 0x00, count: left.count)
