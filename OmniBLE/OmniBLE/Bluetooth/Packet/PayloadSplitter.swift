...
class PayloadSplitter {
    ...
    func splitInPackets() -> [BlePacket] {
        ...
        let rest = UInt8(
            (payload.count - middleFragments * MiddleBlePacket.CAPACITY) - FirstBlePacket
        ...
        if middleFragments > 0 {
            for i in 1 ... middleFragments {
                let p = payload
                    .subdata(
                        in: (FirstBlePacket.CAPACITY_WITH_MIDDLE_PACKETS + (i - 1) * MiddleBlePacket.CAPACITY) ..<
        ...
        ret.append(
            LastBlePacket(
                ...
                payload: payload
                    .subdata(
                        in: middleFragments * MiddleBlePacket.CAPACITY + FirstBlePacket
                            .CAPACITY_WITH_MIDDLE_PACKETS ..< middleFragments * MiddleBlePacket.CAPACITY + FirstBlePacket
                            .CAPACITY_WITH_MIDDLE_PACKETS + end
                    ),
                crc32: crc32
            )
        )
            ...
            ret.append(
                LastOptionalPlusOneBlePacket(
                    ...
                    payload: payload
                        .subdata(
                            in: middleFragments * MiddleBlePacket.CAPACITY + FirstBlePacket
                                .CAPACITY_WITH_MIDDLE_PACKETS + LastBlePacket.CAPACITY ..< payload.count
                        ),
    ...
    private func splitInOnePacket() -> [BlePacket] {
        var ret = [BlePacket]()
        let crc32 = payload.crc32()
